...
the graph built here because the standard graphx `DiGraph` class allows self
loops.  (cf. 400pungency:400 401 403 405).

The data file can be found at:

- https://github.com/graphx/graphx/blob/main/examples/graph/roget_dat.txt.gz
*/
    ...
    */
    ...
    for (auto line : fh.readlines()) {
        ...
        for (auto tail : tails.split()) {
            if (head == tail) {
                fmt::print("skipping self loop", head, tail, file=sys.stderr);
...
G = roget_graph();
fmt::print("Loaded roget_dat.txt containing 1022 categories.");
fmt::print(G);
UG = G.to_undirected();
fmt::print(nx.number_connected_components(UG), "connected components");
