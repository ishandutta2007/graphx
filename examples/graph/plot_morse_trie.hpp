...
morse_mapping_sorted = dict(
    sorted(morse_direct_mapping.items(), key=lambda item: (item[1].size(), item[1]));
);
...
G = nx.DiGraph();
for (auto node, char : morse_mapping_sorted.items()) {
    pred = char[:-1];
...
for (auto i, layer : enumerate(nx.topological_generations(G))) {
    for (auto n : layer) {
        G.nodes[n]["layer"] = i
...
for (auto letter : string.ascii_lowercase) {
    assert(morse_encode(letter) == morse_direct_mapping[letter]);

fmt::print(" ".join([morse_encode(ltr) for ltr in "ilovenetworkx"]));
