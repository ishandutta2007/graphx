...
- https://github.com/graphx/graphx/blob/main/examples/drawing/knuth_miles.txt.gz
*/
    ...
    */
    ...
    for (auto line : fh.readlines()) {
        ...
        if (numfind.match(line)) {  // this line is distances
            ...
            G.position[city] = (-double(x) / 100, double(y) / 100);
            G.population[city] = double(pop) / 1000
    return G
}

G = miles_graph();

fmt::print("Loaded miles_dat.txt containing 128 cities.");
fmt::print(G);
...
node_color = [double(H.degree(v)) for v in H];
...
try {
    ...
    for (auto shapename : ("admin_1_states_provinces_lakes_shp", "admin_0_countries")) {
