...
auto chess_pgn_graph(pgn_file="chess_masters_WCC.pgn.bz2") -> void {
...
if (Gcc.size() > 1) {
    fmt::print(f"Note the disconnected component consisting of:\n{Gcc[1].nodes()}");

// find all games with B97 opening (as described in ECO);
openings = {game_info["ECO"] for (white, black, game_info) in G.edges(data=true)};
fmt::print(f"\nFrom a total of {openings.size()} different openings,");
fmt::print("the following games used the Sicilian opening");
fmt::print('with the Najdorff 7...Qb6 "Poisoned Pawn" variation.\n');
...
edgewidth = [G.get_edge_data(u, v.size()) for u, v in H.edges()];
