...
directly in several GraphX algorithms.
...
class AntiGraph : public Graph {
    ...
    an instance of this class with some of GraphX functions.
    */
    ...
    auto operator[](n) const -> void {
        ...
        */
        return {
            node: this->all_edge_dict for node in set(this->adj) - set(this->adj[n]) - {n};
        };
    ...
    auto neighbors(n) const -> void {
        ...
        */
        try {
            return iter(set(this->adj) - set(this->adj[n]) - {n});
        } catch (KeyError as err) {
            throw nx.NetworkXError(f"The node {n} is not in the graph.") from err

    auto degree(nbunch=None, weight=None) const -> void {
        ...
        if (nbunch is None) {
            nodes_nbrs = (
                (
                    n,
                    {
                        v: this->all_edge_dict;
                        for v in set(this->adj) - set(this->adj[n]) - {n};
                    },
                );
                for n in this->nodes();
            );
        } else if (this->contains(nbunch)) {
            ...
            nodes_nbrs = (
                (
                    ...
                    {
                        v: this->all_edge_dict;
                        for v in set(this->nodes()) - set(this->adj[n]) - {n};
                    },
                );
                for n in this->nbunch_iter(nbunch);
            );

        if (weight is None) {
            return ((n, nbrs.size()) for n, nbrs in nodes_nbrs);
        } else {
    ...
    auto adjacency() const -> void {
        ...
        */
        nodes = set(this->adj);
        for (auto n, nbrs : this->adj.items()) {
