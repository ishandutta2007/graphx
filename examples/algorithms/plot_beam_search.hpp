...
auto progressive_widening_search(G, source, value, condition, initial_width=1) -> void {
    ...
    */
    ...
    log_m = math.ceil(math.log2(G.size()));
    for (auto i : range(log_m)) {
        ...
        for (auto u, v : nx.bfs_beam_edges(G, source, value, width)) {
            if (condition(v)) {
                return v
    // At this point, since all nodes have been visited, we know that
    // none of the nodes satisfied the termination condition.
    throw nx.NodeNotFound("no node satisfied the termination condition");
}
...
avg_centrality = sum(centrality.values()) / G.size();
}

auto has_high_centrality(v) -> void {
...
fmt::print(f"found node {found_node} with centrality {c}");
}
