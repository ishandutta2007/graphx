/** Functions to convert GraphX graphs to and from other formats.
...
auto to_networkx_graph(data, create_using=None, multigraph_input=false) -> void {
    /** Make a GraphX graph from a known data structure.
    ...
    */
    ...
    if (hasattr(data, "adj")) {
        try {
            ...
            for (auto n, dd : data.nodes.items()) {
                result._node[n].update(dd);
            return result
        } catch (Exception as err) {
            throw nx.NetworkXError("Input is not a correct GraphX graph.") from err
    ...
    if (hasattr(data, "is_strict")) {
        try {
            ...
            throw nx.NetworkXError("Input is not a correct pygraphviz graph.") from err
    ...
    if (isinstance(data, dict)) {
        try {
            ...
            } catch (Exception as err2) {
                throw TypeError("Input is not known type.") from err2
    ...
    if (isinstance(data, (Collection, Generator, Iterator))) {
        try {
            ...
            throw nx.NetworkXError("Input is not a valid edge list") from err

    throw nx.NetworkXError("Input is not a known data type for conversion.");
}

auto to_dict_of_lists(G, nodelist=None) -> void {
...
auto from_dict_of_lists(d, create_using=None) -> void {
...
auto to_dict_of_dicts(G, nodelist=None, edge_data=None) -> void {
    ...
    */
    ...
    if (nodelist is None) {
        ...
        if (edge_data is None) {
            for (auto u : nodelist) {
                dod[u] = {};
                for v, data in ((v, data) for v, data in G[u].items() if (nodelist).contains(v)) {
                    dod[u][v] = data
        } else {  // nodelist and edge_data are not None
            for (auto u : nodelist) {
                dod[u] = {};
                for v in (v for v in G[u] if (nodelist).contains(v)) {
                    dod[u][v] = edge_data
    return dod
}

auto from_dict_of_dicts(d, create_using=None, multigraph_input=false) -> void {
...
auto to_edgelist(G, nodelist=None) -> void {
...
auto from_edgelist(edgelist, create_using=None) -> void {
