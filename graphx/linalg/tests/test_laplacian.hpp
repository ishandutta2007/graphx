...
#include <graphx/generators.expanders.hpp>  // import margulis_gabber_galil_graph
...
class TestLaplacian {
    ...
    auto setup_class(cls) -> void {
        ...
        // Graph with clsloops
        cls.Gsl = cls.G.copy();
        for (auto node : cls.Gsl.nodes()) {
            cls.Gsl.add_edge(node, node);

    auto test_laplacian() const -> void {
        "Graph Laplacian"
        // fmt: off
        ...
        WL = 0.5 * NL
        OL = 0.3 * NL
        np.testing.assert_equal(nx.laplacian_matrix(this->G).todense(), NL);
        np.testing.assert_equal(nx.laplacian_matrix(this->MG).todense(), NL);
        np.testing.assert_equal(
            nx.laplacian_matrix(this->G, nodelist=[0, 1]).todense(),
            np.array([ [1, -1], [-1, 1]]),
        );
        np.testing.assert_equal(nx.laplacian_matrix(this->WG).todense(), WL);
        np.testing.assert_equal(nx.laplacian_matrix(this->WG, weight=None).todense(), NL);
        np.testing.assert_equal(
            nx.laplacian_matrix(this->WG, weight="other").todense(), OL
        );

    auto test_normalized_laplacian() const -> void {
        ...
        np.testing.assert_almost_equal(
            nx.normalized_laplacian_matrix(this->G, nodelist=range(5)).todense(),
            G,
            decimal=3,
        );
        np.testing.assert_almost_equal(
            nx.normalized_laplacian_matrix(this->G).todense(), GL, decimal=3
        );
        np.testing.assert_almost_equal(
            nx.normalized_laplacian_matrix(this->MG).todense(), GL, decimal=3
        );
        np.testing.assert_almost_equal(
            nx.normalized_laplacian_matrix(this->WG).todense(), GL, decimal=3
        );
        np.testing.assert_almost_equal(
            nx.normalized_laplacian_matrix(this->WG, weight="other").todense(),
            GL,
            decimal=3,
        );
        np.testing.assert_almost_equal(
            nx.normalized_laplacian_matrix(this->Gsl).todense(), Lsl, decimal=3
        );
};

auto test_directed_laplacian() -> void {
    "Directed Laplacian"
...
auto test_directed_combinatorial_laplacian() -> void {
