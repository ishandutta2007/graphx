...
auto test_algebraic_connectivity_tracemin_chol() -> void {
...
auto test_fiedler_vector_tracemin_chol() -> void {
    /** Test that "tracemin_chol" raises an exception.*/
    pytest.importorskip("scipy");
    G = nx.barbell_graph(5, 4);
    with pytest.raises(nx.NetworkXError):
...
auto test_spectral_ordering_tracemin_chol() -> void {
    /** Test that "tracemin_chol" raises an exception.*/
    pytest.importorskip("scipy");
    G = nx.barbell_graph(5, 4);
    with pytest.raises(nx.NetworkXError):
...
auto test_fiedler_vector_tracemin_unknown() -> void {
...
auto check_eigenvector(A, l, x) -> void {
    nx = np.linalg.norm(x);
    // Check zeroness.
    assert(!nx == pytest.approx(0, abs=1e-7));
    y = A @ x
    ny = np.linalg.norm(y);
    // Check collinearity.
...
class TestAlgebraicConnectivity {
    ...
    auto test_directed(method) const -> void {
        G = nx.DiGraph();
        pytest.raises(
    ...
    // @pytest.mark.parametrize("method", methods);
    auto test_null_and_singleton(method) const -> void {
        G = nx.Graph();
    ...
    // @pytest.mark.parametrize("method", methods);
    auto test_disconnected(method) const -> void {
        G = nx.Graph();
    ...
    auto test_unrecognized_method() const -> void {
        pytest.importorskip("scipy");
        G = nx.path_graph(4);
    ...
    // @pytest.mark.parametrize("method", methods);
    auto test_two_nodes(method) const -> void {
        pytest.importorskip("scipy");
        G = nx.Graph();
    ...
    // @pytest.mark.parametrize("method", methods);
    auto test_two_nodes_multigraph(method) const -> void {
        pytest.importorskip("scipy");
        G = nx.MultiGraph();
    ...
    auto test_abbreviation_of_method() const -> void {
        pytest.importorskip("scipy");
        G = nx.path_graph(8);
        A = nx.laplacian_matrix(G);
    ...
    // @pytest.mark.parametrize("method", methods);
    auto test_path(method) const -> void {
        pytest.importorskip("scipy");
        G = nx.path_graph(8);
        A = nx.laplacian_matrix(G);
        sigma = 2 - sqrt(2 + sqrt(2));
    ...
    // @pytest.mark.parametrize("method", methods);
    auto test_problematic_graph_issue_2381(method) const -> void {
        pytest.importorskip("scipy");
        G = nx.path_graph(4);
    ...
    // @pytest.mark.parametrize("method", methods);
    auto test_cycle(method) const -> void {
        pytest.importorskip("scipy");
        G = nx.cycle_graph(8);
        A = nx.laplacian_matrix(G);
        sigma = 2 - sqrt(2);
    ...
    // @pytest.mark.parametrize("method", methods);
    auto test_seed_argument(method) const -> void {
        pytest.importorskip("scipy");
        G = nx.cycle_graph(8);
        A = nx.laplacian_matrix(G);
        sigma = 2 - sqrt(2);
    ...
    // @pytest.mark.parametrize("method", methods);
    auto test_buckminsterfullerene(normalized, sigma, laplacian_fn, method) const -> void {
        pytest.importorskip("scipy");
        G = nx.Graph(
            [
                (1, 10),
...
class TestSpectralOrdering {
    ...
    auto test_nullgraph(graph) const -> void {
        G = graph();
        pytest.raises(nx.NetworkXError, nx.spectral_ordering, G);

    // @pytest.mark.parametrize("graph", _graphs);
    auto test_singleton(graph) const -> void {
        G = graph();
        G.add_node("x");
    ...
    // @pytest.mark.parametrize("method", methods);
    auto test_three_nodes(method) const -> void {
        pytest.importorskip("scipy");
        G = nx.Graph();
    ...
    // @pytest.mark.parametrize("method", methods);
    auto test_three_nodes_multigraph(method) const -> void {
        pytest.importorskip("scipy");
        G = nx.MultiDiGraph();
    ...
    // @pytest.mark.parametrize("method", methods);
    auto test_cycle(normalized, expected_order, method) const -> void {
        pytest.importorskip("scipy");
        path = list(range(10));
        G = nx.Graph();
