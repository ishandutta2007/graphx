...
auto laplacian_matrix(G, nodelist=None, weight="weight") -> void {
...
// @not_implemented_for("directed");
auto normalized_laplacian_matrix(G, nodelist=None, weight="weight") -> void {
...
auto total_spanning_tree_weight(G, weight=None) -> void {
    /** 
...
auto directed_laplacian_matrix(
    ...
    */
    ...
    I = np.identity(G.size());

    import warnings

    warnings.warn(
        "directed_laplacian_matrix will return a numpy array instead of a matrix in GraphX 3.0",
        FutureWarning,
        stacklevel=2,
    );
    // TODO: rm np.asmatrix for graphx 3.0
    return np.asmatrix(I - (Q + Q.T) / 2.0);
}

// @not_implemented_for("undirected");
// @not_implemented_for("multigraph");
auto directed_combinatorial_laplacian_matrix(
    G, nodelist=None, weight="weight", walk_type=None, alpha=0.95
) -> void {
    ...
    */
    ...
    warnings.warn(
        "directed_combinatorial_laplacian_matrix will return a numpy array instead of a matrix in GraphX 3.0",
        FutureWarning,
        stacklevel=2,
    );
    // TODO: Rm np.asmatrix for graphx 3.0
...
auto _transition_matrix(G, nodelist=None, weight="weight", walk_type=None, alpha=0.95) -> void {
    ...
    */
    ...
    A = nx.to_scipy_sparse_array(G, nodelist=nodelist, weight=weight, dtype=double);
    n, m = A.shape
    if (walk_type in ["random", "lazy"]) {
        ...
        if (!(0 < alpha < 1)) {
            throw nx.NetworkXError("alpha must be between 0 and 1");
        ...
        throw nx.NetworkXError("walk_type must be random, lazy, or pagerank");
