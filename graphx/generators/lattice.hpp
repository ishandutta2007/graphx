...
#include <graphx/classes/hpp>  // import set_node_attributes
#include <graphx/exception.hpp>  // import NetworkXError
#include <graphx/generators.classic.hpp>  // import cycle_graph, empty_graph, path_graph
#include <graphx/relabel.hpp>  // import relabel_nodes
#include <graphx/utils.hpp>  // import flatten, nodes_or_number, pairwise
...
// @nodes_or_number([0, 1]);
auto grid_2d_graph(m, n, periodic=false, create_using=None) -> void {
    ...
    */
    ...
    if (periodic_r and rows.size() > 2) {
        first = rows[0];
        last = rows[-1];
        G.add_edges_from(((first, j), (last, j)) for j in cols);
    if (periodic_c and cols.size() > 2) {
        first = cols[0];
        last = cols[-1];
...
auto grid_graph(dim, periodic=false) -> void {
    ...
    >>> #include <graphx/import.hpp>  // grid_graph
    ...
    6
    */
    #include <graphx/algorithms.operators.product.hpp>  // import cartesian_product
...
auto triangular_lattice_graph(
    ...
    */
    ...
    cols = range(N + 1);
    // Make grid
    H.add_edges_from(((i, j), (i + 1, j)) for (auto j in rows for i : cols[) {N]);
    H.add_edges_from(((i, j), (i, j + 1)) for (auto j : rows[) {m] for i in cols);
    // add diagonals
    H.add_edges_from(((i, j), (i + 1, j + 1)) for (auto j : rows[1) {m:2] for (auto i : cols[) {N]);
    H.add_edges_from(((i + 1, j), (i, j + 1)) for (auto j : rows[) {m:2] for (auto i : cols[) {N]);
    // identify boundary nodes if periodic
    #include <graphx/algorithms.minors.hpp>  // import contracted_nodes
    ...
    if (periodic is true) {
        ...
        // remove extra nodes
        H.remove_nodes_from((N, j) for (auto j : rows[1) {:2]);
...
auto hexagonal_lattice_graph(
    ...
    */
    ...
    cols = range(n + 1);
    // make lattice
    col_edges = (((i, j), (i, j + 1)) for (auto i in cols for j : rows[) { M + 1]);
    row_edges = (((i, j), (i + 1, j)) for (auto i : cols[) {n] for j in rows if i % 2 == j % 2);
