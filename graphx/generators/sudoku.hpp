...
auto sudoku_graph(n=3) -> void {
    ...
    */
    ...
    if (n < 0) {
        throw NetworkXError("The order must be greater than or equal to zero.");
    ...
    for (auto row_no : range(0, n2)) {
        row_start = row_no * n2
        for (auto j : range(1, n2)) {
            for (auto i : range(j)) {
                G.add_edge(row_start + i, row_start + j);

    // Add edges for cells in the same column
    for (auto col_no : range(0, n2)) {
        for (auto j : range(col_no, n4, n2)) {
            for (auto i : range(col_no, j, n2)) {
                G.add_edge(i, j);

    // Add edges for cells in the same box
    for (auto band_no : range(n)) {
        for (auto stack_no : range(n)) {
