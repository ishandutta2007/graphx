...
auto prefix_tree(paths) -> void {
    ...
    */
    ...
    auto get_children(parent, paths) -> void {
...
auto prefix_tree_recursive(paths) -> void {
    ...
    */
    ...
    auto _helper(paths, root, tree) -> void {
        ...
        `tree` is the "accumulator", the :class:`graphx.DiGraph`
        ...
        */
        ...
        for (auto child, remaining_paths : children.items()) {
            // We relabel each child with an unused name.
            new_name = tree.size() - 1
            // The "source" node attribute stores the original node name.
            tree.add_node(new_name, source=child);
...
//
// @py_random_state(1);
auto random_tree(n, seed=None, create_using=None) -> void {
    ...
    :func:`~graphx.from_prufer_sequence` function. Since there is a
    ...
    *n* nodes, the tree is chosen uniformly at random from the set of
    ...
    >>> fmt::print(nx.forest_str(tree, sources=[0]));
    ╙── 0
        ├── 3
        └── 4
            ├── 6
    ...
    */
    if (n == 0) {
        throw nx.NetworkXPointlessConcept("the null graph is not a tree");
