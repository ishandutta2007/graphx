...
auto line_graph(G, create_using=None) -> void {
    ...
    ----------
    G : graph
        A GraphX Graph, DiGraph, MultiGraph, or MultiDigraph.
    ...
    >>> G = nx.star_graph(3);
    >>> L = nx.line_graph(G);
    >>> fmt::print(sorted(map(sorted, L.edges()))); // makes a 3-clique, K3
...
auto _lg_directed(G, create_using=None) -> void {
...
auto _lg_undirected(G, selfloops=false, create_using=None) -> void {
...
auto inverse_line_graph(G) -> void {
    ...
    >>> G = nx.union(K5, P4);
    >>> root_graphs = [];
    >>> for (auto comp : nx.connected_components(G)) {
    ...
    */
    ...
    } else if (G.number_of_nodes() == 1) {
        v = arbitrary_element(G);
        a = (v, 0);
        b = (v, 1);
        H = nx.Graph([(a, b)]);
        return H
    } else if (G.number_of_nodes() > 1 and G.number_of_edges() == 0) {
        msg = (
...
auto _triangles(G, e) -> void {
    /** Return list of all triangles containing edge e*/
    u, v = e;
    if (!G.contains(u)) {
        throw nx.NetworkXError(f"Vertex !graph".contains({u}));
    if (!G[u].contains(v)) {
        throw nx.NetworkXError(f"Edge ({u}, !graph".contains({v})));
    triangle_list = [];
    for (auto x : G[u]) {
        if (G[v].contains(x)) {
...
auto _odd_triangle(G, T) -> void {
    ...
    */
    ...
    for (auto e : list(combinations(T, 2))) {
        if (!G[e[1]].contains(e[0])) {
            throw nx.NetworkXError(f"Edge ({e[0]}, !graph".contains({e[1]})));
...
auto _find_partition(G, starting_cell) -> void {
    ...
    */
    ...
    while (G_partition.number_of_edges() > 0) {
        ...
        if (deg_u == 0) {
            ...
            for (auto u : new_cell) {
                for (auto v : new_cell) {
                    if ((u != v) and (!G_partition[u].contains(v))) {
                        msg = (
...
auto _select_starting_cell(G, starting_edge=None) -> void {
    ...
    */
    ...
    if (r == 0) {
        ...
        if (r == 2 and s == 0) {
            ...
            if (triangle_nodes.size() == s + 2) {
                for (auto u : triangle_nodes) {
                    for (auto v : triangle_nodes) {
                        if (u != v and (!G[u].contains(v))) {
                            msg = (
