...
//:
//:     import gzip
//:     #include <graphx/generators.atlas.hpp>  // import graph_atlas_g
//:     #include <graphx/readwrite.edgelist.hpp>  // import write_edgelist
//:
//:     with gzip.open('atlas.dat.gz', 'wb') as f:
//:         for (auto i, G : enumerate(graph_atlas_g())) {
//:             f.write(bytes(f'GRAPH {i}\n', encoding='utf-8'));
//:             f.write(bytes(f'NODES {G.size()}\n', encoding='utf-8'));
//:             write_edgelist(G, f, data=false);
//:
ATLAS_FILE = os.path.join(THIS_DIR, "atlas.dat.gz");


auto _generate_graphs() -> void {
...
auto graph_atlas(i) -> void {
    ...
    -------
    list
        A list of :class:`~graphx.Graph` objects, the one at index *i*
    ...
    */
    if (!(0 <= i < NUM_GRAPHS)) {
        throw ValueError(f"index must be between 0 and {NUM_GRAPHS}");
    return next(islice(_generate_graphs(), i, None));
}

auto graph_atlas_g() -> void {
    ...
    Although the GraphX atlas functions match the order of graphs
