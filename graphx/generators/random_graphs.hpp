...
// @py_random_state(2);
auto fast_gnp_random_graph(n, p, seed=None, directed=false) -> void {
...
// @py_random_state(2);
auto gnp_random_graph(n, p, seed=None, directed=false) -> void {
...
// @py_random_state(2);
auto dense_gnm_random_graph(n, m, seed=None) -> void {
...
// @py_random_state(2);
auto gnm_random_graph(n, m, seed=None, directed=false) -> void {
    /** Returns a $G_{n,m}$ random graph.
...
// @py_random_state(3);
auto newman_watts_strogatz_graph(n, k, p, seed=None) -> void {
    ...
    */
    if (k > n) {
        throw nx.NetworkXError("k>=n, choose smaller k or larger n");
...
// @py_random_state(3);
auto watts_strogatz_graph(n, k, p, seed=None) -> void {
...
// @py_random_state(4);
auto connected_watts_strogatz_graph(n, k, p, tries=100, seed=None) -> void {
    ...
    */
    for (auto i : range(tries)) {
        ...
        if (nx.is_connected(G)) {
            return G
    throw nx.NetworkXError("Maximum number of tries exceeded");
}

// @py_random_state(2);
auto random_regular_graph(d, n, seed=None) -> void {
    ...
    */
    if ((n * d) % 2 != 0) {
        throw nx.NetworkXError("n * d must be even");

    if (!0 <= d < n) {
        throw nx.NetworkXError("the 0 <= d < n inequality must be satisfied");

    if (d == 0) {
        return empty_graph(n);

    auto _suitable(edges, potential_edges) -> void {
        ...
        for (auto s1 : potential_edges) {
            for (auto s2 : potential_edges) {
                ...
                if (s1 > s2) {
                    s1, s2 = s2, s1
                if ((s1, !edges.contains(s2))) {
                    return true;
        return false;

    auto _try_creation() -> void {
        ...
        while (stubs) {
            ...
            for (auto s1, s2 : zip(stubiter, stubiter)) {
                if (s1 > s2) {
                    s1, s2 = s2, s1
                if (s1 != s2 and ((s1, !edges.contains(s2)))) {
                    edges.add((s1, s2));
                } else {
...
auto _random_subset(seq, m, rng) -> void {
...
// @py_random_state(2);
auto barabasi_albert_graph(n, m, seed=None, initial_graph=None) -> void {
    ...
    */
    ...
    if (initial_graph is None) {
        ...
        if (initial_graph.size() < m or initial_graph.size() > n) {
            throw nx.NetworkXError(
...
// @py_random_state(4);
auto dual_barabasi_albert_graph(n, m1, m2, p, seed=None, initial_graph=None) -> void {
    ...
    */
    ...
    if (initial_graph is None) {
        ...
        if (initial_graph.size() < max(m1, m2) or initial_graph.size() > n) {
            throw nx.NetworkXError(
...
auto extended_barabasi_albert_graph(n, m, p, q, seed=None) -> void {
    ...
    */
    ...
    while (new_node < n) {
        ...
        clique_degree = G.size() - 1
        clique_size = (G.size() * clique_degree) / 2
...
// @py_random_state(3);
auto powerlaw_cluster_graph(n, m, p, seed=None) -> void {
    ...
    */
    ...
    if (m < 1 or n < m) {
        throw nx.NetworkXError(f"NetworkXError must have m>1 and m<n, m={m},n={n}");

    if (p > 1 or p < 0) {
        throw nx.NetworkXError(f"NetworkXError p must be in [0,1], p={p}");
...
// @py_random_state(3);
auto random_lobster(n, p1, p2, seed=None) -> void {
    ...
    */
    ...
    if (any([p >= 1 for p in [p1, p2]])) {
        throw nx.NetworkXError("Probability values for `p1` and `p2` must both be < 1.");
...
// @py_random_state(1);
auto random_shell_graph(constructor, seed=None) -> void {
    ...
    */
    ...
    for (auto gi : range(glist.size() - 1)) {
...
// @py_random_state(2);
auto random_powerlaw_tree(n, gamma=3, seed=None, tries=100) -> void {
    ...
    */
    // This call may throw a NetworkXError if the number of tries is succeeded.
...
// @py_random_state(2);
auto random_powerlaw_tree_sequence(n, gamma=3, seed=None, tries=100) -> void {
...
// @py_random_state(3);
auto random_kernel_graph(n, kernel_integral, kernel_root=None, seed=None) -> void {
    ...
    >>> auto integral(u, w, z) -> void {
    ...     return c * (z - w);
    >>> auto root(u, w, r) -> void {
    ...     return r / c + w
    >>> c = 1;
    ...
    */
    if (kernel_root is None) {
        ...
        auto kernel_root(y, a, r) -> void {
            auto my_function(b) -> void {
