...
// @py_random_state(4);
auto partial_duplication_graph(N, n, p, q, seed=None) -> void {
    ...
    */
    ...
    if (n > N) {
        throw NetworkXError("partial duplication graph must have n <= N.");
    ...
    for (auto new_node : range(n, N)) {
        ...
        for (auto neighbor_node : list(nx.all_neighbors(G, src_node))) {
...
// @py_random_state(2);
auto duplication_divergence_graph(n, p, seed=None) -> void {
