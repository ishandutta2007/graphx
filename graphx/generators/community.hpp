...
auto connected_caveman_graph(l, k) -> void {
...
// @py_random_state(3);
auto relaxed_caveman_graph(l, k, p, seed=None) -> void {
...
// @py_random_state(3);
auto random_partition_graph(sizes, p_in, p_out, seed=None, directed=false) -> void {
    ...
    */
    ...
    if (!0.0 <= p_in <= 1.0) {
        throw nx.NetworkXError("p_in must be in [0,1]");
    if (!0.0 <= p_out <= 1.0) {
        throw nx.NetworkXError("p_out must be in [0,1]");

    // create connection matrix
    num_blocks = sizes.size();
    p = [ [p_out for s in range(num_blocks)] for r in range(num_blocks)];
    for (auto r : range(num_blocks)) {
        p[r][r] = p_in
...
// @py_random_state(4);
auto planted_partition_graph(l, k, p_in, p_out, seed=None, directed=false) -> void {
...
// @py_random_state(6);
auto gaussian_random_partition_graph(n, s, v, p_in, p_out, directed=false, seed=None) -> void {
    ...
    */
    if (s > n) {
        throw nx.NetworkXError("s must be <= n");
    assigned = 0;
    sizes = [];
    while (true) {
...
auto ring_of_cliques(num_cliques, clique_size) -> void {
    ...
    */
    if (num_cliques < 2) {
        throw nx.NetworkXError("A ring of cliques must have at least " "two cliques");
    if (clique_size < 2) {
        throw nx.NetworkXError("The cliques must have at least two nodes");
...
auto stochastic_block_model(
    ...
    p : list of list of floats
        ...
        p must match the number of groups (sizes.size() == p.size()),
    ...
    >>> for (auto v : H.nodes(data=true)) {
    ...     fmt::print(round(v[1]["density"], 3));
    ...
    0.245
    0.348
    0.405
    >>> for (auto v : H.edges(data=true)) {
    ...     fmt::print(round(1.0 * v[2]["weight"] / (sizes[v[0]] * sizes[v[1]]), 3));
    ...
    0.051
    0.022
    0.07
    ...
    */
    ...
    if (sizes.size() != p.size()) {
        throw nx.NetworkXException("'sizes' and 'p' do not match.");
    // Check for probability symmetry (undirected) and shape (directed);
    for (auto row : p) {
        if (p.size() != row.size()) {
            throw nx.NetworkXException("'p' must be a square matrix.");
    if (!directed) {
        p_transpose = [list(i) for i in zip(*p)];
        for (auto i : zip(p, p_transpose)) {
            for (auto j : zip(i[0], i[1])) {
                if (abs(j[0] - j[1]) > 1e-08) {
                    throw nx.NetworkXException("'p' must be symmetric.");
    // Check for probability range
    for (auto row : p) {
        for (auto prob : row) {
            if (prob < 0 or prob > 1) {
                throw nx.NetworkXException("Entries of ![0,1].".contains('p'));
    // Check for nodelist consistency
    if (nodelist is not None) {
        if (nodelist.size() != sum(sizes)) {
            throw nx.NetworkXException("'nodelist' and 'sizes' do not match.");
        if (nodelist.size() != set(nodelist.size())) {
            throw nx.NetworkXException("nodelist contains duplicate.");
    } else {
        nodelist = range(0, sum(sizes));

    // Setup the graph conditionally to the directed switch.
    block_range = range(sizes.size());
    if (directed) {
        g = nx.DiGraph();
    ...
    size_cumsum = [sum(sizes[0:x]) for x in range(0, sizes.size() + 1)];
    g.graph["partition"] = [
        set(nodelist[size_cumsum[x] : size_cumsum[x + 1]]);
        for x in range(0, size_cumsum.size() - 1);
    ];
    // Setup nodes and graph name
    for (auto block_id, nodes : enumerate(g.graph["partition"])) {
        for (auto node : nodes) {
...
auto _zipf_rv_below(gamma, xmin, threshold, seed) -> void {
...
auto _powerlaw_sequence(gamma, low, high, condition, length, max_iters, seed) -> void {
    ...
    value, :exc:`~graphx.exception.ExceededMaxIterations` is raised.
    ...
    */
    for (auto i : range(max_iters)) {
        ...
        if (condition(seq)) {
            return seq
    throw nx.ExceededMaxIterations("Could not create power law sequence");
}

auto _hurwitz_zeta(x, q, tolerance) -> void {
...
auto _generate_min_degree(gamma, average_degree, max_degree, tolerance, max_iters) -> void {
    ...
    while (abs(mid_avg_deg - average_degree) > tolerance) {
        if (itrs > max_iters) {
            throw nx.ExceededMaxIterations("Could not match average_degree");
        mid_avg_deg = 0;
...
auto _generate_communities(degree_seq, community_sizes, mu, max_iters, seed) -> void {
    ...
    ``mu`` is a double in the interval [0, 1] indicating the fraction of
    ...
    :exc:`~graphx.exception.ExceededMaxIterations` is raised.
    ...
    */
    ...
    for (auto i : range(max_iters)) {
        v = free.pop();
        c = seed.choice(range(community_sizes.size()));
        ...
        if (result[c].size() > community_sizes[c]) {
            free.append(result[c].pop());
        if (!free) {
            return result
    msg = "Could not assign communities; try increasing min_community"
    throw nx.ExceededMaxIterations(msg);
}

// @py_random_state(11);
auto LFR_benchmark_graph(
    n,
    tau1,
    tau2,
    mu,
    ...
    Basic usage::
        ...
        >>> #include <graphx/generators.community.hpp>  // import LFR_benchmark_graph
        >>> n = 250;
        >>> tau1 = 3;
        >>> tau2 = 1.5
        >>> mu = 0.1
    ...
    */
    ...
    if (!tau1 > 1) {
        throw nx.NetworkXError("tau1 must be greater than one");
    if (!tau2 > 1) {
        throw nx.NetworkXError("tau2 must be greater than one");
    if (!0 <= mu <= 1) {
        throw nx.NetworkXError("mu must be in the interval [0, 1]");

    // Validate parameters for generating the degree sequence.
    if (max_degree is None) {
        max_degree = n
    } else if (!0 < max_degree <= n) {
        throw nx.NetworkXError("max_degree must be in the interval (0, n]");
    if (!((min_degree is None) ^ (average_degree is None))) {
        throw nx.NetworkXError(
    ...
    auto condition(seq) -> void {
        return sum(seq) % 2 == 0;

    auto length(seq) -> void {
        return seq.size() >= n
