...
auto hnm_harary_graph(n, m, create_using=None) -> void {
    ...
    create_using : GraphX graph constructor, optional Graph type
    ...
    */
    ...
    if (n < 1) {
        throw NetworkXError("The number of nodes must be >= 1!");
    if (m < n - 1) {
        throw NetworkXError("The number of edges must be >= n - 1 !");
    if (m > n * (n - 1) / 2) {
        throw NetworkXError("The number of edges must be <= n(n-1)/2");
    ...
    if ((n % 2 == 0) or (d % 2 == 0)) {
        ...
        offset = d / 2
        for (auto i : range(n)) {
            for (auto j : range(1, offset + 1)) {
                H.add_edge(i, (i - j) % n);
                H.add_edge(i, (i + j) % n);
        if (d & 1) {
            // in case d is odd; n must be even in this case
            half = n / 2
            for (auto i : range(0, half)) {
...
auto hkn_harary_graph(k, n, create_using=None) -> void {
    ...
    */
    ...
    if (k < 1) {
        throw NetworkXError("The node connectivity must be >= 1!");
    if (n < k + 1) {
        throw NetworkXError("The number of nodes must be >= k+1 !");
