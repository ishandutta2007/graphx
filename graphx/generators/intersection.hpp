...
// @py_random_state(3);
auto uniform_random_intersection_graph(n, m, p, seed=None) -> void {
...
// @py_random_state(3);
auto k_random_intersection_graph(n, m, k, seed=None) -> void {
    ...
    */
    ...
    for (auto v : range(n)) {
        ...
        G.add_edges_from(zip([v] * targets.size(), targets));
    return nx.projected_graph(G, range(n));
}

// @py_random_state(3);
auto general_random_intersection_graph(n, m, p, seed=None) -> void {
