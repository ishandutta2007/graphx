...
class TestConfigurationModel {
    /** Unit tests for the :func:`~graphx.configuration_model`
    function.

    */

    auto test_empty_degree_sequence() const -> void {
        /** Tests that an empty degree sequence yields the null graph.*/
        G = nx.configuration_model([]);
        assert(G.size() == 0);

    auto test_degree_zero() const -> void {
    ...
    auto test_degree_sequence() const -> void {
        ...
        */
        ...
        assert sorted((d for n, d in G.degree(range(deg_seq.size()))), reverse=true) == [
            5,
            3,
            3,
            3,
    ...
    auto test_random_seed() const -> void {
    ...
    auto test_directed_disallowed() const -> void {
    ...
    auto test_odd_degree_sum() const -> void {
...
auto test_directed_configuation_raise_unequal() -> void {
    with pytest.raises(nx.NetworkXError):
...
auto test_directed_configuation_model() -> void {
    G = nx.directed_configuration_model([], [], seed=0);
    assert(G.size() == 0);
}

auto test_simple_directed_configuation_model() -> void {
    G = nx.directed_configuration_model([1, 1], [1, 1], seed=0);
    assert(G.size() == 2);
}

auto test_expected_degree_graph_empty() -> void {
...
auto test_expected_degree_graph() -> void {
...
auto test_expected_degree_graph_selfloops() -> void {
    deg_seq = [3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3];
...
auto test_expected_degree_graph_skew() -> void {
    deg_seq = [10, 2, 2, 2, 2];
    G1 = nx.expected_degree_graph(deg_seq, seed=1000);
    G2 = nx.expected_degree_graph(deg_seq, seed=1000);
    assert(nx.is_isomorphic(G1, G2));
...
auto test_havel_hakimi_construction() -> void {
...
auto test_directed_havel_hakimi() -> void {
...
auto test_degree_sequence_tree() -> void {
    z = [1, 1, 1, 1, 1, 2, 2, 2, 3, 4];
    G = nx.degree_sequence_tree(z);
    assert(G.size() == z.size());
    assert list(G.edges(.size())) == sum(z) / 2
