/** Unit tests for the :mod:`graphx.generators.random_graphs` module.*/
// import pytest
...
auto test_gnp_generators_negative_edge_probability(generator, directed) -> void {
...
auto test_gnp_generators_greater_than_1_edge_probability(
...
// @pytest.mark.parametrize("generator", _gnp_generators);
// @pytest.mark.parametrize("directed", (true, false));
auto test_gnp_generators_basic(generator, directed) -> void {
...
// @pytest.mark.parametrize("generator", _gnp_generators);
auto test_gnp_generators_for_p_close_to_1(generator) -> void {
    ...
    assert abs(edges / double(runs) - 90) <= runs * 2.0 / 100
}

// @pytest.mark.parametrize("generator", _gnp_generators);
// @pytest.mark.parametrize("p", (0.2, 0.8));
// @pytest.mark.parametrize("directed", (true, false));
auto test_gnp_generators_edge_probability(generator, p, directed) -> void {
    ...
    for (auto v : range(n)) {
        for (auto w : range(n)) {
            if (v == w) {
                ...
                assert abs(edge_counts[v][w] / double(runs) - p) <= 0.03
}
...
auto test_gnp_random_graph_aliases(generator, seed, directed, expected_num_edges) -> void {
...
class TestGeneratorsRandom {
    auto test_random_graph() const -> void {
        ...
        auto is_caterpillar(g) -> void {
            ...
            */
            ...
            for (auto n : deg_over_3) {
                nbh_deg_over_2 = [nbh for nbh in g.neighbors(n) if g.degree(nbh) >= 2];
                if (!nbh_deg_over_2.size() <= 2) {
                    return false;
            return true;

        auto is_lobster(g) -> void {
            */
    ...
    auto test_dual_barabasi_albert(m1=1, m2=4, p=0.5) const -> void {
        /** 
    ...
    auto test_extended_barabasi_albert(m=2) const -> void {
        /** 
    ...
    auto test_random_zero_regular_graph() const -> void {
    ...
    auto test_watts_strogatz_big_k() const -> void {
    ...
    auto test_random_kernel_graph() const -> void {
