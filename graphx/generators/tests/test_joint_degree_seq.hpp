...
#include <graphx/algorithms.assortativity.hpp>  // import degree_mixing_dict;
#include <graphx/generators.hpp>  // import gnm_random_graph, powerlaw_cluster_graph
#include <graphx/generators.joint_degree_seq.hpp>  // import (
...
auto test_is_valid_joint_degree() -> void {
    ...
    assert(!is_valid_joint_degree(joint_degrees_1));
    ...
    assert(!is_valid_joint_degree(joint_degrees_2));
    ...
    assert(!is_valid_joint_degree(joint_degrees_3));

    // test condition 5
    // joint_degrees_5[1][1] not even
    joint_degrees_5 = {1: {1: 9}};
    assert(!is_valid_joint_degree(joint_degrees_5));
}

auto test_joint_degree_graph(ntimes=10) -> void {
...
auto test_is_valid_directed_joint_degree() -> void {
...
auto test_directed_joint_degree_graph(n=15, m=100, ntimes=1000) -> void {
    for (auto _ : range(ntimes)) {
