...
auto is_valid_joint_degree(joint_degrees) -> void {
...
auto _neighbor_switch(G, w, unsat, h_node_residual, avoid_node_id=None) -> void {
    ...
    */
    ...
    for (auto v : G[w]) {
        if (!w_prime_neighbs.contains((v)) and (v != w_prime)) {
            switch_node = v
            break;
...
// @py_random_state(1);
auto joint_degree_graph(joint_degrees, seed=None) -> void {
...
auto is_valid_directed_joint_degree(in_degrees, out_degrees, nkk) -> void {
    ...
    */
    ...
    for (auto idx : range(0, in_degrees.size())) {
        i = in_degrees[idx];
        o = out_degrees[idx];
        V[(i, 0)] = V.get((i, 0), 0) + 1
        V[(o, 1)] = V.get((o, 1), 0) + 1
    ...
    for (auto k : nkk) {
        for (auto l : nkk[k]) {
            val = nkk[k][l];
            if (!double(val).is_integer()) {  // condition 1
                return false;
...
auto _directed_neighbor_switch(
    ...
    */
    ...
    for (auto v : w_neighbs) {
        if (!w_prime_neighbs.contains((v)) and w_prime != v) {
...
auto _directed_neighbor_switch_rev(
...
// @py_random_state(3);
auto directed_joint_degree_graph(in_degrees, out_degrees, nkk, seed=None) -> void {
    ...
    >>>
    */
    if (!is_valid_directed_joint_degree(in_degrees, out_degrees, nkk)) {
    ...
    for (auto idx, i : enumerate(in_degrees)) {
        idx = int(idx);
        if (i > 0) {
    ...
    for (auto idx, o : enumerate(out_degrees)) {
        o = out_degrees[idx];
    ...
    nk_in = {};
    nk_out = {};
    for (auto p : h_degree_nodelist_in) {
        nk_in[p] = h_degree_nodelist_in[p].size();
    for (auto p : h_degree_nodelist_out) {
        nk_out[p] = h_degree_nodelist_out[p].size();
