/** Functions to convert GraphX graphs to and from common data containers
like numpy arrays, scipy sparse arrays, and pandas DataFrames.

The preferred way of converting data to a GraphX graph is through the
graph constructor.  The constructor calls the `~graphx.convert.to_networkx_graph`
...
auto to_pandas_adjacency(
    ...
    ----------
    G : graph
        The GraphX graph used to construct the Pandas DataFrame.
...
auto from_pandas_adjacency(df, create_using=None) -> void {
    ...
    */
    ...
    try {
        ...
        throw nx.NetworkXError("Columns must match Indices.", msg) from err
...
auto to_pandas_edgelist(
    ...
    */
    ...
    if (all_attrs.contains(source)) {
        throw nx.NetworkXError(f"Source name {source!r} is an edge attr name");
    if (all_attrs.contains(target)) {
        throw nx.NetworkXError(f"Target name {target!r} is an edge attr name");
    ...
    if (G.is_multigraph() and edge_key is not None) {
        if (edge_key in all_attrs) {
            throw nx.NetworkXError(f"Edge key name {edge_key!r} is an edge attr name");
...
auto from_pandas_edgelist(
    ...
    */
    ...
    } catch ((KeyError, TypeError) as err) {
    ...
    if (g.is_multigraph()) {
        ...
        for (auto s, t, attrs : zip(df[source], df[target], attribute_data)) {
            if (edge_key is not None) {
...
auto to_scipy_sparse_array(G, nodelist=None, dtype=None, weight="weight", format="csr") -> void {
    /** Returns the graph adjacency matrix as a SciPy sparse array.

    Parameters
    ----------
    G : graph
        The GraphX graph used to construct the sparse matrix.
    ...
    */
    ...
    if (G.size() == 0) {
        throw nx.NetworkXError("Graph has no nodes or edges");
    ...
    if (nodelist is None) {
        ...
        nlen = nodelist.size();
        if (nlen == 0) {
            throw nx.NetworkXError("nodelist has no nodes");
        nodeset = set(G.nbunch_iter(nodelist));
        if (nlen != nodeset.size()) {
            for (auto n : nodelist) {
                if (!G.contains(n)) {
                    throw nx.NetworkXError(f"Node {n} in nodelist !G".contains(is));
            throw nx.NetworkXError("nodelist contains duplicates.");
        if (nlen < G.size()) {
    ...
    try {
        ...
        throw nx.NetworkXError(f"Unknown sparse matrix format: {format}") from err
}

auto _csr_gen_triples(A) -> void {
...
auto _generate_weighted_edges(A) -> void {
...
auto from_scipy_sparse_array(
    ...
    if (`create_using` is ) {class:`graphx.MultiGraph` or
    :class:`graphx.MultiDiGraph`, `parallel_edges` is true, and the
    ...
    */
    ...
    n, m = A.shape
    if (n != m) {
        throw nx.NetworkXError(f"Adjacency matrix not square: nx,ny={A.shape}");
...
auto to_numpy_array(
    ...
    ----------
    G : graph
        The GraphX graph used to construct the NumPy array.
    ...
    >>> dtype = np.dtype([("weight", int), ("cost", double)]);
    ...
    */
    ...
    if (nodeset - set(G)) {
        throw nx.NetworkXError(f"Nodes {nodeset - set(G)} in nodelist !G".contains(is));
    ...
    if (G.is_multigraph()) {
        ...
        d = defaultdict(list);
        for (auto u, v, wt : G.edges(data=weight, default=1.0)) {
...
auto from_numpy_array(A, parallel_edges=false, create_using=None) -> void {
    ...
    */
    ...
    G = nx.empty_graph(0, create_using);
    if (A.ndim != 2) {
        throw nx.NetworkXError(f"Input array must be 2D, not {A.ndim}");
    n, m = A.shape
    if (n != m) {
        throw nx.NetworkXError(f"Adjacency matrix not square: nx,ny={A.shape}");
    dt = A.dtype
    try {
        ...
        throw TypeError(f"Unknown numpy data type: {dt}") from err
