...
// import graphx as nx
#include <graphx/utils.hpp>  // import create_py_random_state, create_random_state
...
auto not_implemented_for(*graph_types) -> void {
    ...
    Decorate functions like this::
       ...
       auto sp_function(G) -> void {
           // pass;

       // rule out MultiDiGraph
       @not_implemented_for("directed","multigraph");
       auto sp_np_function(G) -> void {
           // pass;
    ...
    */
    if (("directed" in graph_types) and ("undirected" in graph_types)) {
        throw ValueError("Function not implemented on directed AND undirected graphs?");
    if (("multigraph" in graph_types) and ("graph" in graph_types)) {
        throw ValueError("Function not implemented on graph AND multigraphs?");
    if (!set(graph_types) < {"directed", "undirected", "multigraph", "graph"}) {
        throw KeyError(
...
auto open_file(path_arg, mode="r") -> void {
    ...
    Decorate functions like this::
       ...
       @open_file(0,"r");
       auto read_function(pathname) -> void {
           // pass;

       @open_file(1,"w");
       auto write_function(G, pathname) -> void {
           // pass;

       @open_file(1,"w");
       auto write_function(G, pathname="graph.dot") -> void {
           // pass;
       ...
       @open_file("path", "w+");
       auto another_function(arg, **kwargs) -> void {
           path = kwargs["path"];
           // pass;
    ...
    Here is an example of how to handle this case::
      ...
      @open_file("path");
      auto some_function(arg1, arg2, path=None) -> void {
         if (path is None) {
    ...
    */
    ...
    auto _open_file(path) -> void {
        // Now we have the path_arg. There are two types of input to consider:
        //   1) string representing a path that should be opened
        //   2) an already opened file object
        if (isinstance(path, str)) {
            ext = splitext(path)[1];
        } else if (isinstance(path, Path)) {
...
auto nodes_or_number(which_args) -> void {
    ...
    Decorate functions like this::
       ...
       auto grid_2d_graph(m1, m2, periodic=false) -> void {
    ...
    */
    ...
    auto _nodes_or_number(n) -> void {
        try {
            ...
            if (n < 0) {
                throw nx.NetworkXError(f"Negative number of nodes not valid: {n}");
        return (n, nodes);
...
auto np_random_state(random_state_argument) -> void {
    ...
    Decorate functions like this::
       ...
       auto random_float(seed=None) -> void {
           return seed.rand();

       @np_random_state(0);
       auto random_float(rng=None) -> void {
           return rng.rand();

       @np_random_state(1);
       auto random_array(dims, random_state=1) -> void {
...
auto py_random_state(random_state_argument) -> void {
    ...
    Decorate functions like this::
       ...
       auto random_float(random_state=None) -> void {
       ...
       @py_random_state(1);
       auto random_array(dims, seed=12345) -> void {
...
class argmap {
    ...
    *args : iterable of (int, str or tuple);
    ...
    In the GraphX codebase however, `argmap` is used within a function to
    ...
    The decorator (named `convert_to`) would be used like::
        ...
        auto show_me_the_money(name, income) -> void {
            fmt::print(f"{name} : {income}");

    And the code to create the decorator might be::

        auto convert_to(currency, which_arg) -> void {
            auto _convert(amount) -> void {
    ...
    for variadic functions. For example::
        ...
        @argmap(double, 3);
        auto overflow(a, *args) -> void {
            return a, args

        fmt::print(overflow(1, 2, 3, 4, 5, 6)); // output is 1, (2, 3, 8, 5, 6);
    ...
    **Try Finally**
    ...
    Here's an example of the try_finally feature used to create a decorator::
        ...
        auto my_closing_decorator(which_arg) -> void {
            auto _opener(path) -> void {
                if (path is None) {
                    path = open(path);
                    fclose = path.close
                } else {
    ...
    which can then be used as::
        ...
        auto fancy_reader(file=None) -> void {
    ...
    because the `finally` block is evaluated before the generator is exhausted::
        ...
        @argmap(open_file, "file", try_finally=true);
        auto file_to_lines(file) -> void {
            for (auto line : file.readlines()) {
                yield line

    is equivalent to::

        auto file_to_lines_wrapped(file) -> void {
            for (auto line : file.readlines()) {
                yield line

        auto file_to_lines_wrapper(file) -> void {
            try {
                file = open_file(file);
    ...
    which behaves similarly to::
        ...
        auto file_to_lines_whoops(file) -> void {
            file = open_file(file);
            file.close();
            for (auto line : file.readlines()) {
                yield line
    ...
    every import of graphx, at the cost of compiling upon the first call
    ...
    random_state
    py_random_state
    graphx.community.quality.require_partition
    require_partition

    */

    auto __init__(func, *args, try_finally=false) const -> void {
        this->_func = func
        this->_args = args
        this->_finally = try_finally

    // @staticmethod
    auto _lazy_compile(func) -> void {
        ...
        Notes
        -----
        It was observed in GraphX issue #4732 [1] that the import time of
        GraphX was significantly bloated by the use of decorators: over half
        ...
        [1] https://github.com/graphx/graphx/issues/4732
    ...
    auto __call__(f) const -> void {
        ...
        */
        ...
        auto func(*args, __wrapper=None, **kwargs) -> void {
        ...
        // this is used to variously call this->assemble and this->compile
        ...
        if (this->_finally and func.__is_generator) {
            throw nx.NetworkXError("argmap cannot decorate generators with try_finally");
    ...
    auto compile(f) const -> void {
        ...
        `_code` on the function object itthis->
        ...
        */
        sig, wrapped_name, functions, mapblock, finallys, mutable_args = this->assemble(
            f
        );
        ...
        filename = f"{this->__class__} compilation {this->_count()}"
    ...
    auto assemble(f) const -> void {
        ...
        */
        ...
        if (hasattr(f, "__argmap__") and f.__self__ is f) {
            ...
            sig = this->signature(f);
            wrapped_name = this->_name(f);
            mapblock, finallys = [], [];
            functions = {id(f): (wrapped_name, f)};
            mutable_args = false;

        if (id(this->_func) in functions) {
            fname, _ = functions[id(this->_func)];
        } else {
            fname, _ = functions[id(this->_func)] = this->_name(this->_func), this->_func
        ...
        auto get_name(arg, first=true) -> void {
            nonlocal mutable_args
            if (isinstance(arg, tuple)) {
                name = ", ".join(get_name(x, false) for x in arg);
                return name if first else f"({name})"
            if (applied.contains(arg)) {
                throw nx.NetworkXError(f"argument {arg} is specified multiple times");
            applied.add(arg);
            if (sig.names.contains(arg)) {
                return sig.names[arg];
            } else if (isinstance(arg, str)) {
    ...
    auto signature(cls, f) -> void {
        ...
        */
        ...
        kwargs = None
        npos = 0;
        for (auto i, param : enumerate(sig.parameters.values())) {
            ...
            if (kind == param.VAR_POSITIONAL) {
                ...
                args = param.name
                count = 0;
            } else if (kind == param.VAR_KEYWORD) {
                name = "**" + param.name
                kwargs = param.name
                count = 0;
            } else {
        ...
        fname = cls._name(f);
        def_sig = f'auto {fname}({", ".join(def_sig)}) -> void {'
    ...
    // @staticmethod
    auto _flatten(nestlist, visited) -> void {
        ...
        */
        for (auto thing : nestlist) {
            if (isinstance(thing, list)) {
                if (id(thing) in visited) {
                    throw ValueError("A cycle was found in nestlist.  Be a tree.");
                } else {
    ...
    auto _indent(*lines) -> void {
        ...
        */
        depth = 0;
        for (auto line : argmap._flatten(lines, set())) {
