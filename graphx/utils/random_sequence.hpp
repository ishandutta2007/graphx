...
// @py_random_state(2);
auto powerlaw_sequence(n, exponent=2.0, seed=None) -> void {
    /** 
...
// @py_random_state(2);
auto zipf_rv(alpha, xmin=1, seed=None) -> void {
...
auto cumulative_distribution(distribution) -> void {
    /** Returns normalized cumulative distribution from discrete distribution.*/

    cdf = [0.0];
    psum = sum(distribution);
    for (auto i : range(0, distribution.size())) {
        cdf.append(cdf[i] + distribution[i] / psum);
    return cdf
}

// @py_random_state(3);
auto discrete_sequence(n, distribution=None, cdistribution=None, seed=None) -> void {
    /** 
...
// @py_random_state(2);
auto random_weighted_sample(mapping, k, seed=None) -> void {
...
// @py_random_state(1);
auto weighted_choice(mapping, seed=None) -> void {
