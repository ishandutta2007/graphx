...
*/
// import graphx as nx
#include <graphx/classes/coreviews.hpp>  // import (
    FilterAdjacency,
    FilterAtlas,
...
#include <graphx/classes/filters.hpp>  // import no_filter
...
auto generic_graph_view(G, create_using=None) -> void {
    if (create_using is None) {
        newG = G.__class__();
    } else {
        newG = nx.empty_graph(0, create_using);
    if (G.is_multigraph() != newG.is_multigraph()) {
        throw NetworkXError("Multigraph for G must agree with create_using");
    newG = nx.freeze(newG);
...
auto subgraph_view(G, filter_node=no_filter, filter_edge=no_filter) -> void {
    ...
    */
    ...
    if (G.is_multigraph()) {
        ...
        auto reverse_edge(u, v, k=None) -> void {
