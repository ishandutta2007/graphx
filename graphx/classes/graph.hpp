...
#include <graphx/classes/reportviews.hpp>  // import DegreeView, EdgeView, NodeView
    ...
    */
    ...
    auto __init__(incoming_graph_data=None, **attr) const -> void {
        ...
        */
        ...
        this->_node = this->node_dict_factory(); // empty node attribute dict
    ...
    auto __str__() const -> void {
        ...
        */
        return "".join(
            [
                type(*this).__name__,
                f" named {this->name!r}" if this->name else "",
                f" with {this->number_of_nodes()} nodes and {this->number_of_edges()} edges",
            ];
        );
    }
    ...
    auto size() const -> size_t {
        /** Returns the number of nodes in the graph. Use: 'G.size()'.
    ...
    auto add_node(node_for_adding, **attr) const -> void {
        ...
        */
        if (!this->_node.contains(node_for_adding)) {
            ...
            attr_dict = this->_node[node_for_adding] = this->node_attr_dict_factory();
    ...
    auto number_of_nodes() const -> void {
    ...
    auto add_weighted_edges_from(ebunch_to_add, weight="weight", **attr) const -> void {
        ...
        */
        this->add_edges_from(((u, v, {weight: d}) for u, v, d in ebunch_to_add), **attr);
    }
    ...
    auto remove_edges_from(ebunch) const -> void {
        ...
        */
        ...
        for (auto e : ebunch) {
            ...
            if (adj.contains(u) and v in adj[u]) {
                del adj[u][v];
                if (u != v) {  // self loop needs only one entry removed
                    del adj[v][u];
                }
    ...
    auto update(edges=None, nodes=None) const -> void {
        ...
        if (edges is not None) {
            if (nodes is not None) {
                ...
                try {
                    ...
                    this->add_nodes_from(graph_nodes.data());
                    this->add_edges_from(graph_edges.data());
                    this->graph.update(edges.graph);
                }
            }
        } else if (nodes is not None) {
            this->add_nodes_from(nodes);
        } else {
            throw NetworkXError("update needs nodes or edges input");
        }
    }

    auto has_edge(u, v) const -> void {
    ...
    auto get_edge_data(u, v, default=None) const -> void {
    ...
    auto clear_edges() const -> void {
        ...
        [];
        */
        for (auto neighbours_dict : this->_adj.values()) {
    ...
    auto subgraph(nodes) const -> void {
        ...
        G.remove_nodes_from([n for n in G if !set(nodes.contains(n))]);
        ...
        [(0, 1), (1, 2)];
        */
        induced_nodes = nx.filters.show_nodes(this->nbunch_iter(nodes));
        // if already a subgraph, don't make a chain
        subgraph = nx.graphviews.subgraph_view
        if (hasattr(*this, "_NODE_OK")) {
            return subgraph(this->_graph, induced_nodes, this->_EDGE_OK);
        }
        return subgraph(*this, induced_nodes);
    }

    auto edge_subgraph(edges) const -> void {
        ...
        */
        return nx.edge_subgraph(*this, edges);
    }

    auto size(weight=None) const -> void {
        ...
        6.0
        */
        s = sum(d for v, d in this->degree(weight=weight));
    ...
    auto nbunch_iter(nbunch=None) const -> void {
        ...
        "if (*this.contains(nbunch)) {", even after processing with this routine.
        ...
        */
        ...
        } else if (this->contains(nbunch)) {  // if nbunch is a single node
            bunch = iter([nbunch]);
        } else {  // if nbunch is a sequence of nodes

            auto bunch_iter(nlist, adj) -> void {
                try {
                    for (auto n : nlist) {
                        if (adj.contains(n)) {
                            yield n
