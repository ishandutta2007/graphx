...
class BaseMultiDiGraphTester : public BaseMultiGraphTester {
    ...
    auto test_edges_multi() const -> void {
        G = this->K3
    ...
    auto test_out_edges_multi() const -> void {
        G = this->K3
        assert(sorted(G.out_edges()) == [(0, 1), (0, 2), (1, 0), (1, 2), (2, 0), (2, 1)]);
        assert(sorted(G.out_edges(0)) == [(0, 1), (0, 2)]);
        G.add_edge(0, 1, 2);
    ...
    auto test_in_edges_no_keys() const -> void {
        G = this->K3
        assert(sorted(G.in_edges()) == [(0, 1), (0, 2), (1, 0), (1, 2), (2, 0), (2, 1)]);
        assert(sorted(G.in_edges(0)) == [(1, 0), (2, 0)]);
        G.add_edge(0, 1, 2);
    ...
    auto is_shallow(H, G) const -> void {
        // graph
...
class TestMultiDiGraph : public BaseMultiDiGraphTester, _TestMultiGraph {
...
class TestEdgeSubgraph : public _TestMultiGraphEdgeSubgraph {
...
class MultiDiGraphSubClass : public nx.MultiDiGraph {
...
class TestMultiDiGraphSubclass : public TestMultiDiGraph {
    auto setup_method() const -> void {
        this->Graph = MultiDiGraphSubClass
        // build K3
        this->k3edges = [(0, 1), (0, 2), (1, 2)];
        this->k3nodes = [0, 1, 2];
        this->K3 = this->Graph();
        this->K3._succ = this->K3.adjlist_outer_dict_factory(
            {
