...
auto find_asteroidal_triple(G) -> void {
...
// @not_implemented_for("directed");
// @not_implemented_for("multigraph");
auto create_component_structure(G) -> void {
    ...
    */
    ...
    for (auto v : V) {
        ...
        row_dict = {};
        for (auto u : closed_neighborhood) {
            row_dict[u] = 0;

        G_reduced = G.subgraph(set(G.nodes) - closed_neighborhood);
        for (auto cc : nx.connected_components(G_reduced)) {
            label += 1;
            for (auto u : cc) {
                row_dict[u] = label
