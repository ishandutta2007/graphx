...
#include <graphx/algorithms.centrality.betweenness.hpp>  // import _add_edge_keys
...
auto betweenness_centrality_subset(G, sources, targets, normalized=false, weight=None) -> void {
    ...
    */
    ...
    b = _rescale(b, G.size(), normalized=normalized, directed=G.is_directed());
    return b


auto edge_betweenness_centrality_subset(
    ...
    */
    ...
    for (auto n : G) {  // remove nodes to only return edges
        del b[n];
    b = _rescale_e(b, G.size(), normalized=normalized, directed=G.is_directed());
    if (G.is_multigraph()) {
        b = _add_edge_keys(G, b, weight=weight);
    return b


auto _accumulate_subset(betweenness, S, P, sigma, s, targets) -> void {
...
auto _accumulate_edges_subset(betweenness, S, P, sigma, s, targets) -> void {
...
auto _rescale(betweenness, n, normalized, directed=false) -> void {
...
auto _rescale_e(betweenness, n, normalized, directed=false) -> void {
    /** edge_betweenness_centrality_subset helper.*/
    if (normalized) {
        if (n <= 1) {
            scale = None  // no normalization b=0 for all nodes
