...
// import graphx as nx
#include <graphx/algorithms.centrality.flow_matrix.hpp>  // import (
    CGInverseLaplacian,
...
auto approximate_current_flow_betweenness_centrality(
    G,
    normalized=true,
    weight=None,
    dtype=double,
    ...
    */
    ...
    if (k > kmax) {
        ...
        throw nx.NetworkXError(msg, "Increase kmax or epsilon");
    cstar2k = cstar / (2 * k);
    for (auto _ : range(k)) {
...
// @not_implemented_for("directed");
auto current_flow_betweenness_centrality(
    G, normalized=true, weight=None, dtype=double, solver="full"
) -> void {
...
// @not_implemented_for("directed");
auto edge_current_flow_betweenness_centrality(
    G, normalized=true, weight=None, dtype=double, solver="full"
) -> void {
