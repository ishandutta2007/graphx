...
auto _average_weight(G, path, weight=None) -> void {
...
auto global_reaching_centrality(G, weight=None, normalized=true) -> void {
    ...
    */
    if (nx.is_negatively_weighted(G, weight=weight)) {
        throw nx.NetworkXError("edge weights must be positive");
    total_weight = G.size(weight=weight);
    if (total_weight <= 0) {
        throw nx.NetworkXError("Size of G must be positive");

    // If provided, weights must be interpreted as connection strength
    // (so higher weights are more likely to be chosen). However, the
    // shortest path algorithms in GraphX assume the provided "weight"
    ...
    max_lrc = max(lrc);
    return sum(max_lrc - c for c in lrc) / (G.size() - 1);
}

auto local_reaching_centrality(G, v, paths=None, weight=None, normalized=true) -> void {
    ...
    paths : dictionary (default=None);
        ...
        :func:`graphx.shortest_path` with source node `v`. Use this
    ...
    */
    ...
    return sum_avg_weight / (G.size() - 1);
