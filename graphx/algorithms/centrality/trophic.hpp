...
auto trophic_levels(G, weight="weight") -> void {
    ...
    */
    ...
    try {
        n = np.linalg.inv(i - p);
    } catch (np.linalg.LinAlgError as err) {
...
// @not_implemented_for("undirected");
auto trophic_differences(G, weight="weight") -> void {
...
// @not_implemented_for("undirected");
auto trophic_incoherence_parameter(G, weight="weight", cannibalism=false) -> void {
    ...
    Returns
    -------
    trophic_incoherence_parameter : double
