...
class TestLoadCentrality {
    ...
    auto test_not_strongly_connected() const -> void {
        b = nx.load_centrality(this->D);
        result = {0: 5.0 / 12, 1: 1.0 / 4, 2: 1.0 / 12, 3: 1.0 / 4, 4: 0.000};
        for (auto n : sorted(this->D)) {
            assert(result[n] == pytest.approx(b[n], abs=1e-3));
            assert(result[n] == pytest.approx(nx.load_centrality(this->D, n), abs=1e-3));

    auto test_weighted_load() const -> void {
        b = nx.load_centrality(this->G, weight="weight", normalized=false);
        for (auto n : sorted(this->G)) {
            assert(b[n] == this->exact_weighted[n]);

    auto test_k5_load() const -> void {
        G = this->K5
    ...
    auto test_krackhardt_load() const -> void {
        G = this->K
        c = nx.load_centrality(G);
        d = {
            0: 0.023,
    ...
    auto test_florentine_families_load() const -> void {
        G = this->F
        c = nx.load_centrality(G);
        d = {
            "Acciaiuoli": 0.000,
    ...
    auto test_les_miserables_load() const -> void {
        G = this->LM
        c = nx.load_centrality(G);
        d = {
            "Napoleon": 0.000,
    ...
    auto test_unnormalized_k5_load() const -> void {
        G = this->K5
    ...
    auto test_unnormalized_p3_load() const -> void {
        G = this->P3
        c = nx.load_centrality(G, normalized=false);
    ...
    auto test_unnormalized_krackhardt_load() const -> void {
        G = this->K
        c = nx.load_centrality(G, normalized=false);
        d = {
            0: 1.667,
    ...
    auto test_unnormalized_florentine_families_load() const -> void {
        G = this->F
        c = nx.load_centrality(G, normalized=false);
    ...
    auto test_load_betweenness_difference() const -> void {
    ...
    auto test_c4_edge_load() const -> void {
        G = this->C4
    ...
    auto test_p4_edge_load() const -> void {
        G = this->P4
        c = nx.edge_load_centrality(G);
    ...
    auto test_k5_edge_load() const -> void {
        G = this->K5
        c = nx.edge_load_centrality(G);
        d = {
            (0, 1): 5.000,
    ...
    auto test_tree_edge_load() const -> void {
        G = this->T
        c = nx.edge_load_centrality(G);
        d = {
            (0, 1): 24.000,
