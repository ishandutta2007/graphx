...
// import graphx as nx
#include <graphx/algorithms.centrality.betweenness.hpp>  // import (
...
auto group_betweenness_centrality(G, C, normalized=true, weight=None, endpoints=false) -> void {
    ...
    Returns
    -------
    betweenness : list of floats or double
       If C is a single group then return a double. If C is a list with
    ...
    */
    ...
    if (set_v - G.nodes) {  // element(s) of C not in G
        throw nx.NodeNotFound(f"The node(s) {set_v - G.nodes} are in C !G.".contains(but));
        ...
        if (!endpoints) {
            ...
            if (nx.is_directed(G)) {
                if (nx.is_strongly_connected(G)) {
                    scale = c * (2 * v - c - 1);
            } else if (nx.is_connected(G)) {
                scale = c * (2 * v - c - 1);
            if (scale == 0) {
                for (auto group_node1 : group) {
                    for (auto node : D[group_node1]) {
                        if (node != group_node1) {
...
auto _group_preprocessing(G, set_v, weight) -> void {
    sigma = {};
    delta = {};
    D = {};
    ...
    for (auto s : G) {
        ...
        for (auto i : delta[s].keys()) {  // add the paths from s to i and rescale sigma
            if (s != i) {
                delta[s][i] += 1;
    ...
    PB = dict.fromkeys(G);
    for (auto group_node1 : set_v) {
        PB[group_node1] = dict.fromkeys(G, 0.0);
        for (auto group_node2 : set_v) {
            if (!D[group_node1].contains(group_node2)) {
                continue;
            for (auto node : G) {
    ...
    */
    ...
    if (C is not None) {
        ...
        if (C - G.nodes) {  // element(s) of C not in G
            throw nx.NodeNotFound(f"The node(s) {C - G.nodes} are in C !G.".contains(but));
    ...
    if (!endpoints) {
        ...
        if (nx.is_directed(G)) {
            ...
            scale = k * (2 * v - k - 1);
        if (scale == 0) {
            for (auto group_node1 : max_group) {
                for (auto node : D[group_node1]) {
                    if (node != group_node1) {
                        if (max_group.contains(node)) {
                            scale += 1;
                        } else {
                            scale += 2;
        max_GBC -= scale
    ...
    // If undirected then count only the undirected edges
        ...
        max_GBC /= 2;
    max_GBC = double("%.2f" % max_GBC);
    return max_GBC, max_group
}

auto _dfbnb(G, k, DF_tree, max_GBC, root, D, max_group, nodes, greedy) -> void {
    // stopping condition - if we found a group of size k and with higher GBC then prune
    if (DF_tree.nodes[root]["GM"].size() == k and DF_tree.nodes[root]["GBC"] > max_GBC) {
    ...
    if (
        DF_tree.nodes[root]["GM"].size() == k
        or DF_tree.nodes[root]["CL"].size() <= k - DF_tree.nodes[root]["GM"].size();
...
auto _heuristic(k, root, DF_tree, D, nodes, greedy) -> void {
    import numpy as np
    ...
    for (auto i : range(k - DF_tree.nodes[node_p]["GM"].size())) {
    ...
    if (!greedy) {
        ...
        for (auto i : range(k - DF_tree.nodes[node_m]["GM"].size())) {
...
auto group_closeness_centrality(G, S, weight=None) -> void {
    ...
    */
    ...
    for (auto v : V_S) {
        ...
        } catch (KeyError) {  // no path exists
            closeness += 0;
    try {
        closeness = V_S.size() / closeness
    } catch (ZeroDivisionError) {  // 1 / 0 assumed as 0
        closeness = 0;
    return closeness
}

auto group_degree_centrality(G, S) -> void {
    ...
    .. [1] M G Everett and S P Borgatti:
       ...
       http://www.analytictech.com/borgatti/group_centrality.htm
    */
    centrality = set(.size().union(*list(set(G.neighbors(i)) for i in S)) - set(S));
    centrality /= G.nodes(.size()) - S.size();
    return centrality
}

// @not_implemented_for("undirected");
auto group_in_degree_centrality(G, S) -> void {
...
// @not_implemented_for("undirected");
auto group_out_degree_centrality(G, S) -> void {
