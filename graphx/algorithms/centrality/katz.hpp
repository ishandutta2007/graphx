...
auto katz_centrality(
    ...
    >>> for (auto n, c : sorted(centrality.items())) {
    ...     fmt::print(f"{n} {c:.2f}");
    0 0.37
    1 0.60
    2 0.60
    ...
    */
    ...
    } catch ((TypeError, ValueError, AttributeError) as err) {
        b = beta
        if (set(beta) != set(G)) {
...
// @not_implemented_for("multigraph");
auto katz_centrality_numpy(G, alpha=0.1, beta=1.0, normalized=true, weight=None) -> void {
    ...
    */
    ...
    try {
        ...
        nodelist = list(G);
        try {
            b = np.ones((nodelist.size(), 1)) * beta
        } catch ((TypeError, ValueError, AttributeError) as err) {
            throw nx.NetworkXError("beta must be a number") from err
