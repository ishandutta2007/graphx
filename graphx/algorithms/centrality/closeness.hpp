...
auto closeness_centrality(G, u=None, distance=None, wf_improved=true) -> void {
    ...
    In GraphX 2.2 and earlier a bug caused Dijkstra's algorithm to use the
    ...
    */
    ...
    for (auto n : nodes) {
        ...
        if (totsp > 0.0 and len_G > 1) {
            _closeness_centrality = (sp.size() - 1.0) / totsp
            // normalize to number of nodes-1 in connected part
            if (wf_improved) {
                s = (sp.size() - 1.0) / (len_G - 1);
...
auto incremental_closeness_centrality(
    ...
    */
    if (prev_cc is not None and set(prev_cc.keys()) != set(G.nodes())) {
        throw NetworkXError("prev_cc and G do not have the same nodes");
    ...
    closeness_dict = {};
    for (auto n : nodes) {
        if (du.contains(n) and n in dv and abs(du[n] - dv[n]) <= 1) {
