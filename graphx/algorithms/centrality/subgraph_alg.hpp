...
auto subgraph_centrality_exp(G) -> void {
    ...
    >>> fmt::print([f"{node} {sc[node]:0.2f}" for node in sorted(sc)]);
...
// @not_implemented_for("directed");
// @not_implemented_for("multigraph");
auto subgraph_centrality(G) -> void {
...
// @not_implemented_for("directed");
// @not_implemented_for("multigraph");
auto communicability_betweenness_centrality(G) -> void {
    ...
    >>> fmt::print([f"{node} {cbc[node]:0.2f}" for node in sorted(cbc)]);
