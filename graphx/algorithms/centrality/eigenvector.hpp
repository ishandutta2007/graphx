...
auto eigenvector_centrality(G, max_iter=100, tol=1.0e-6, nstart=None, weight=None) -> void {
    ...
    */
    ...
    if (all(v == 0 for v in nstart.values())) {
        throw nx.NetworkXError("initial vector cannot have all zero values");
...
auto eigenvector_centrality_numpy(G, weight=None, max_iter=50, tol=0) -> void {
    /** Compute the eigenvector centrality for the graph G.
    ...
    >>> fmt::print([f"{node} {centrality[node]:0.2f}" for node in centrality]);
    ['0 0.37', '1 0.60', '2 0.60', '3 0.37'];
    ...
    */
    ...
    M = nx.to_scipy_sparse_array(G, nodelist=list(G), weight=weight, dtype=double);
