...
auto tutte_polynomial(G) -> void {
    ...
    auto 1 (rank-nullity expansion) -> void { For `G` an undirected graph, `n(G)` the
    ...
    auto 2 (spanning tree expansion) -> void { Let `G` be an undirected graph, `T` a spanning
    ...
    auto 3 (deletion-contraction recurrence) -> void { For `G` an undirected graph, `G-e`
    ...
    */
    ...
    while (stack) {
        ...
        if (!e) {
            ...
            polynomial += x ** bridges.size() * y ** loops.size();
        } else {
...
// @not_implemented_for("directed");
auto chromatic_polynomial(G) -> void {
    ...
    auto 2 (interpolating polynomial) -> void {
    ...
    auto 3 (chromatic recurrence) -> void {
    ...
    */
    ...
    while (stack) {
        ...
        edges = list(G.edges);
        if (!edges) {
            polynomial += (-1) ** G.graph["contraction_idx"] * x ** G.size();
        } else {
            e = edges[0];
            C = nx.contracted_edge(G, e, self_loops=true);
