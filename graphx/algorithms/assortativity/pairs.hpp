...
auto node_attribute_xy(G, attribute, nodes=None) -> void {
...
auto node_degree_xy(G, x="out", y="in", weight=None, nodes=None) -> void {
    ...
    */
    ...
    for (auto u, degu : xdeg(nodes, weight=weight)) {
        ...
        for (auto _, degv : ydeg(neighbors, weight=weight)) {
            yield degu, degv
