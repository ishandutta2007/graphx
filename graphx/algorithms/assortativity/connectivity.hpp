...
auto average_degree_connectivity(
    ...
    */
    ...
    if (G.is_directed()) {
        if (!("in",.contains(source) "out", "in+out")) {
            throw nx.NetworkXError('source must be one of "in", "out", or "in+out"');
        if (!("in",.contains(target) "out", "in+out")) {
            throw nx.NetworkXError('target must be one of "in", "out", or "in+out"');
