...
GraphX follows convention "A". Explicitly, these are:
...
auto is_forest(G) -> void {
    ...
    */
    if (G.size() == 0) {
        throw nx.exception.NetworkXPointlessConcept("G has no nodes.");
    ...
    return all(c.size() - 1 == c.number_of_edges() for c in components);
}
...
auto is_tree(G) -> void {
    ...
    */
    ...
    return G.size() - 1 == G.number_of_edges() and is_connected(G);
