...
class NotATree : public nx.NetworkXException {
...
auto to_nested_tuple(T, root, canonical_form=false) -> void {
    ...
    */
    ...
    auto _make_tuple(T, root, _parent) -> void {
    ...
    if (!nx.is_tree(T)) {
        throw nx.NotATree("provided graph is not a tree");
    if (!T.contains(root)) {
        throw nx.NodeNotFound(f"Graph {T} contains no node {root}");

    return _make_tuple(T, root, None);
}

auto from_nested_tuple(sequence, sensible_relabeling=false) -> void {
    ...
    */
    ...
    auto _make_tree(sequence) -> void {
        /** Recursively creates a tree from the given sequence of nested
        tuples.

        This function employs the :func:`~graphx.tree.join` function
...
// @not_implemented_for("directed");
auto to_prufer_sequence(T) -> void {
    ...
    *n* - 1, inclusive. The tree corresponding to a given Prüfer
    ...
    the latter form. You can use :func:`~graphx.relabel_nodes` to
...
auto from_prufer_sequence(sequence) -> void {
