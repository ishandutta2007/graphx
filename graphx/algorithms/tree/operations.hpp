...
auto join(rooted_trees, label_attribute=None) -> void {
    ...
    ----------
    rooted_trees : list
        A list of pairs in which each left element is a GraphX graph
    ...
    */
    ...
    lengths = (tree.size() for (auto tree : trees[) {-1]);
