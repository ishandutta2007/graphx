...
auto preflow_push_impl(G, s, t, capacity, residual, global_relabel_freq, value_only) -> void {
    ...
    if (global_relabel_freq < 0) {
        throw nx.NetworkXError("global_relabel_freq must be nonnegative.");
        ...
        */
        ...
        while (q) {
            ...
            for (auto v, attr : R_pred[u].items()) {
                if (!heights.contains(v) and attr["flow"] < attr["capacity"]) {
                    heights[v] = height
                    q.append((v, height));
        return heights
    ...
    auto discharge(u, is_phase1) -> void {
    ...
    auto gap_heuristic(height) -> void {
    ...
    auto global_relabel(from_sink) -> void {
        ...
        if (from_sink) {
            ...
            for (auto u : R) {
                if (!heights.contains(u) and R_nodes[u]["height"] < n) {
                    heights[u] = n + 1
        } else {
        ...
        for (auto u, new_height : heights.items()) {
            ...
            if (new_height != old_height) {
                if (levels[old_height].active.contains(u)) {
    ...
    while (height > 0) {
        ...
        while (true) {
            ...
            if (grt.is_reached()) {
                ...
                max_height = height
                grt.clear_work();
            } else if (!old_level.active and not old_level.inactive) {
...
auto preflow_push(
    ...
    Examples
    --------
    >>> #include <graphx/algorithms.flow.hpp>  // import preflow_push
