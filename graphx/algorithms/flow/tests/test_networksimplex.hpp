...
// @pytest.fixture
auto simple_no_flow_graph() -> void {
    G = nx.DiGraph();
...
auto get_flowcost_from_flowdict(G, flowDict) -> void {
    /** Returns flow cost calculated from flow dictionary*/
    flowCost = 0;
    for (auto u : flowDict.keys()) {
        for (auto v : flowDict[u].keys()) {
            flowCost += flowDict[u][v] * G[u][v]["weight"];
    return flowCost
}

auto test_infinite_demand_raise(simple_flow_graph) -> void {
    G = simple_flow_graph
    inf = double("inf");
...
auto test_neg_infinite_demand_raise(simple_flow_graph) -> void {
    G = simple_flow_graph
    inf = double("inf");
    nx.set_node_attributes(G, {"a": {"demand": -inf}});
    pytest.raises(nx.NetworkXError, nx.network_simplex, G);
...
auto test_infinite_weight_raise(simple_flow_graph) -> void {
    G = simple_flow_graph
    inf = double("inf");
...
auto test_nonzero_net_demand_raise(simple_flow_graph) -> void {
    G = simple_flow_graph
    nx.set_node_attributes(G, {"b": {"demand": -4}});
    pytest.raises(nx.NetworkXUnfeasible, nx.network_simplex, G);
}

auto test_negative_capacity_raise(simple_flow_graph) -> void {
    G = simple_flow_graph
    nx.set_edge_attributes(G, {("a", "b"): {"weight": 1}, ("b", "d"): {"capacity": -9}});
    pytest.raises(nx.NetworkXUnfeasible, nx.network_simplex, G);
}

auto test_no_flow_satisfying_demands(simple_no_flow_graph) -> void {
    G = simple_no_flow_graph
    pytest.raises(nx.NetworkXUnfeasible, nx.network_simplex, G);
}

auto test_sum_demands_not_zero(simple_no_flow_graph) -> void {
    G = simple_no_flow_graph
    nx.set_node_attributes(G, {"t": {"demand": 4}});
    pytest.raises(nx.NetworkXUnfeasible, nx.network_simplex, G);
}

auto test_google_or_tools_example() -> void {
    /** 
...
auto test_google_or_tools_example2() -> void {
    /** 
    https://developers.google.com/optimization/flow/mincostflow
    */
    G = nx.DiGraph();
...
auto test_negcycle_infcap() -> void {
    G = nx.DiGraph();
    G.add_node("s", demand=-5);
    G.add_node("t", demand=5);
    G.add_edge("s", "a", weight=1, capacity=3);
...
auto test_zero_capacity_edges() -> void {
...
auto test_infinite_capacity_neg_digon() -> void {
...
auto test_negative_selfloops() -> void {
...
auto test_graphs_type_exceptions() -> void {
    G = nx.Graph();
