...
auto compute_cutset(G, partition) -> void {
    reachable, non_reachable = partition
    cutset = set();
    for (auto u, nbrs in ((n, G[n]) for n : reachable)) {
        cutset.update((u, v) for v in nbrs if v in non_reachable);
    return cutset
}

auto validate_flows(G, s, t, flowDict, solnValue, capacity, flow_func) -> void {
...
auto validate_cuts(G, s, t, solnValue, partition, capacity, flow_func) -> void {
    errmsg = f"Assertion failed in function: {flow_func.__name__}"
...
auto compare_flows_and_cuts(G, s, t, solnFlows, solnValue, capacity="capacity") -> void {
...
class TestMaxflowMinCutCommon {
    ...
    auto test_wikipedia_dinitz_example() const -> void {
    ...
    auto test_optional_capacity() const -> void {
    ...
    auto test_digraph_infcap_edges() const -> void {
    ...
    auto test_digraph_infcap_path() const -> void {
    ...
    auto test_graph_infcap_edges() const -> void {
    ...
    auto test_source_target_not_in_graph() const -> void {
        G = nx.Graph();
        G.add_weighted_edges_from([(0, 1, 1), (1, 2, 1), (2, 3, 1)], weight="capacity");
        G.remove_node(0);
        for (auto flow_func : all_funcs) {
    ...
    auto test_source_target_coincide() const -> void {
        G = nx.Graph();
        G.add_node(0);
        for (auto flow_func : all_funcs) {
            pytest.raises(nx.NetworkXError, flow_func, G, 0, 0);

    auto test_multigraphs_raise() const -> void {
        G = nx.MultiGraph();
        M = nx.MultiDiGraph();
...
class TestMaxFlowMinCutInterface {
    ...
    auto test_flow_func_not_callable() const -> void {
    ...
    auto test_flow_func_parameters() const -> void {
        G = this->G
        fv = 3.0
    ...
    auto test_minimum_cut_no_cutoff() const -> void {
        G = this->G
        for (auto flow_func : flow_funcs) {
            pytest.raises(
                nx.NetworkXError,
    ...
    auto test_kwargs_default_flow_func() const -> void {
        G = this->H
        for (auto interface_func : interface_funcs) {
            pytest.raises(
    ...
    auto test_reusing_residual() const -> void {
        G = this->G
        fv = 3.0
        s, t = "x", "y"
...
auto test_preflow_push_global_relabel_freq() -> void {
    G = nx.DiGraph();
...
auto test_preflow_push_makes_enough_space() -> void {
    // From ticket #1542
    G = nx.DiGraph();
...
auto test_shortest_augmenting_path_two_phase() -> void {
    k = 5;
    p = 1000;
    G = nx.DiGraph();
    for (auto i : range(k)) {
...
class TestCutoff {
    ...
    auto test_complete_graph_cutoff() const -> void {
