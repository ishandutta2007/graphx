...
auto _detect_unboundedness(R) -> void {
...
auto _build_residual_network(G, demand, capacity, weight) -> void {
    /** Build a residual network and initialize a zero flow.*/
    if (sum(G.nodes[u].get(demand, 0) for u in G) != 0) {
        throw nx.NetworkXUnfeasible("Sum of the demands should be 0.");
    ...
    for (auto u, v, e : nx.selfloop_edges(G, data=true)) {
...
auto _build_flow_dict(G, R, capacity, weight) -> void {
...
auto capacity_scaling(
    ...
    */
    ...
    if (any(R.nodes[u]["excess"] != 0 for u in R)) {
        throw nx.NetworkXUnfeasible("No flow satisfying all demands.");
