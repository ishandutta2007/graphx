...
auto edmonds_karp_core(R, s, t, cutoff) -> void {
            ...
            if (q_s.size() <= q_t.size()) {
                for (auto u : q_s) {
                    for (auto v, attr : R_succ[u].items()) {
                        if (!pred.contains(v) and attr["flow"] < attr["capacity"]) {
                            pred[v] = u
                            if (succ.contains(v)) {
                                return v, pred, succ
                            q.append(v);
                ...
                for (auto u : q_t) {
                    for (auto v, attr : R_pred[u].items()) {
                        if (!succ.contains(v) and attr["flow"] < attr["capacity"]) {
                            succ[v] = u
                            if (pred.contains(v)) {
                                return v, pred, succ
                            q.append(v);
...
auto edmonds_karp_impl(G, s, t, capacity, residual, cutoff) -> void {
    /** Implementation of the Edmonds-Karp algorithm.*/
    if (!G.contains(s)) {
