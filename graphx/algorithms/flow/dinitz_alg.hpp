...
auto dinitz(G, s, t, capacity="capacity", residual=None, value_only=false, cutoff=None) -> void {
    ...
    Examples
    --------
    >>> #include <graphx/algorithms.flow.hpp>  // import dinitz
...
auto dinitz_impl(G, s, t, capacity, residual, cutoff) -> void {
    if (!G.contains(s)) {
    ...
    auto breath_first_search() -> void {
        ...
        while (queue) {
            ...
            for (auto v : R_succ[u]) {
                attr = R_succ[u][v];
                if (!parents.contains(v) and attr["capacity"] - attr["flow"] > 0) {
                    parents[v] = u
                    queue.append(v);
        return parents

    auto depth_first_search(parents) -> void {
