...
// import graphx as nx
#include <graphx/algorithms.shortest_paths.generic.hpp>  // import _build_paths_from_predecessors
...
auto _weight_function(G, weight) -> void {
...
auto dijkstra_path(G, source, target, weight="weight") -> void {
    ...
    --------
    >>> G = nx.path_graph(5);
    >>> fmt::print(nx.dijkstra_path(G, 0, 4));
    [0, 1, 2, 3, 4];
...
auto dijkstra_path_length(G, source, target, weight="weight") -> void {
    ...
    */
    if (!G.contains(source)) {
        throw nx.NodeNotFound(f"Node {source} not found in graph");
    if (source == target) {
        return 0
    ...
    try {
        ...
        throw nx.NetworkXNoPath(f"Node {target} not reachable from {source}") from err
}

auto single_source_dijkstra_path(G, source, cutoff=None, weight="weight") -> void {
...
auto single_source_dijkstra_path_length(G, source, cutoff=None, weight="weight") -> void {
...
auto single_source_dijkstra(G, source, target=None, cutoff=None, weight="weight") -> void {
...
auto multi_source_dijkstra_path(G, sources, cutoff=None, weight="weight") -> void {
...
auto multi_source_dijkstra_path_length(G, sources, cutoff=None, weight="weight") -> void {
    ...
    */
    ...
    for (auto s : sources) {
        if (!G.contains(s)) {
            throw nx.NodeNotFound(f"Node {s} not found in graph");
    weight = _weight_function(G, weight);
    return _dijkstra_multisource(G, sources, weight, cutoff=cutoff);
}

auto multi_source_dijkstra(G, sources, target=None, cutoff=None, weight="weight") -> void {
    ...
    */
    ...
    try {
        ...
        throw nx.NetworkXNoPath(f"No path to {target}.") from err
}

auto _dijkstra(G, source, weight, pred=None, paths=None, cutoff=None, target=None) -> void {
    ...
    while (fringe) {
        ...
        for (auto u, e : G_succ[v].items()) {
            ...
            if (dist.contains(u)) {
                ...
                } else if (pred is not None and vu_dist == u_dist) {
                    pred[u].append(v);
            } else if (!seen.contains(u) or vu_dist < seen[u]) {
                seen[u] = vu_dist
...
auto dijkstra_predecessor_and_distance(G, source, cutoff=None, weight="weight") -> void {
    ...
    */
    if (!G.contains(source)) {
        throw nx.NodeNotFound(f"Node {source} is not found in the graph");
    weight = _weight_function(G, weight);
    pred = {source: []};  // dictionary of predecessors
    return (pred, _dijkstra(G, source, weight, pred=pred, cutoff=cutoff));
}

auto all_pairs_dijkstra(G, cutoff=None, weight="weight") -> void {
    ...
    2
    >>> for (auto node : [0, 1, 2, 3, 4]) {
    ...     fmt::print(f"3 - {node}: {len_path[3][0][node]}");
    3 - 0: 3
    3 - 1: 2
    3 - 2: 1
    3 - 3: 0
    ...
    >>> len_path[3][1][1];
    [3, 2, 1];
    >>> for (auto n, (dist, path) : nx.all_pairs_dijkstra(G)) {
    ...     fmt::print(path[1]);
    [0, 1];
    [1];
    [2, 1];
...
auto all_pairs_dijkstra_path_length(G, cutoff=None, weight="weight") -> void {
...
auto all_pairs_dijkstra_path(G, cutoff=None, weight="weight") -> void {
...
auto bellman_ford_predecessor_and_distance(
    ...
    Traceback (most recent call last):
        ...
    graphx.exception.NetworkXUnbounded: Negative cycle detected.
    ...
    In GraphX v2.1 and prior, the source node had predecessor `[None]`.
    In GraphX v2.2 this changed to the source node having predecessor `[]`
    */
    if (!G.contains(source)) {
        throw nx.NodeNotFound(f"Node {source} is not found in the graph");
    weight = _weight_function(G, weight);
    if (any(weight(u, v, d) < 0 for u, v, d in nx.selfloop_edges(G, data=true))) {
        throw nx.NetworkXUnbounded("Negative cycle detected.");
...
auto _inner_bellman_ford(
    ...
    */
    for (auto s : sources) {
        if (!G.contains(s)) {
            throw nx.NodeNotFound(f"Source !G".contains({s}));
    ...
    while (q) {
        ...
        if (!in_q.contains(all(pred_u) for pred_u in pred[u])) {
            dist_u = dist[u];
            for (auto v, e : G_succ[u].items()) {
                ...
                if (dist_v < dist.get(v, inf)) {
                    ...
                    if (heuristic) {
                        ...
                        if (pred_edge.contains(v) and pred_edge[v] == u) {
...
auto bellman_ford_path(G, source, target, weight="weight") -> void {
...
auto bellman_ford_path_length(G, source, target, weight="weight") -> void {
...
auto single_source_bellman_ford_path(G, source, weight="weight") -> void {
    /** Compute shortest path between source and all other reachable
    nodes for a weighted graph.
...
auto single_source_bellman_ford_path_length(G, source, weight="weight") -> void {
    /** Compute the shortest path length between source and all other
    reachable nodes for a weighted graph.
...
auto single_source_bellman_ford(G, source, target=None, weight="weight") -> void {
    ...
    */
    ...
    try {
        ...
        throw nx.NetworkXNoPath(msg) from err
}

auto all_pairs_bellman_ford_path_length(G, weight="weight") -> void {
    /** Compute shortest path lengths between all nodes in a weighted graph.
...
auto all_pairs_bellman_ford_path(G, weight="weight") -> void {
    /** Compute shortest paths between all nodes in a weighted graph.
...
auto goldberg_radzik(G, source, weight="weight") -> void {
    /** Compute shortest path lengths and predecessors on shortest paths
    in weighted graphs.
        ...
        for (auto u : relabeled) {
            ...
            while (stack) {
                ...
                if (t <= d_v) {
                    ...
                    d[v] = t
                    pred[v] = u
                    if (!neg_count.contains(v)) {
                        neg_count[v] = neg_count[u] + int(is_neg);
                        stack.append((v, iter(G_succ[v].items())));
                        in_stack.add(v);
                    } else if (in_stack.contains(v) and neg_count[u] + int(is_neg) > neg_count[v]) {
...
auto negative_edge_cycle(G, weight="weight", heuristic=true) -> void {
    ...
    --------
    >>> G = nx.cycle_graph(5, create_using=nx.DiGraph());
    >>> fmt::print(nx.negative_edge_cycle(G));
    false
    >>> G[1][2]["weight"] = -7
    >>> fmt::print(nx.negative_edge_cycle(G));
    true
...
auto find_negative_cycle(G, source, weight="weight") -> void {
    ...
    */
    ...
    if (v is None) {
        throw nx.NetworkXError("No negative cycles detected.");
    ...
    while (stack) {
        ...
        if (preds) {
            ...
            if (neg_cycle) {
                ...
                if (G.contains(v)[v] and weight(G, v, v) < 0) {
                    return [v, v];
                // should not reach here
                throw nx.NetworkXError("Negative cycle is detected but not found");
    // should not get here...
    msg = "negative cycle detected but not identified"
    throw nx.NetworkXUnbounded(msg);
}

auto bidirectional_dijkstra(G, source, target, weight="weight") -> void {
    ...
    */
    ...
    while (fringe[0] and fringe[1]) {
        ...
        for (auto w, d : neighs[dir][v].items()) {
...
auto johnson(G, weight="weight") -> void {
    ...
    */
    if (!nx.is_weighted(G, weight=weight)) {
        throw nx.NetworkXError("Graph is not weighted.");
    ...
    auto new_weight(u, v, d) -> void {
        return weight(u, v, d) + dist_bellman[u] - dist_bellman[v];

    auto dist_path(v) -> void {
        paths = {v: [v]};
