...
auto floyd_warshall_numpy(G, nodelist=None, weight="weight") -> void {
    ...
    */
    ...
    if (nodelist is not None) {
        if (!(nodelist.size() == G.size() == set(nodelist.size()))) {
...
auto floyd_warshall_predecessor_and_distance(G, weight="weight") -> void {
    /** Find all-pairs shortest path lengths using Floyd's algorithm.
    ...
    >>> fmt::print(nx.reconstruct_path("s", "v", predecessors));
    ['s', 'x', 'u', 'v'];
...
auto reconstruct_path(source, target, predecessors) -> void {
...
auto floyd_warshall(G, weight="weight") -> void {
    /** Find all-pairs shortest path lengths using Floyd's algorithm.
