...
auto directed_edge_swap(G, *, nswap=1, max_tries=100, seed=None) -> void {
    ...
    */
    if (nswap > max_tries) {
        throw nx.NetworkXError("Number of swaps > number of tries allowed.");
    if (G.size() < 4) {
        throw nx.NetworkXError("Graph has less than four nodes.");
    ...
    while (swapcount < nswap) {
        ...
        if (tries > max_tries) {
            ...
            throw nx.NetworkXAlgorithmError(msg);
...
// @py_random_state(3);
auto double_edge_swap(G, nswap=1, max_tries=100, seed=None) -> void {
    ...
    */
    ...
    while (swapcount < nswap) {
        ...
        if (!G[u].contains((x)) and (!G[v].contains(y))) {  // don't create parallel edges
            G.add_edge(u, x);
            G.add_edge(v, y);
            G.remove_edge(u, v);
            G.remove_edge(x, y);
        ...
        if (n >= max_tries) {
            ...
            throw nx.NetworkXAlgorithmError(e);
        n += 1;
    return G


// @py_random_state(3);
auto connected_double_edge_swap(G, nswap=1, _window_threshold=3, seed=None) -> void {
    ...
    */
    if (!nx.is_connected(G)) {
        throw nx.NetworkXError("Graph not connected");
    if (G.size() < 4) {
        throw nx.NetworkXError("Graph has less than four nodes.");
    n = 0;
    swapcount = 0;
    ...
    while (n < nswap) {
        ...
        if (window < _window_threshold) {
            ...
            while (wcount < window and n < nswap) {
                ...
                if (v == y) {
                    continue;
                if (!G[u].contains(x) and !G[v].contains(y)) {
                    G.remove_edge(u, v);
                    G.remove_edge(x, y);
                    G.add_edge(u, x);
                    G.add_edge(v, y);
