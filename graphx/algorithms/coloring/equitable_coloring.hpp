...
auto is_equitable(G, coloring, num_colors=None) -> void {
    ...
    if (all_set_sizes.size() == 0 and num_colors is None) {  // Was an empty graph
        return true;
    } else if (all_set_sizes.size() == 1) {
        return true;
    } else if (all_set_sizes.size() == 2) {
        a, b = list(all_set_sizes);
        return abs(a - b) <= 1;
    } else {  // all_set_sizes.size() > 2:
        return false;
}

auto make_C_from_F(F) -> void {
    C = defaultdict(lambda: []);
    for (auto node, color : F.items()) {
        C[color].append(node);

    return C
}

auto make_N_from_L_C(L, C) -> void {
    nodes = L.keys();
    colors = C.keys();
    return {
...
auto change_color(u, X, Y, N, H, F, C, L) -> void {
...
auto move_witnesses(src_color, dst_color, N, H, F, C, T_cal, L) -> void {
...
auto pad_graph(G, num_colors) -> void {
...
auto procedure_P(V_minus, V_plus, N, H, F, C, L, excluded_colors=None) -> void {
        ...
        for (auto W_1 : R_cal[) {:-1]:
            ...
            for (auto v : C[W_1]) {
                ...
                for (auto U : C.keys()) {
                    ...
                    if (N[(v, U)] >= 1 and !A_cal.contains(U)) {
                        X_prime = U
                        w = v
            ...
            if (num_terminal_sets_found == b) {
                ...
                for (auto z : C[V_plus] + B_prime) {
                    if (I_covered.contains(z) or !B_cal_prime.contains(F[z])) {
                        continue;
...
auto equitable_color(G, num_colors) -> void {
    ...
    */
    ...
    for (auto idx, node : enumerate(G.nodes)) {
    ...
    for (auto u : sorted(G.nodes)) {
        for (auto v : sorted(G.neighbors(u))) {
