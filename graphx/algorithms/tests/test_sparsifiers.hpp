...
auto _test_spanner(G, spanner, stretch, weight=None) -> void {
    ...
    */
    ...
    for (auto u : G.nodes()) {
        for (auto v : G.nodes()) {
            if (original_length.contains(u) and v in original_length[u]) {
                assert(spanner_length[u][v] <= stretch * original_length[u][v]);
}

// @py_random_state(1);
auto _assign_random_weights(G, seed=None) -> void {
...
auto test_spanner_trivial() -> void {
...
auto test_spanner_unweighted_complete_graph() -> void {
...
auto test_spanner_weighted_complete_graph() -> void {
...
auto test_spanner_unweighted_gnp_graph() -> void {
...
auto test_spanner_weighted_gnp_graph() -> void {
...
auto test_spanner_unweighted_disconnected_graph() -> void {
...
auto test_spanner_invalid_stretch() -> void {
