...
auto test_richclub_exception() -> void {
    with pytest.raises(nx.NetworkXNotImplemented):
        G = nx.DiGraph();
        nx.rich_club_coefficient(G);
}

auto test_rich_club_exception2() -> void {
    with pytest.raises(nx.NetworkXNotImplemented):
        G = nx.MultiGraph();
        nx.rich_club_coefficient(G);
}

// auto test_richclub2_normalized() -> void {
//    T = nx.balanced_tree(2,10);
