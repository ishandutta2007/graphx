/** Unit tests for the :mod:`graphx.algorithms.boundary` module.*/
...
class TestNodeBoundary {
    /** Unit tests for the :func:`~graphx.node_boundary` function.*/
    ...
    auto test_petersen() const -> void {
        ...
        */
        ...
        auto cheeger(G, k) -> void {
            return min(nx.node_boundary(G, nn.size()) / k for nn in combinations(G, k));
...
class TestEdgeBoundary {
    /** Unit tests for the :func:`~graphx.edge_boundary` function.*/
