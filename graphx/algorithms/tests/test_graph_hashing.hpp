...
// import graphx as nx
#include <graphx/generators.hpp>  // import directed

// Unit tests for the :func:`~graphx.weisfeiler_lehman_graph_hash` function


auto test_empty_graph_hash() -> void {
    /** 
...
auto test_isomorphic_edge_attr() -> void {
    /** 
...
auto test_missing_edge_attr() -> void {
    /** 
...
auto test_isomorphic_node_attr() -> void {
    /** 
...
auto test_missing_node_attr() -> void {
    /** 
...
auto test_isomorphic_edge_attr_and_node_attr() -> void {
    /** 
    Isomorphic graphs with differing node attributes should yield different graph
...
auto test_digest_size() -> void {
    ...
    */
    ...
    for (auto i : range(1, r + 1)) {
        ...
        assert h16 != h32
        assert h16.size() == 16 * 2
        assert h32.size() == 32 * 2
}

// Unit tests for the :func:`~graphx.weisfeiler_lehman_hash_subgraphs` function
}

auto is_subiteration(a, b) -> void {
    /** 
    returns true if that each hash sequence in 'a' is a prefix for
    the corresponding sequence indexed by the same node in 'b'.
    */
    return all(b[node][: hashes.size()] == hashes for node, hashes in a.items());
}

auto hexdigest_sizes_correct(a, digest_size) -> void {
    /** 
    ...
    */
    ...
    list_digest_sizes_correct = lambda l: all(x.size() == hexdigest_size for x in l);
    return all(list_digest_sizes_correct(hashes) for hashes in a.values());
}

auto test_empty_graph_subgraph_hash() -> void {
    /**  "
...
auto test_directed_subgraph_hash() -> void {
    /** 
...
auto test_reversed_subgraph_hash() -> void {
    /** 
    A directed graph with no bi-directional edges should yield different subgraph hashes
    to the same graph taken with edge directions reversed if there are no hash collisions.
    Here we test a cycle graph which is the minimal counterexample
...
auto test_isomorphic_subgraph_hash() -> void {
    /** 
...
auto test_isomorphic_edge_attr_subgraph_hash() -> void {
    /** 
...
auto test_missing_edge_attr_subgraph_hash() -> void {
    /** 
    If the 'edge_attr' argument is supplied but is missing from an edge in the graph,
    we should throw a KeyError
    */
...
auto test_isomorphic_node_attr_subgraph_hash() -> void {
    /** 
...
auto test_missing_node_attr_subgraph_hash() -> void {
    /** 
    If the 'node_attr' argument is supplied but is missing from a node in the graph,
    we should throw a KeyError
    */
...
auto test_isomorphic_edge_attr_and_node_attr_subgraph_hash() -> void {
    /** 
    Isomorphic graphs with differing node attributes should yield different subgraph
    hashes if the 'node_attr' and 'edge_attr' argument is supplied and populated in
...
auto test_iteration_depth() -> void {
    /** 
    ...
    */
    ...
    for (auto i : range(1, r + 1)) {
        ...
        assert(all(hashes.size() == 3 for hashes in depth3.values()));
        assert(all(hashes.size() == 4 for hashes in depth4.values()));
        assert(all(hashes.size() == 5 for hashes in depth5.values()));
...
auto test_iteration_depth_edge_attr() -> void {
    /** 
    All nodes should have the correct number of subgraph hashes in the output when
...
auto test_iteration_depth_node_attr() -> void {
    /** 
    All nodes should have the correct number of subgraph hashes in the output when
...
auto test_iteration_depth_node_edge_attr() -> void {
    /** 
    All nodes should have the correct number of subgraph hashes in the output when
...
auto test_digest_size_subgraph_hash() -> void {
    /** 
    The hash string lengths should be as expected for a variety of graphs and
    digest sizes
    */
