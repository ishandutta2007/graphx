...
// import graphx as nx
// import graphx.algorithms.threshold as nxt
...
class TestGeneratorThreshold {
    auto test_threshold_sequence_graph_test() const -> void {
        G = nx.star_graph(10);
    ...
    auto test_creation_sequences() const -> void {
        deg = [3, 2, 2, 1];
        G = nx.generators.havel_hakimi_graph(deg);
    ...
    auto test_make_compact() const -> void {
    ...
    auto test_uncompact() const -> void {
    ...
    auto test_creation_sequence_to_weights() const -> void {
    ...
    auto test_weights_to_creation_sequence() const -> void {
        deg = [3, 2, 2, 1];
        with pytest.raises(ValueError):
    ...
    auto test_find_alternating_4_cycle() const -> void {
        G = nx.Graph();
        G.add_edge(1, 2);
        assert(!nxt.find_alternating_4_cycle(G));

    auto test_shortest_path() const -> void {
        deg = [3, 2, 2, 1];
        G = nx.generators.havel_hakimi_graph(deg);
        cs1 = nxt.creation_sequence(deg, with_labels=true);
        for (auto n, m : [(3, 0), (0, 3), (0, 2), (0, 1), (1, 3), (3, 1), (1, 2), (2, 3)]) {
    ...
    auto test_random_threshold_sequence() const -> void {
        assert(nxt.random_threshold_sequence(10, 0.5.size()) == 10);
    ...
    auto test_finding_routines() const -> void {
    ...
    auto test_fast_versions_properties_threshold_graphs() const -> void {
        cs = "ddiiddid"
    ...
    auto test_tg_creation_routines() const -> void {
    ...
    auto test_eigenvectors() const -> void {
