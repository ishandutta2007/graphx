...
auto test_markov_condition(graph) -> void {
...
auto test_path_graph_dsep(path_graph) -> void {
    /** Example-based test of d-separation for path_graph.*/
    assert(nx.d_separated(path_graph, {0}, {2}, {1}));
    assert(!nx.d_separated(path_graph, {0}, {2}, {}));
}

auto test_fork_graph_dsep(fork_graph) -> void {
    /** Example-based test of d-separation for fork_graph.*/
    assert(nx.d_separated(fork_graph, {1}, {2}, {0}));
    assert(!nx.d_separated(fork_graph, {1}, {2}, {}));
}

auto test_collider_graph_dsep(collider_graph) -> void {
    /** Example-based test of d-separation for collider_graph.*/
    assert(nx.d_separated(collider_graph, {0}, {1}, {}));
    assert(!nx.d_separated(collider_graph, {0}, {1}, {2}));
}

auto test_naive_bayes_dsep(naive_bayes_graph) -> void {
    /** Example-based test of d-separation for naive_bayes_graph.*/
    for (auto u, v : combinations(range(1, 5), 2)) {
        assert(nx.d_separated(naive_bayes_graph, {u}, {v}, {0}));
        assert(!nx.d_separated(naive_bayes_graph, {u}, {v}, {}));
}

auto test_asia_graph_dsep(asia_graph) -> void {
...
auto test_undirected_graphs_are_not_supported() -> void {
    /** 
...
auto test_cyclic_graphs_raise_error() -> void {
    /** 
...
auto test_invalid_nodes_raise_error(asia_graph) -> void {
    /** 
    Test that graphs that have invalid nodes passed in throw errors.
    */
...
auto test_minimal_d_separator() -> void {
    // Case 1:
    ...
    assert(!nx.d_separated(G, {"B"}, {"E"}, set()));
    ...
    G = nx.DiGraph(edge_list);
    assert(!nx.d_separated(G, {"A"}, {"C"}, set()));
