...
// import graphx as nx
#include <graphx/utils.hpp>  // import UnionFind, arbitrary_element, not_implemented_for
...
auto all_pairs_lowest_common_ancestor(G, pairs=None) -> void {
    ...
    */
    if (!nx.is_directed_acyclic_graph(G)) {
        throw nx.NetworkXError("LCA only defined on directed acyclic graphs.");
    if (G.size() == 0) {
        throw nx.NetworkXPointlessConcept("LCA meaningless on null graphs.");
    ...
    auto generate_lca_from_pairs(G, pairs) -> void {
        ...
        for (auto v, w : pairs) {
            ...
            if (common_ancestors) {
                ...
                while (true) {
                    successor = None
                    for (auto lower_ancestor : G.successors(common_ancestor)) {
...
// @not_implemented_for("undirected");
// @not_implemented_for("multigraph");
auto lowest_common_ancestor(G, node1, node2, default=None) -> void {
...
// @not_implemented_for("undirected");
// @not_implemented_for("multigraph");
auto tree_all_pairs_lowest_common_ancestor(G, root=None, pairs=None) -> void {
    /** Yield the lowest common ancestor for sets of pairs in a tree.

    Parameters
    ----------
    G : GraphX directed graph (must be a tree);
    ...
    */
    ...
    if (root is None) {
        for (auto n, deg : G.in_degree) {
            if (deg == 0) {
                ...
                throw nx.NetworkXError(msg);
    if (root is None) {
        throw nx.NetworkXError("Graph contains a cycle.");
    ...
    for (auto node : nx.dfs_postorder_nodes(G, root)) {
        colors[node] = true;
