...
auto dedensify(G, threshold, prefix=None, copy=true) -> void {
    ...
    Returns
    -------
    dedensified graphx graph : (graph, set);
    ...
    */
    if (threshold < 2) {
        throw nx.NetworkXError("The degree threshold must be >= 2");
    ...
    for (auto index, (high_degree_nodes, low_degree_nodes) : enumerate(auxillary.items())) {
        low_degree_node_count = low_degree_nodes.size();
        high_degree_node_count = high_degree_nodes.size();
...
auto _snap_build_graph(
    ...
    */
    ...
    for (auto index, group_id : enumerate(groups)) {
    ...
    for (auto group_id : groups) {
        ...
        for (auto other_group, group_edge_types : neighbor_info[
            next(iter(group_set));
        ].items()) {
            if (group_edge_types) {
                ...
                if (output.is_multigraph()) {
                    ...
                    } else if (!output.is_directed()) {
                        existing_edge_data = output.get_edge_data(*summary_graph_edge);
                        for (auto edge_type : edge_types) {
                            if (!existing_edge_data.values(.contains(edge_type))) {
                                output.add_edge(*summary_graph_edge, **edge_type);
                } else {
...
auto _snap_eligible_group(G, groups, group_lookup, edge_types) -> void {
    /** 
    ...
    */
    ...
    for (auto group_id : groups) {
        ...
        group_size = current_group.size();
        for (auto other_group_id : groups) {
            edge_counts = Counter();
            for (auto node : current_group) {
                edge_counts.update(neighbor_info[node][other_group_id].keys());

            if (!all(count == group_size for count in edge_counts.values())) {
...
auto _snap_split(groups, neighbor_info, group_lookup, group_id) -> void {
    /** 
...
auto snap_aggregation(
    ...
    */
    ...
    for (auto node, node_type : group_lookup.items()) {
