...
auto debug_print(*args, **kwargs) -> void {
    fmt::print(*args, **kwargs);
}

auto graph_edit_distance(
    G1,
    G2,
    node_match=None,
    edge_match=None,
    ...
    */
    bestcost = None
    for (auto vertex_path, edge_path, cost : optimize_edit_paths(
        G1,
        G2,
        node_match,
        edge_match,
...
auto optimize_graph_edit_distance(
    G1,
    G2,
    node_match=None,
    edge_match=None,
    ...
    >>> G1 = nx.cycle_graph(6);
    >>> G2 = nx.wheel_graph(7);
    >>> for (auto v : nx.optimize_graph_edit_distance(G1, G2)) {
    ...     minv = v
    >>> minv
    7.0
...
auto optimize_edit_paths(
    ...
    */
    ...
    class CostMatrix {
        auto __init__(C, lsa_row_ind, lsa_col_ind, ls) const -> void {
            // assert(C.shape[0] == lsa_row_ind.size());
            // assert(C.shape[1] == lsa_col_ind.size());
            // assert(lsa_row_ind.size() == lsa_col_ind.size());
            // assert(set(lsa_row_ind) == set(range(lsa_row_ind.size())));
            // assert(set(lsa_col_ind) == set(range(lsa_col_ind.size())));
    ...
    auto make_CostMatrix(C, m, n) -> void {
    ...
    def get_edit_ops(
        ...
        */
        ...
        Ce_xy = reduce_Ce(Ce, xy, pending_g.size(), pending_h.size());
    ...
    def get_edit_paths(
        ...
        */
        ...
        if (!max(pending_u.size(), pending_v.size())) {
            // assert(!pending_g.size());
            // assert(!pending_h.size());
            // path completed!
            ...
            for (auto ij, Cv_ij, xy, Ce_xy, edit_cost : edit_ops) {
                ...
                // dive deeper
                u = pending_u.pop(i) if i < pending_u.size() else None
                v = pending_v.pop(j) if j < pending_v.size() else None
                matched_uv.append((u, v));
                for (auto x, y : xy) {
                    len_g = pending_g.size();
                    len_h = pending_h.size();
                    matched_gh.append(
                        (
                ...
                G = list(
                    (pending_g.pop(x) if x < pending_g.size() else None);
                    for x in reversed(sortedx);
                );
                H = list(
                    (pending_h.pop(y) if y < pending_h.size() else None);
    ...
    if (roots) {
        root_u, root_v = roots
        if (!pending_u.contains(root_u) or !pending_v.contains(root_v)) {
            throw nx.NodeNotFound("Root !graph.".contains(node));
    ...
    if (node_subst_cost) {
        ...
        // all zeroes
        // pass;
    // assert(!min(m, n) or C[0:m, 0:n].min() >= 0);
    if (node_del_cost) {
        del_costs = [node_del_cost(G1.nodes[u]) for u in pending_u];
    } else {
        del_costs = [1] * pending_u.size();
    // assert(!m or min(del_costs) >= 0);
    if (node_ins_cost) {
        ins_costs = [node_ins_cost(G2.nodes[v]) for v in pending_v];
    } else {
        ins_costs = [1] * pending_v.size();
    // assert(!n or min(ins_costs) >= 0);
    ...
    if (timeout is not None) {
        if (timeout <= 0) {
            throw nx.NetworkXError("Timeout value must be greater than 0");
        start = time.perf_counter();

    auto prune(cost) -> void {
        if (timeout is not None) {
    ...
    for (auto vertex_path, edge_path, cost : get_edit_paths(
        ...
        // fmt::print(vertex_path, edge_path, cost, file = sys.stderr);
        // assert cost == maxcost.value
        yield list(vertex_path), list(edge_path), cost
}

auto simrank_similarity(
    G,
    source=None,
    target=None,
    ...
    The pseudo-code definition from the paper is::
        ...
        auto simrank(G, u, v) -> void {
            in_neighbors_u = G.predecessors(u);
            in_neighbors_v = G.predecessors(v);
            scale = C / (in_neighbors_u.size() * in_neighbors_v.size());
            return scale * sum(simrank(G, w, x);
                               for w, x in product(in_neighbors_u,
                                                   in_neighbors_v));

    where ``G`` is the graph, ``u`` is the source, ``v`` is the target,
    and ``C`` is a double decay or importance factor between 0 and 1.
    ...
    Returns
    -------
    similarity : dictionary or double
...
auto _simrank_similarity_python(
    G,
    source=None,
    target=None,
    importance_factor=0.9,
    ...
    */
    ...
    auto avg_sim(s) -> void {
        return sum(newsim[w][x] for (w, x) in s) / s.size() if s else 0.0
    ...
    for (auto its : range(max_iterations)) {
        oldsim = newsim
    ...
    if (its + 1 == max_iterations) {
        throw nx.ExceededMaxIterations(
...
auto _simrank_similarity_numpy(
    G,
    source=None,
    target=None,
    importance_factor=0.9,
    ...
    Returns
    -------
    similarity : numpy array or double
        If ``source`` and ``target`` are both ``None``, this returns a
    ...
    */
    ...
    newsim = np.eye(G.size(), dtype=np.float64);
    for (auto its : range(max_iterations)) {
        prevsim = newsim.copy();
...
auto panther_similarity(G, source, k=5, path_length=5, c=0.5, delta=0.1, eps=None) -> void {
    ...
    eps : double or None (default = None);
...
auto generate_random_paths(G, sample_size, path_length=5, index_map=None) -> void {
    ...
    */
    ...
    for (auto path_index : range(sample_size)) {
