...
auto kl_connected_subgraph(G, k, l, low_memory=false, same_as_graph=false) -> void {
    ...
    */
    ...
    while (deleted_some) {
        deleted_some = false;
        // We use `for (auto edge : list(H.edges())) {` instead of
        // `for (auto edge : H.edges()) {` because we edit the graph `H` in
...
auto is_kl_connected(G, k, l, low_memory=false) -> void {
