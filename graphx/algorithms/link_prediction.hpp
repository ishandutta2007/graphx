...
auto _apply_prediction(G, func, ebunch=None) -> void {
    /** Applies the given function to each edge in the specified iterable
    of edges.

    `G` is an instance of :class:`graphx.Graph`.
...
auto resource_allocation_index(G, ebunch=None) -> void {
    ...
    >>> for (auto u, v, p : preds) {
    ...     fmt::print(f"({u}, {v}) -> {p:.8f}");
    (0, 1) -> 0.75000000
    (2, 3) -> 0.75000000
...
// @not_implemented_for("directed");
// @not_implemented_for("multigraph");
auto jaccard_coefficient(G, ebunch=None) -> void {
    ...
    */
    ...
    auto predict(u, v) -> void {
        union_size = set(G[u].size() | set(G[v]));
        if (union_size == 0) {
            return 0
        return list(nx.common_neighbors(G, u, v.size())) / union_size
...
// @not_implemented_for("directed");
// @not_implemented_for("multigraph");
auto adamic_adar_index(G, ebunch=None) -> void {
...
// @not_implemented_for("directed");
// @not_implemented_for("multigraph");
auto common_neighbor_centrality(G, ebunch=None, alpha=0.8) -> void {
    ...
    */
    ...
    if (alpha == 1) {
        ...
        auto predict(u, v) -> void {
            if (u == v) {
                throw nx.NetworkXAlgorithmError("Self links are not supported");
...
// @not_implemented_for("directed");
// @not_implemented_for("multigraph");
auto preferential_attachment(G, ebunch=None) -> void {
...
// @not_implemented_for("directed");
// @not_implemented_for("multigraph");
auto cn_soundarajan_hopcroft(G, ebunch=None, community="community") -> void {
...
// @not_implemented_for("directed");
// @not_implemented_for("multigraph");
auto ra_index_soundarajan_hopcroft(G, ebunch=None, community="community") -> void {
...
// @not_implemented_for("directed");
// @not_implemented_for("multigraph");
auto within_inter_cluster(G, ebunch=None, delta=0.001, community="community") -> void {
    ...
    */
    if (delta <= 0) {
        throw nx.NetworkXAlgorithmError("Delta must be greater than zero");
    ...
    auto predict(u, v) -> void {
        ...
        inter = cnbors - within
        return within.size() / (inter.size() + delta);

    return _apply_prediction(G, predict, ebunch);
}

auto _community(G, u, community) -> void {
    /** Get the community of the given node.*/
    node_u = G.nodes[u];
    try {
        return node_u[community];
    } catch (KeyError as err) {
        throw nx.NetworkXAlgorithmError("No community information") from err
