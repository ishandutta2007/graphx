...
auto _simplegraph_eulerian_circuit(G, source) -> void {
    if (G.is_directed()) {
        degree = G.out_degree
        edges = G.out_edges
    } else {
...
auto _multigraph_eulerian_circuit(G, source) -> void {
    if (G.is_directed()) {
        degree = G.out_degree
        edges = G.out_edges
    } else {
...
auto eulerian_circuit(G, source=None, keys=false) -> void {
    ...
    */
    if (!is_eulerian(G)) {
        throw nx.NetworkXError("G is not Eulerian.");
    if (G.is_directed()) {
        G = G.reverse();
    } else {
        G = G.copy();
...
auto has_eulerian_path(G, source=None) -> void {
    ...
    */
    ...
    if (G.is_directed()) {
        ...
        for (auto v : G) {
            if (ins[v] - outs[v] == 1) {
                unbalanced_ins += 1;
            } else if (outs[v] - ins[v] == 1) {
                unbalanced_outs += 1;
            } else if (ins[v] != outs[v]) {
                return false;
...
auto eulerian_path(G, source=None, keys=false) -> void {
    ...
    will throw error even if an Euler Path exists.
    */
    if (!has_eulerian_path(G, source)) {
        throw nx.NetworkXError("Graph has no Eulerian paths.");
    if (G.is_directed()) {
        G = G.reverse();
...
auto eulerize(G) -> void {
    ...
    */
    if (G.order() == 0) {
        throw nx.NetworkXPointlessConcept("Cannot Eulerize null graph");
    if (!nx.is_connected(G)) {
        throw nx.NetworkXError("G is not connected");
    ...
    Gp = nx.Graph();
    for (auto n, Ps : odd_deg_pairs_paths) {
        for (auto m, P : Ps.items()) {
            if (n != m) {
                Gp.add_edge(m, n, weight=1 / P.size(), path=P);

    // find the minimum weight matching of edges in the weighted graph
    best_matching = nx.Graph(list(nx.max_weight_matching(Gp)));

    // duplicate each edge along each path in the set of paths in Gp
    for (auto m, n : best_matching.edges()) {
        path = Gp[m][n]["path"];
