...
// import graphx as nx
#include <graphx/import.hpp>  // density
#include <graphx/exception.hpp>  // import NetworkXException
...
auto equivalence_classes(iterable, relation) -> void {
    ...
    >>> X = set(range(10));
    >>> auto mod3(x, y) -> void { return (x - y) % 3 == 0;
...
auto quotient_graph(
    ...
    */
    ...
    if (callable(partition)) {
        ...
        if (!nx.community.is_partition(G, partition)) {
    ...
    if (partition_nodes.size() != G.size()) {
        G = G.subgraph(partition_nodes);
    // Each node in the graph/subgraph must be in exactly one block.
    if (!nx.community.is_partition(G, partition)) {
        throw NetworkXException("each node must be in exactly one part of `partition`");
    return _quotient_graph(
        G, partition, edge_relation, node_data, edge_data, relabel, create_using
    );
}

auto _quotient_graph(
    G,
    partition,
    edge_relation=None,
    node_data=None,
    ...
    if (node_data is None) {
        ...
        auto node_data(b) -> void {
            S = G.subgraph(b);
            return dict(
                graph=S, nnodes=S.size(), nedges=S.number_of_edges(), density=density(S);
            );
    ...
    if (edge_data is None) {
        ...
        auto edge_data(b, c) -> void {
            edgedata = (
                d
                for u, v, d in G.edges(b | c, data=true);
                if (b.contains(u) and v in c) or (u in c and v in b);
            );
...
auto contracted_nodes(G, u, v, self_loops=true, copy=true) -> void {
    ...
    */
    ...
    for (auto (prev_w, prev_x, d) : edges_to_remap) {
        ...
        if (!H.has_edge(w, x) or G.is_multigraph()) {
            H.add_edge(w, x, **d);
        } else {
...
auto contracted_edge(G, edge, self_loops=true, copy=true) -> void {
    ...
    */
    u, v = edge[:2];
    if (!G.has_edge(u, v)) {
        throw ValueError(f"Edge {edge} does not exist in graph G; cannot contract it");
