...
:mod:`graphx.algorithms.tournament` module::
...
// import graphx as nx
#include <graphx/algorithms.simple_paths.hpp>  // import is_simple_path as is_path
#include <graphx/utils.hpp>  // import arbitrary_element, not_implemented_for, py_random_state
...
auto index_satisfying(iterable, condition) -> void {
    ...
    */
    ...
    try {
        ...
        throw ValueError("iterable must be non-empty") from err
}

// @not_implemented_for("undirected");
// @not_implemented_for("multigraph");
auto is_tournament(G) -> void {
...
auto hamiltonian_path(G) -> void {
    ...
    */
    ...
    index = index_satisfying(hampath, lambda u: !G[u].contains(v));
    hampath.insert(index, v);
    return hampath
}

// @py_random_state(1);
auto random_tournament(n, seed=None) -> void {
...
auto is_reachable(G, s, t) -> void {
    ...
    :mod:`graphx.algorithms.shortest_paths`.
    ...
    */
    ...
    auto two_neighborhood(G, v) -> void {
    ...
    return all(!(is_closed(G, S) and s in S and !S.contains(t)) for S in neighborhoods);
}
...
auto is_strongly_connected(G) -> void {
    ...
    :func:`~graphx.algorithms.components.is_strongly_connected`
    function.
