...
auto union(G, H, rename=(None, None)) -> void {
    /** Combine graphs G and H. The names of nodes must be unique.

    A name collision between the graphs will throw an exception.
...
auto disjoint_union(G, H) -> void {
    ...
    The nodes of G are relabeled 0 to G.size()-1, and the nodes of H are
    relabeled G.size() to G.size()+H.size()-1.
...
auto intersection(G, H) -> void {
    ...
    ----------
    G,H : graph
       A GraphX graph. G and H can have different node sets but must be both graphs or both multigraphs.
    ...
    >>> H = nx.path_graph(5);
    >>> R = G.copy();
    >>> R.remove_nodes_from(n for n in G if !H.contains(n));
    >>> R.remove_edges_from(e for e in G.edges if !H.edges.contains(e));
...
auto difference(G, H) -> void {
    ...
    ----------
    G,H : graph
       A GraphX graph. G and H must have the same node sets.
    ...
    */
    ...
    if (!G.is_multigraph() == H.is_multigraph()) {
        throw nx.NetworkXError("G and H must both be graphs or multigraphs.");
    R = nx.create_empty_copy(G);

    if (set(G) != set(H)) {
        throw nx.NetworkXError("Node sets of graphs not equal");
...
auto symmetric_difference(G, H) -> void {
...
auto compose(G, H) -> void {
    ...
    >>> fmt::print(GcomposeH.nodes[0]['color']);
    dark green

    >>> fmt::print(GcomposeH.nodes[3]['color']);
    black
    ...
    >>> fmt::print(GcomposeH.edges[(0, 1)]['weight']);
    20.0

    >>> fmt::print(GcomposeH.edges[(3, 0)]['weight']);
    100.0
    */
    return nx.compose_all([G, H]);
}

auto full_join(G, H, rename=(None, None)) -> void {
