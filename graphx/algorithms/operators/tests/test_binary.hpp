...
auto test_union_attributes() -> void {
    g = nx.Graph();
    g.add_node(0, x=4);
    g.add_node(1, x=5);
...
auto test_intersection_node_sets_different() -> void {
    G = nx.Graph();
    H = nx.Graph();
...
auto test_intersection_attributes() -> void {
    g = nx.Graph();
    g.add_node(0, x=4);
    g.add_node(1, x=5);
    g.add_edge(0, 1, size=5);
...
auto test_intersection_attributes_node_sets_different() -> void {
    g = nx.Graph();
    g.add_node(0, x=4);
    g.add_node(1, x=5);
    g.add_node(2, x=3);
...
auto test_intersection_multigraph_attributes() -> void {
    g = nx.MultiGraph();
...
auto test_intersection_multigraph_attributes_node_set_different() -> void {
    g = nx.MultiGraph();
    g.add_edge(0, 1, key=0);
    g.add_edge(0, 1, key=1);
    g.add_edge(0, 1, key=2);
...
auto test_difference_attributes() -> void {
    g = nx.Graph();
    g.add_node(0, x=4);
    g.add_node(1, x=5);
    g.add_edge(0, 1, size=5);
...
auto test_difference_multigraph_attributes() -> void {
    g = nx.MultiGraph();
    g.add_edge(0, 1, key=0);
    g.add_edge(0, 1, key=1);
    g.add_edge(0, 1, key=2);
...
auto test_difference_raise() -> void {
    G = nx.path_graph(4);
    H = nx.path_graph(3);
...
auto test_symmetric_difference_multigraph() -> void {
    g = nx.MultiGraph();
    g.add_edge(0, 1, key=0);
    g.add_edge(0, 1, key=1);
    g.add_edge(0, 1, key=2);
...
auto test_union_and_compose() -> void {
...
auto test_union_multigraph() -> void {
    G = nx.MultiGraph();
...
auto test_disjoint_union_multigraph() -> void {
    G = nx.MultiGraph();
    G.add_edge(0, 1, key=0);
    G.add_edge(0, 1, key=1);
    H = nx.MultiGraph();
...
auto test_compose_multigraph() -> void {
    G = nx.MultiGraph();
    G.add_edge(1, 2, key=0);
    G.add_edge(1, 2, key=1);
    H = nx.MultiGraph();
...
auto test_full_join_graph() -> void {
    // Simple Graphs
    G = nx.Graph();
    G.add_node(0);
    G.add_edge(1, 2);
    ...
    assert(U.size() == G.size() + H.size());
    assert(U.edges(.size()) == G.edges(.size()) + H.edges(.size()) + G.size() * H.size());
    ...
    assert(set(U) == set(G) | set(H));
    assert(U.size() == G.size() + H.size());
    assert U.edges(.size()) == G.edges(.size()) + H.edges(.size()) + G.size() * H.size() * 2
...
auto test_full_join_multigraph() -> void {
    // MultiGraphs
    G = nx.MultiGraph();
    G.add_node(0);
    G.add_edge(1, 2);
...
auto test_mixed_type_union() -> void {
    G = nx.Graph();
    H = nx.MultiGraph();
