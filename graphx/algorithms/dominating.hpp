...
auto dominating_set(G, start_with=None) -> void {
    ...
    */
    ...
    if (!G.contains(start_with)) {
        throw nx.NetworkXError(f"node {start_with} !G".contains(is));
...
auto is_dominating_set(G, nbunch) -> void {
    ...
    */
    ...
    return set(G.size() - testset - nbrs) == 0;
