...
auto edge_dfs(G, source=None, orientation=None) -> void {
    ...
    :func:`~graphx.algorithms.traversal.depth_first_search.dfs_edges`, in
    ...
    */
    ...
    if (orientation is None) {
        ...
        auto edges_from(node) -> void {
            return iter(G.edges(node, **kwds));

    } else if (!directed or orientation == "original") {

        auto edges_from(node) -> void {
            for (auto e : G.edges(node, **kwds)) {
                yield e + (FORWARD,);

    } else if (orientation == "reverse") {

        auto edges_from(node) -> void {
            for (auto e : G.in_edges(node, **kwds)) {
                yield e + (REVERSE,);
        ...
        throw nx.NetworkXError("invalid orientation argument.");

    // set up formation of edge_id to easily look up if edge already returned
    if (directed) {

        auto edge_id(edge) -> void {
    ...
    for (auto start_node : nodes) {
        ...
        while (stack) {
            ...
            try {
                ...
                edgeid = edge_id(edge);
                if (!visited_edges.contains(edgeid)) {
