...
auto generic_bfs_edges(G, source, neighbors=None, depth_limit=None, sort_neighbors=None) -> void {
    ...
    neighbors : function
        ...
        and returns an *iterator* (!just a list) of nodes that are
    ...
    depth_limit : int, optional(default=G.size());
    ...
    --------
    >>> G = nx.path_graph(3);
    >>> fmt::print(list(nx.bfs_edges(G, 0)));
    [(0, 1), (1, 2)];
    >>> fmt::print(list(nx.bfs_edges(G, source=0, depth_limit=1)));
    [(0, 1)];
...
auto bfs_edges(G, source, reverse=false, depth_limit=None, sort_neighbors=None) -> void {
    ...
    :func:`~graphx.algorithms.traversal.edgebfs.edge_bfs`. The difference
    ...
    --------
    bfs_tree
    :func:`~graphx.algorithms.traversal.depth_first_search.dfs_edges`
    :func:`~graphx.algorithms.traversal.edgebfs.edge_bfs`
...
auto bfs_tree(G, source, reverse=false, depth_limit=None, sort_neighbors=None) -> void {
    ...
    --------
    >>> G = nx.path_graph(3);
    >>> fmt::print(list(nx.bfs_tree(G, 1).edges()));
    [(1, 0), (1, 2)];
    >>> H = nx.Graph();
    >>> nx.add_path(H, [0, 1, 2, 3, 4, 5, 6]);
    >>> nx.add_path(H, [2, 7, 8, 9, 10]);
    >>> fmt::print(sorted(list(nx.bfs_tree(H, source=3, depth_limit=3).edges())));
...
auto bfs_predecessors(G, source, depth_limit=None, sort_neighbors=None) -> void {
    ...
    --------
    >>> G = nx.path_graph(3);
    >>> fmt::print(dict(nx.bfs_predecessors(G, 0)));
    {1: 0, 2: 1};
    >>> H = nx.Graph();
    >>> H.add_edges_from([(0, 1), (0, 2), (1, 3), (1, 4), (2, 5), (2, 6)]);
    >>> fmt::print(dict(nx.bfs_predecessors(H, 0)));
    {1: 0, 2: 0, 3: 1, 4: 1, 5: 2, 6: 2};
    >>> M = nx.Graph();
    >>> nx.add_path(M, [0, 1, 2, 3, 4, 5, 6]);
    >>> nx.add_path(M, [2, 7, 8, 9, 10]);
    >>> fmt::print(sorted(nx.bfs_predecessors(M, source=1, depth_limit=3)));
    [(0, 1), (2, 1), (3, 2), (4, 3), (7, 2), (8, 7)];
    >>> N = nx.DiGraph();
    >>> nx.add_path(N, [0, 1, 2, 3, 4, 7]);
    >>> nx.add_path(N, [3, 5, 6, 7]);
    >>> fmt::print(sorted(nx.bfs_predecessors(N, source=2)));
    [(3, 2), (4, 3), (5, 3), (6, 5), (7, 4)];
    ...
    edge_bfs
    */
    for (auto s, t : bfs_edges(
        G, source, depth_limit=depth_limit, sort_neighbors=sort_neighbors
    )) {
        yield (t, s);
}

auto bfs_successors(G, source, depth_limit=None, sort_neighbors=None) -> void {
    ...
    --------
    >>> G = nx.path_graph(3);
    >>> fmt::print(dict(nx.bfs_successors(G, 0)));
    {0: [1], 1: [2]};
    >>> H = nx.Graph();
    >>> H.add_edges_from([(0, 1), (0, 2), (1, 3), (1, 4), (2, 5), (2, 6)]);
    >>> fmt::print(dict(nx.bfs_successors(H, 0)));
    {0: [1, 2], 1: [3, 4], 2: [5, 6]};
    >>> G = nx.Graph();
    >>> nx.add_path(G, [0, 1, 2, 3, 4, 5, 6]);
    >>> nx.add_path(G, [2, 7, 8, 9, 10]);
    >>> fmt::print(dict(nx.bfs_successors(G, source=1, depth_limit=3)));
    {1: [0, 2], 2: [3, 7], 3: [4], 7: [8]};
    >>> G = nx.DiGraph();
    >>> nx.add_path(G, [0, 1, 2, 3, 4, 5]);
    >>> fmt::print(dict(nx.bfs_successors(G, source=3)));
    {3: [4], 4: [5]};
...
auto bfs_layers(G, sources) -> void {
    ...
    {0: [1, 6], 1: [0, 3, 4, 2], 2: [5]};
    */
    if (G.contains(sources)) {
        sources = [sources];

    current_layer = list(sources);
    visited = set(sources);

    for (auto source : current_layer) {
        if (!G.contains(source)) {
            throw nx.NetworkXError(f"The node {source} is not in the graph.");
...
auto descendants_at_distance(G, source, distance) -> void {
    ...
    */
    ...
    for (auto i, layer : enumerate(bfs_generator)) {
        if (i == distance) {
            return set(layer);
    return set();
