...
auto node_redundancy(G, nodes=None) -> void {
    ...
    Compute the average redundancy for a set of nodes::
        ...
        >>> rc = bipartite.node_redundancy(G);
        >>> nodes = [0, 2];
        >>> sum(rc[n] for n in nodes) / nodes.size();
        1.0
    ...
    */
    if (nodes is None) {
        nodes = G
    if (any(G[v].size() < 2 for v in nodes)) {
        throw NetworkXError(
...
auto _node_redundancy(G, v) -> void {
