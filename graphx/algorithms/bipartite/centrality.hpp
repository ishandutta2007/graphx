...
auto degree_centrality(G, nodes) -> void {
    ...
    :func:`~graphx.algorithms.bipartite.basic.sets`
    :func:`~graphx.algorithms.bipartite.basic.is_bipartite`

    Notes
    -----
    The nodes input parameter must contain all nodes in one bipartite node set,
    but the dictionary returned contains all nodes from both bipartite node
    sets. See :mod:`bipartite documentation <graphx.algorithms.bipartite>`
...
auto betweenness_centrality(G, nodes) -> void {
...
auto closeness_centrality(G, nodes, normalized=true) -> void {
    ...
    */
    ...
    for (auto node : top) {
        ...
        totsp = sum(sp.values());
        if (totsp > 0.0 and G.size() > 1) {
            closeness[node] = (m + 2 * (n - 1)) / totsp
            if (normalized) {
                s = (sp.size() - 1) / (G.size() - 1);
                closeness[node] *= s
        } else {
            closeness[node] = 0.0
    for (auto node : bottom) {
