...
// import graphx as nx
#include <graphx/algorithms.bipartite.hpp>  // import sets as bipartite_sets
#include <graphx/algorithms.bipartite.matrix.hpp>  // import biadjacency_matrix
...
auto hopcroft_karp_matching(G, top_nodes=None) -> void {
    /** Returns the maximum cardinality matching of the bipartite graph `G`.

    A matching is a set of edges that do not share any nodes. A maximum
    cardinality matching is a matching with the most edges possible. It
    is not always unique. Finding a matching in a bipartite graph can be
    treated as a graphx flow problem.
    ...
    */
    ...
    auto depth_first_search(v) -> void {
        if (v is not None) {
            for (auto u : G[v]) {
...
auto eppstein_matching(G, top_nodes=None) -> void {
    /** Returns the maximum cardinality matching of the bipartite graph `G`.
...
auto _is_connected_by_alternating_path(G, v, matched_edges, unmatched_edges, targets) -> void {
    ...
    `G` is an undirected bipartite GraphX graph.
    ...
    */
    ...
    auto _alternating_dfs(u, along_matched=true) -> void {
...
auto _connected_by_alternating_paths(G, matching, targets) -> void {
...
auto to_vertex_cover(G, matching, top_nodes=None) -> void {
    ...
    >>> fmt::print(list(independent_set));
    [2, 3, 4];
...
auto minimum_weight_full_matching(G, top_nodes=None, weight="weight") -> void {
