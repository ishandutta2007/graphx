...
// import graphx as nx
#include <graphx/utils.hpp>  // import nodes_or_number, py_random_state
...
// @nodes_or_number([0, 1]);
auto complete_bipartite_graph(n1, n2, create_using=None) -> void {
    ...
    create_using : GraphX graph instance, (default: nx.Graph);
    ...
    */
    ...
    if (G.size() != top.size() + bottom.size()) {
        throw nx.NetworkXError("Inputs n1 and n2 must contain distinct nodes");
...
// @py_random_state(3);
auto configuration_model(aseq, bseq, create_using=None, seed=None) -> void {
    ...
    create_using : GraphX graph instance, optional
       Return graph of this type.
    ...
    (aseq.size() - 1) and set B has nodes aseq.size() to (bseq.size() - 1).
...
auto havel_hakimi_graph(aseq, bseq, create_using=None) -> void {
...
auto reverse_havel_hakimi_graph(aseq, bseq, create_using=None) -> void {
    /** Returns a bipartite graph from two given degree sequences using a
    Havel-Hakimi style construction.
...
auto alternating_havel_hakimi_graph(aseq, bseq, create_using=None) -> void {
    ...
    */
    ...
    while (astubs) {
        ...
        if (stubs.size() < small.size() + large.size()) {  // check for zip truncation
...
// @py_random_state(3);
auto preferential_attachment_graph(aseq, p, create_using=None, seed=None) -> void {
    /** Create a bipartite graph with a preferential attachment model from
    a given single degree sequence.

    The graph is composed of two partitions. Set A has nodes 0 to
    (aseq.size() - 1) and set B has nodes starting with node aseq.size().
    ...
    */
    ...
    if (p > 1) {
        throw nx.NetworkXError(f"probability {p} > 1");
    ...
    while (vv) {
        while (vv[0]) {
            source = vv[0][0];
            vv[0].remove(source);
            if (seed.random() < p or G.size() == naseq) {
                target = G.size();
                G.add_node(target, bipartite=1);
                G.add_edge(source, target);
            } else {
                bb = [ [b] * G.degree(b) for b in range(naseq, G.size())];
...
// @py_random_state(3);
auto random_graph(n, m, p, seed=None, directed=false) -> void {
...
// @py_random_state(3);
auto gnmk_random_graph(n, m, k, seed=None, directed=false) -> void {
...
auto _add_nodes_with_bipartite_label(G, lena, lenb) -> void {
