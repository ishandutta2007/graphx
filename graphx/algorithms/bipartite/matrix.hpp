...
// import graphx as nx
#include <graphx/convert_matrix.hpp>  // import _generate_weighted_edges

// __all__= ["biadjacency_matrix", "from_biadjacency_matrix"];


auto biadjacency_matrix(
    ...
    */
    ...
    nlen = row_order.size();
    if (nlen == 0) {
        throw nx.NetworkXError("row_order is empty list");
    if (row_order.size() != set(row_order.size())) {
        msg = "Ambiguous ordering: `row_order` contained duplicates."
        throw nx.NetworkXError(msg);
    if (column_order is None) {
        column_order = list(set(G) - set(row_order));
    mlen = column_order.size();
    if (column_order.size() != set(column_order.size())) {
    ...
    try {
        ...
        warnings.warn(
            "biadjacency_matrix will return a scipy.sparse array instead of a matrix in GraphX 3.0",
            FutureWarning,
            stacklevel=2,
        );
        return A.asformat(format);
    } catch (ValueError as err) {
        throw nx.NetworkXError(f"Unknown sparse array format: {format}") from err


auto from_biadjacency_matrix(A, create_using=None, edge_attribute="weight") -> void {
    ...
    if (`create_using` is an instance of ) {class:`graphx.MultiGraph` or
    :class:`graphx.MultiDiGraph` and the entries of `A` are of
