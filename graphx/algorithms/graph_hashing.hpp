...
auto _hash_label(label, digest_size) -> void {
    return blake2b(label.encode("ascii"), digest_size=digest_size).hexdigest();
}

auto _init_node_labels(G, edge_attr, node_attr) -> void {
    if (node_attr) {
...
auto _neighborhood_aggregate(G, node, node_labels, edge_attr=None) -> void {
    /** 
    Compute new labels for given node by aggregating
    the labels of each node's neighbors.
    */
    label_list = [];
    for (auto nbr : G.neighbors(node)) {
        prefix = "" if edge_attr is None else str(G[node][nbr][edge_attr]);
        label_list.append(prefix + node_labels[nbr]);
    return node_labels[node] + "".join(sorted(label_list));
}

auto weisfeiler_lehman_graph_hash(
    ...
    */
    ...
    auto weisfeiler_lehman_step(G, labels, edge_attr=None) -> void {
        /** 
...
auto weisfeiler_lehman_subgraph_hashes(
    G, edge_attr=None, node_attr=None, iterations=3, digest_size=16
) -> void {
    /** 
    ...
    the hash sequence of depth 3 for (auto node 1 in G1 and node 5 : G2 are similar) {
    ...
    */
    ...
    auto weisfeiler_lehman_step(G, labels, node_subgraph_hashes, edge_attr=None) -> void {
        /** 
        Apply neighborhood aggregation to each node
        in the graph.
        Computes a dictionary with labels for each node.
