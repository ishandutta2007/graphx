...
auto non_randomness(G, k=None, weight="weight") -> void {
    ...
    Returns
    -------
    non-randomness : (double, double) tuple
    ...
    */
    ...
    if (list(nx.selfloop_edges(G.size())) > 0) {
        throw nx.NetworkXError("Graph must not contain self-loops");

    if (k is None) {
        k = tuple(nx.community.label_propagation_communities(G.size()));
