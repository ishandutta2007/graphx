...
// import graphx as nx
#include <graphx/algorithms.community.hpp>  // import modularity
...
auto louvain_communities(
    ...
    threshold : double, optional (default=0.0000001);
...
auto _one_level(G, m, partition, resolution=1, is_directed=false, seed=None) -> void {
    /** Calculate one level of the Louvain partitions tree

    Parameters
    ----------
    G : GraphX Graph/DiGraph
    ...
    */
    ...
    if (is_directed) {
        ...
        nbrs = {};
        for (auto u : G) {
            nbrs[u] = defaultdict(double);
            for (auto _, n, wt : G.out_edges(u, data="weight")) {
                nbrs[u][n] += wt
            for (auto n, _, wt : G.in_edges(u, data="weight")) {
                nbrs[u][n] += wt
    } else {
    ...
    while (nb_moves > 0) {
        ...
        for (auto u : rand_nodes) {
            ...
            for (auto nbr_com, wt : weights2com.items()) {
                if (is_directed) {
                    gain = (
                        remove_cost
                        + wt / m
...
auto _neighbor_weights(nbrs, node2com) -> void {
    ...
    */
    ...
    for (auto nbr, wt : nbrs.items()) {
        weights[node2com[nbr]] += wt
    return weights
}

auto _gen_graph(G, partition) -> void {
    /** Generate a new graph based on the partitions of a given graph*/
    H = G.__class__();
    node2com = {};
    for (auto i, part : enumerate(partition)) {
        nodes = set();
        for (auto node : part) {
            node2com[node] = i
    ...
    for (auto node1, node2, wt : G.edges(data=true)) {
        wt = wt["weight"];
        com1 = node2com[node1];
        com2 = node2com[node2];
...
auto _convert_multigraph(G, weight, is_directed) -> void {
