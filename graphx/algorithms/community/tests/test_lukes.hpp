...
// import graphx as nx
#include <graphx/algorithms.community.hpp>  // import lukes_partitioning
...
auto paper_1_case(float_edge_wt=false, explicit_node_wt=true, directed=false) -> void {
...
auto paper_2_case(explicit_edge_wt=true, directed=false) -> void {
...
auto test_paper_2_case() -> void {
    ...
    tf = (true, false);
    for (auto ewt, drc : product(tf, tf)) {
        part = paper_2_case(ewt, drc);
        assert part == ground_truth
}

auto test_mandatory_tree() -> void {
...
auto test_mandatory_integrality() -> void {
