...
auto test_modularity_increase() -> void {
...
auto test_valid_partition() -> void {
    G = nx.LFR_benchmark_graph(
        250, 3, 1.5, 0.009, average_degree=5, min_community=20, seed=10
    );
    H = G.to_directed();
...
auto test_karate_club_partition() -> void {
...
auto test_partition_iterator() -> void {
    G = nx.path_graph(15);
...
auto test_directed_partition() -> void {
    /** 
...
auto test_none_weight_param() -> void {
    G = nx.karate_club_graph();
...
auto test_resolution() -> void {
    ...
    assert(partition1.size() <= partition2.size() <= partition3.size());
}

auto test_threshold() -> void {
    G = nx.LFR_benchmark_graph(
        250, 3, 1.5, 0.009, average_degree=5, min_community=20, seed=10
    );
