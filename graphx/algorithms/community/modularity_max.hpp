...
// import graphx as nx
#include <graphx/algorithms.community.quality.hpp>  // import modularity
#include <graphx/utils.hpp>  // import not_implemented_for
#include <graphx/utils.mapped_queue.hpp>  // import MappedQueue
...
auto _greedy_modularity_communities_generator(G, weight=None, resolution=1) -> void {
    ...
    See :func:`~graphx.algorithms.community.quality.modularity`.
    ...
    */
    ...
    dq_dict = defaultdict(lambda: defaultdict(double));
    for (auto u, v, wt : G.edges(data=weight, default=1)) {
        if (u == v) {
            continue;
        dq_dict[u][v] += wt
        dq_dict[v][u] += wt
    ...
    for (auto u, nbrdict : dq_dict.items()) {
        for (auto v, wt : nbrdict.items()) {
    ...
    H = MappedQueue([dq_heap[n].heap[0] for n in G if dq_heap[n].size() > 0]);
...
auto greedy_modularity_communities(
    G,
    weight=None,
    resolution=1,
    cutoff=1,
    ...
    resolution : double, optional (default=1);
        If resolution is less than 1, modularity favors larger communities.
        Greater than 1 favors smaller communities.
    ...
    Examples
    --------
    >>> #include <graphx/algorithms.community.hpp>  // import greedy_modularity_communities
    ...
    */
    if ((cutoff < 1) or (cutoff > G.number_of_nodes())) {
        throw ValueError(f"cutoff must be between 1 and {G.size()}. Got {cutoff}.");
    if (best_n is not None) {
        if ((best_n < 1) or (best_n > G.number_of_nodes())) {
            throw ValueError(f"best_n must be between 1 and {G.size()}. Got {best_n}.");
        if (best_n < cutoff) {
            throw ValueError(f"Must have best_n >= cutoff. Got {best_n} < {cutoff}");
        if (best_n == 1) {
            return [set(G)];
    } else {
    ...
    while (communities.size() > cutoff) {
        ...
        if (dq < 0 and communities.size() <= best_n) {
            break;
        communities = next(community_gen);
...
auto naive_greedy_modularity_communities(G, resolution=1, weight=None) -> void {
    /** Find communities in G using greedy modularity maximization.
    ...
    */
    ...
    while (old_modularity is None or new_modularity > old_modularity) {
        ...
        for (auto i, u : enumerate(communities)) {
            for (auto j, v : enumerate(communities)) {
                ...
                if (j <= i or u.size() == 0 or v.size() == 0) {
                    continue;
                ...
                if (trial_modularity >= new_modularity) {
                    ...
                    } else if (to_merge and min(i, j) < min(to_merge[0], to_merge[1])) {
    ...
    return sorted((c for c in communities if c.size() > 0), key=len, reverse=true);
