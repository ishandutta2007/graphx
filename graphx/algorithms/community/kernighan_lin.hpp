...
#include <graphx/utils.hpp>  // import BinaryHeap, not_implemented_for, py_random_state

// __all__= ["kernighan_lin_bisection"];


auto _kernighan_lin_sweep(edges, side) -> void {
    /** 
...
auto kernighan_lin_bisection(G, partition=None, max_iter=10, weight="weight", seed=None) -> void {
    ...
    */
    ...
    if (partition is None) {
        ...
        try {
            A, B = partition
        } catch ((TypeError, ValueError) as err) {
            throw nx.NetworkXError("partition must be two sets") from err
        if (!is_partition(G, (A, B))) {
            throw nx.NetworkXError("partition invalid");
        side = [0] * n
        for (auto a : A) {
            side[index[a]] = 1;
