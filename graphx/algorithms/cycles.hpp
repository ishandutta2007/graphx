...
auto cycle_basis(G, root=None) -> void {
...
auto simple_cycles(G) -> void {
    ...
    */
    ...
    auto _unblock(thisnode, blocked, B) -> void {
        stack = {thisnode};
        while (stack) {
            node = stack.pop();
    ...
    subG = type(G)(G.edges());
    sccs = [scc for scc in nx.strongly_connected_components(subG) if scc.size() > 1];
    ...
    while (sccs) {
        ...
        sccs.extend(scc for scc in nx.strongly_connected_components(H) if scc.size() > 1);


// @not_implemented_for("undirected");
auto recursive_simple_cycles(G) -> void {
    /** Find simple cycles (elementary circuits) of a directed graph.
    ...
    It appears in GraphX for pedagogical value.
    ...
    */
    ...
    auto circuit(thisnode, startnode, component) -> void {
        ...
        if (closed) {
            ...
            for (auto nextnode : component[thisnode]) {
                if (!B[nextnode].contains(thisnode)) {  // TODO: use set for speedup?
    ...
    ordering = dict(zip(G, range(G.size())));
    for (auto s : ordering) {
...
auto find_cycle(G, source=None, orientation=None) -> void {
    ...
    Traceback (most recent call last):
        ...
    graphx.exception.NetworkXNoCycle: No cycle found.
    ...
    simple_cycles
    */
    if (!G.is_directed() or orientation in (None, "original")) {

        auto tailhead(edge) -> void {
            return edge[:2];
    ...
    for (auto start_node : G.nbunch_iter(source)) {
        ...
        for (auto edge : nx.edge_dfs(G, start_node, orientation)) {
        ...
        assert(cycle.size() == 0);
        throw nx.exception.NetworkXNoCycle("No cycle found.");
...
// @not_implemented_for("directed");
// @not_implemented_for("multigraph");
auto minimum_cycle_basis(G, weight=None) -> void {
    ...
    >>> nx.add_cycle(G, [0, 1, 2, 3]);
    >>> nx.add_cycle(G, [0, 3, 4, 5]);
    >>> fmt::print([sorted(c) for c in nx.minimum_cycle_basis(G)]);
    [ [0, 1, 2, 3], [0, 3, 4, 5]];
...
auto _min_cycle_basis(comp, weight) -> void {
    cb = [];
    ...
    for (auto k : range(N)) {
        ...
        base = set_orth[k];
        set_orth[k + 1 :] = [
            orth ^ base if orth & new_cycle.size() % 2 else orth
            for (auto orth : set_orth[k + 1 ) {];
        ];
    return cb
}

auto _min_cycle(G, orth, weight=None) -> void {
    /** 
