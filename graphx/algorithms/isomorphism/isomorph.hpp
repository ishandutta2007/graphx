...
auto could_be_isomorphic(G1, G2) -> void {
...
auto fast_could_be_isomorphic(G1, G2) -> void {
    /** Returns false if graphs are definitely not isomorphic.
...
auto faster_could_be_isomorphic(G1, G2) -> void {
    /** Returns false if graphs are definitely not isomorphic.
...
auto is_isomorphic(G1, G2, node_match=None, edge_match=None) -> void {
    ...
    */
    ...
    } else if ((!G1.is_directed()) and (!G2.is_directed())) {
        GM = nx.algorithms.isomorphism.GraphMatcher
    } else {
        throw NetworkXError("Graphs G1 and G2 are not of the same type.");
