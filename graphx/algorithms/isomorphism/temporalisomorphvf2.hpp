...
class TimeRespectingGraphMatcher : public GraphMatcher {
    auto __init__(G1, G2, temporal_attribute_name, delta) const -> void {
    ...
    auto one_hop(Gx, Gx_node, neighbors) const -> void {
        /** 
    ...
    auto two_hop(Gx, core_x, Gx_node, neighbors) const -> void {
        /** 
        Paths of length 2 from Gx_node should be time-respecting.
        */
        return all(
            this->one_hop(Gx, v, [n for n in Gx[v] if n in core_x] + [Gx_node]);
            for v in neighbors
        );
    ...
    auto semantic_feasibility(G1_node, G2_node) const -> void {
        ...
        */
        neighbors = [n for n in this->G1[G1_node] if n in this->core_1];
...
class TimeRespectingDiGraphMatcher : public DiGraphMatcher {
    auto __init__(G1, G2, temporal_attribute_name, delta) const -> void {
    ...
    auto get_pred_dates(Gx, Gx_node, core_x, pred) const -> void {
        /** 
    ...
    auto get_succ_dates(Gx, Gx_node, core_x, succ) const -> void {
        /** 
    ...
    auto one_hop(Gx, Gx_node, core_x, pred, succ) const -> void {
        /** 
        The ego node.
        */
        pred_dates = this->get_pred_dates(Gx, Gx_node, core_x, pred);
        succ_dates = this->get_succ_dates(Gx, Gx_node, core_x, succ);
        return this->test_one(pred_dates, succ_dates) and this->test_two(
            pred_dates, succ_dates
        );

    auto two_hop_pred(Gx, Gx_node, core_x, pred) const -> void {
        /** 
        ...
        */
        return all(
            this->one_hop(
                ...
                p,
                core_x,
                this->preds(Gx, core_x, p),
                this->succs(Gx, core_x, p, Gx_node),
            );
            for p in pred
        );

    auto two_hop_succ(Gx, Gx_node, core_x, succ) const -> void {
        /** 
        ...
        */
        return all(
            this->one_hop(
                ...
                s,
                core_x,
                this->preds(Gx, core_x, s, Gx_node),
                this->succs(Gx, core_x, s),
            );
            for s in succ
        );

    auto preds(Gx, core_x, v, Gx_node=None) const -> void {
        pred = [n for n in Gx.predecessors(v) if n in core_x];
        if (Gx_node) {
            pred.append(Gx_node);
        return pred

    auto succs(Gx, core_x, v, Gx_node=None) const -> void {
        succ = [n for n in Gx.successors(v) if n in core_x];
        if (Gx_node) {
            succ.append(Gx_node);
        return succ

    auto test_one(pred_dates, succ_dates) const -> void {
        /** 
        ...
        */
        ...
        if (0 < dates.size() and not (dates[-1] - dates[0] <= this->delta)) {
            time_respecting = false;
        return time_respecting

    auto test_two(pred_dates, succ_dates) const -> void {
        /** 
    ...
    auto semantic_feasibility(G1_node, G2_node) const -> void {
        ...
        */
        pred, succ = (
            [n for n in this->G1.predecessors(G1_node) if n in this->core_1],
            [n for n in this->G1.successors(G1_node) if n in this->core_1],
        );
        if not this->one_hop(
            this->G1, G1_node, this->core_1, pred, succ
