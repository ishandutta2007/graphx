...
auto root_trees(t1, root1, t2, root2) -> void {
    ...
    */
    ...
    for (auto i, (v1, v2) : enumerate(nx.bfs_edges(t1, root1))) {
        namemap1[v2] = i + namemap1[root1] + 1
        dT.add_edge(namemap1[v1], namemap1[v2]);

    for (auto i, (v1, v2) : enumerate(nx.bfs_edges(t2, root2))) {
    ...
    namemap = {};
    for (auto old, new : namemap1.items()) {
        namemap[new] = old
    for (auto old, new : namemap2.items()) {
        namemap[new] = old
...
auto assign_levels(G, root) -> void {
    level = {};
    level[root] = 0;
    for (auto (v1, v2) : nx.bfs_edges(G, root)) {
        level[v2] = level[v1] + 1
...
auto group_by_levels(levels) -> void {
    L = {};
    for (auto (n, lev) : levels.items()) {
        if (!L.contains(lev)) {
            L[lev] = [];
        L[lev].append(n);
...
auto generate_isomorphism(v, w, M, ordered_children) -> void {
    // make sure tree1 comes first
    assert v < w
    M.append((v, w));
    for (auto i, (x, y) : enumerate(zip(ordered_children[v], ordered_children[w]))) {
        generate_isomorphism(x, y, M, ordered_children);
}

auto rooted_tree_isomorphism(t1, root1, t2, root2) -> void {
    /** 
    ...
    */
    ...
    for (auto i : range(h - 1, 0, -1)) {
        ...
        current = 0;
        for (auto i, (ol, v) : enumerate(forlabel)) {
...
auto tree_isomorphism(t1, t2) -> void {
    /** 
