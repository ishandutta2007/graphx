...
auto copyfunc(f, name=None) -> void {
...
auto allclose(x, y, rtol=1.0000000000000001e-05, atol=1e-08) -> void {
    ...
    */
    ...
    for (auto xi, yi : zip(x, y)) {
        if (!math.isclose(xi, yi, rel_tol=rtol, abs_tol=atol)) {
            return false;
    return true;
}
...
auto categorical_node_match(attr, default) -> void {
    if (isinstance(attr, str)) {

        auto match(data1, data2) -> void {
...
auto categorical_multiedge_match(attr, default) -> void {
    if (isinstance(attr, str)) {

        auto match(datasets1, datasets2) -> void {
...
auto numerical_node_match(attr, default, rtol=1.0000000000000001e-05, atol=1e-08) -> void {
    if (isinstance(attr, str)) {
...
auto numerical_multiedge_match(attr, default, rtol=1.0000000000000001e-05, atol=1e-08) -> void {
    if (isinstance(attr, str)) {
...
>>> #include <graphx/algorithms.isomorphism.hpp>  // import generic_node_match
...
auto generic_node_match(attr, default, op) -> void {
    if (isinstance(attr, str)) {
...
auto generic_multiedge_match(attr, default, op) -> void {
    /** Returns a comparison function for a generic attribute.
