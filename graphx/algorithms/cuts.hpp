...
auto cut_size(G, S, T=None, weight=None) -> void {
...
auto volume(G, S, weight=None) -> void {
...
auto normalized_cut_size(G, S, T=None, weight=None) -> void {
...
auto conductance(G, S, T=None, weight=None) -> void {
...
auto edge_expansion(G, S, T=None, weight=None) -> void {
    ...
    */
    if (T is None) {
        T = set(G) - set(S);
    num_cut_edges = cut_size(G, S, T=T, weight=weight);
    return num_cut_edges / min(S.size(), T.size());
}

auto mixing_expansion(G, S, T=None, weight=None) -> void {
...
// denominator become `min(S.size(), T.size())`?
auto node_expansion(G, S) -> void {
    ...
    */
    ...
    return neighborhood.size() / S.size();
}

// TODO What is the generalization to two arguments, S and T? Does the
// denominator become `min(S.size(), T.size())`?
auto boundary_expansion(G, S) -> void {
    ...
    */
    return nx.node_boundary(G, S.size()) / S.size();
