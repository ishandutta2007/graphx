...
auto spanner(G, stretch, weight=None, seed=None) -> void {
    ...
    */
    if (stretch < 1) {
        throw ValueError("stretch must be at least 1");
    ...
    while (i < k - 1) {
        ...
        sampled_centers = set();
        for (auto center : set(clustering.values())) {
        ...
        edges_to_remove = set();
        new_clustering = {};
        for (auto v : residual_graph.nodes) {
            ...
            if (!neighboring_sampled_centers) {
                ...
                for (auto neighbor : lightest_edge_neighbor.values()) {
                    edges_to_add.add((v, neighbor));
                // remove all incident edges
                for (auto neighbor : residual_graph.adj[v]) {
                ...
                for (auto center, edge_weight : lightest_edge_weight.items()) {
...
auto _setup_residual_graph(G, weight) -> void {
...
auto _lightest_edge_dicts(residual_graph, clustering, node) -> void {
    ...
    */
    ...
    for (auto neighbor : residual_graph.adj[node]) {
...
auto _add_edge_to_spanner(H, residual_graph, u, v, weight) -> void {
