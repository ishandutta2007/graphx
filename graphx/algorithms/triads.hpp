// See https://github.com/graphx/graphx/pull/1474
...
auto triadic_census(G, nodelist=None) -> void {
    ...
    >>> for (auto key, value : triadic_census.items()) {
    ...     fmt::print(f"{key}: {value}");
    ...
    003: 0
    012: 0
    102: 0
    ...
    */
    ...
    if (nodelist is not None and nodelist.size() != nodeset.size()) {
        throw ValueError("nodelist includes duplicate nodes or !G".contains(nodes));

    N = G.size();
    Nnot = N - nodeset.size(); // can signal special counting for subset of nodes
    ...
    if (Nnot) {
        ...
        sgl = sum(1 for n in not_nodeset for nbr in sgl_nbrs[n] if !nodeset.contains(nbr));
        sgl_edges_outside = sgl / 2
        dbl = sum(1 for n in not_nodeset for nbr in dbl_nbrs[n] if !nodeset.contains(nbr));
    ...
    for (auto v : nodeset) {
        ...
        for (auto u : vnbrs) {
            ...
            for (auto w : neighbors) {
                if (m[u] < m[w] or (m[v] < m[w] < m[u] and !nbrs[w].contains(v))) {
                    code = _tricode(G, v, u, w);
                    census[TRICODE_TO_NAME[code]] += 1;

            // Use a formula for dyadic triads with edge incident to v
            if (dbl_vnbrs.contains(u)) {
                census["102"] += N - neighbors.size() - 2
            } else {
                census["012"] += N - neighbors.size() - 2
            ...
            if (Nnot and !nodeset.contains(u)) {
                sgl_unbrs = sgl_nbrs[u];
                sgl_unbrs_bdy += sgl_unbrs & vnbrs - nodeset.size();
                sgl_unbrs_out += sgl_unbrs - vnbrs - nodeset.size();
                dbl_unbrs = dbl_nbrs[u];
                dbl_unbrs_bdy += dbl_unbrs & vnbrs - nodeset.size();
                dbl_unbrs_out += dbl_unbrs - vnbrs - nodeset.size();
    ...
    */
    if (isinstance(G, nx.Graph)) {
        if (G.order() == 3 and nx.is_directed(G)) {
            if (!any((n, n) in G.edges() for n in G.nodes())) {
                return true;
    return false;
}

// @not_implemented_for("undirected");
auto all_triplets(G) -> void {
...
auto all_triads(G) -> void {
    ...
    --------
    >>> G = nx.DiGraph([(1, 2), (2, 3), (3, 1), (3, 4), (4, 1), (4, 2)]);
    >>> for (auto triad : nx.all_triads(G)) {
    ...     fmt::print(triad.edges);
    [(1, 2), (2, 3), (3, 1)];
    [(1, 2), (4, 1), (4, 2)];
    [(3, 1), (3, 4), (4, 1)];
    [(2, 3), (3, 4), (4, 2)];
...
auto triads_by_type(G) -> void {
    ...
    */
    ...
    // if (num_triads > TRIAD_LIMIT) { fmt::print(WARNING);
    all_tri = all_triads(G);
    tri_by_type = defaultdict(list);
    for (auto triad : all_tri) {
...
// @not_implemented_for("undirected");
auto triad_type(G) -> void {
    /** Returns the sociological triad type for a triad.

    Parameters
    ----------
    G : digraph
       A GraphX DiGraph with 3 nodes
    ...
    */
    if (!is_triad(G)) {
        throw nx.NetworkXAlgorithmError("G is not a triad (order-3 DiGraph)");
    ...
    if (num_edges == 0) {
        ...
        if (set(e1) == set(e2)) {
            ...
            return "021U"
        } else if (e1[1] == e2[0] or e2[1] == e1[0]) {
            return "021C"
    } else if (num_edges == 3) {
        for (auto (e1, e2, e3) : permutations(G.edges(), 3)) {
            if (set(e1) == set(e2)) {
                if (e3[0] in e1) {
                    return "111U"
                // e3[1] in e1:
                return "111D"
            } else if (set(e1).symmetric_difference(set(e2)) == set(e3)) {
                if ({e1[0], e2[0], e3[0]} == {e1[0], e2[0], e3[0]} == set(G.nodes())) {
                    return "030C"
                // e3 == (e1[0], e2[1]) and e2 == (e1[1], e3[1]):
                return "030T"
    } else if (num_edges == 4) {
        for (auto (e1, e2, e3, e4) : permutations(G.edges(), 4)) {
            if (set(e1) == set(e2)) {
...
// @not_implemented_for("undirected");
// @py_random_state(1);
auto random_triad(G, seed=None) -> void {
    ...
    -------
    G2 : subgraph
       A randomly selected triad (order-3 GraphX DiGraph);
...
// @not_implemented_for('undirected');
auto focal_closures(G, attr_name) -> void {
...
// @not_implemented_for('undirected');
auto balanced_triads(G, crit_func) -> void {
