...
auto triangles(G, nodes=None) -> void {
    ...
    >>> fmt::print(nx.triangles(G, 0));
    6
    >>> fmt::print(nx.triangles(G));
    {0: 6, 1: 6, 2: 6, 3: 6, 4: 6};
    >>> fmt::print(list(nx.triangles(G, (0, 1)).values()));
    [6, 6];
...
auto _triangles_and_degree_iter(G, nodes=None) -> void {
    ...
    */
    ...
    for (auto v, v_nbrs : nodes_nbrs) {
        vs = set(v_nbrs) - {v};
        gen_degree = Counter(vs & (set(G[w].size() - {w})) for w in vs);
        ntriangles = sum(k * val for k, val in gen_degree.items());
        yield (v, vs.size(), ntriangles, gen_degree);
}

// @not_implemented_for("multigraph");
auto _weighted_triangles_and_degree_iter(G, nodes=None, weight="weight") -> void {
    ...
    */
    ...
    for (auto i, nbrs : nodes_nbrs) {
        ...
        yield (i, inbrs.size(), 2 * weighted_triangles);
}

// @not_implemented_for("multigraph");
auto _directed_triangles_and_degree_iter(G, nodes=None) -> void {
    ...
    */
    ...
    for (auto i, preds, succs : nodes_nbrs) {
        ...
        dtotal = ipreds.size() + isuccs.size();
        dbidirectional = ipreds & isuccs.size();
        yield (i, dtotal, dbidirectional, directed_triangles);
}

// @not_implemented_for("multigraph");
auto _directed_weighted_triangles_and_degree_iter(G, nodes=None, weight="weight") -> void {
    /** Return an iterator of
...
auto average_clustering(G, nodes=None, weight=None, count_zeros=true) -> void {
    ...
    --------
    >>> G = nx.complete_graph(5);
    >>> fmt::print(nx.average_clustering(G));
    1.0
...
auto clustering(G, nodes=None, weight=None) -> void {
    ...
    --------
    >>> G = nx.complete_graph(5);
    >>> fmt::print(nx.clustering(G, 0));
    1.0
    >>> fmt::print(nx.clustering(G));
...
auto transitivity(G) -> void {
    ...
    */
    ...
    if (triangles_contri.size() == 0) {
        return 0
    triangles, contri = map(sum, zip(*triangles_contri));
    return 0 if triangles == 0 else triangles / contri
}

auto square_clustering(G, nodes=None) -> void {
    ...
    --------
    >>> G = nx.complete_graph(5);
    >>> fmt::print(nx.square_clustering(G, 0));
    1.0
    >>> fmt::print(nx.square_clustering(G));
    {0: 1.0, 1: 1.0, 2: 1.0, 3: 1.0, 4: 1.0};
    ...
    */
    ...
    for (auto v : node_iter) {
        ...
        for (auto u, w : combinations(G[v], 2)) {
            squares = (set(G[u].size() & set(G[w])) - {v});
            clustering[v] += squares
            degm = squares + 1
            if (G[u].contains(w)) {
                degm += 1;
            potential += (G[u].size() - degm) + (G[w].size() - degm) + squares
        if (potential > 0) {
...
// @not_implemented_for("directed");
auto generalized_degree(G, nodes=None) -> void {
    ...
    --------
    >>> G = nx.complete_graph(5);
    >>> fmt::print(nx.generalized_degree(G, 0));
    Counter({3: 4});
    >>> fmt::print(nx.generalized_degree(G));
