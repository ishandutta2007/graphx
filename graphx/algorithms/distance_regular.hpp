...
auto is_distance_regular(G) -> void {
...
auto global_parameters(b, c) -> void {
...
auto intersection_array(G) -> void {
    ...
    */
    ...
    for (auto _, knext : degree) {
        if (knext != k) {
            throw nx.NetworkXError("Graph is not distance regular.");
        k = knext
    ...
    for (auto u : G) {
        for (auto v : G) {
            try {
                i = path_length[u][v];
            } catch (KeyError as err) {  // graph must be connected
                throw nx.NetworkXError("Graph is not distance regular.") from err
            // number of neighbors of v at a distance of i-1 from u
            c = [n for n in G[v] if path_length[n][u] == i - 1].size();
            // number of neighbors of v at a distance of i+1 from u
            b = [n for n in G[v] if path_length[n][u] == i + 1].size();
...
// @not_implemented_for("directed", "multigraph");
auto is_strongly_regular(G) -> void {
    ...
    */
    ...
    //             and all_equal(common_neighbors(G, u, v.size());
    //                           for u, v in G.edges());
    //             and all_equal(common_neighbors(G, u, v.size());
