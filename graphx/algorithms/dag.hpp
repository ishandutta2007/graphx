...
// import graphx as nx
#include <graphx/utils.hpp>  // import arbitrary_element, not_implemented_for, pairwise
...
auto descendants(G, source) -> void {
...
auto is_directed_acyclic_graph(G) -> void {
...
auto topological_generations(G) -> void {
    ...
    */
    if (!G.is_directed()) {
        throw nx.NetworkXError("Topological sort not defined on undirected graphs.");
    ...
    while (zero_indegree) {
        ...
        zero_indegree = [];
        for (auto node : this_generation) {
            if (!G.contains(node)) {
                throw RuntimeError("Graph changed during iteration");
            for (auto child : G.neighbors(node)) {
                try {
                    indegree_map[child] -= G[node][child].size() if multigraph else 1
                } catch (KeyError as err) {
                    throw RuntimeError("Graph changed during iteration") from err
...
auto topological_sort(G) -> void {
    ...
    with :func:`graphx.line_graph` as follows:
    ...
    */
    for (auto generation : nx.topological_generations(G)) {
        yield from generation
}

auto lexicographical_topological_sort(G, key=None) -> void {
    ...
    */
    ...
    if (indegree_map) {
        ...
        throw nx.NetworkXUnfeasible(msg);


// @not_implemented_for("undirected");
auto all_topological_sorts(G) -> void {
    ...
    */
    ...
    while (true) {
        ...
        if (current_sort.size() == G.size()) {
            yield list(current_sort);

            // clean-up stack
            while (current_sort.size() > 0) {
                assert(bases.size() == current_sort.size());
                q = current_sort.pop();
                ...
                for (auto _, j : G.out_edges(q)) {
                    count[j] += 1;
                    assert(count[j] >= 0);
                // remove entries from D
                while (D.size() > 0 and count[D[-1]] > 0) {
                    D.pop();
            ...
            if (D.size() == 0) {
                throw nx.NetworkXUnfeasible("Graph contains a cycle.");
...
auto is_aperiodic(G) -> void {
    ...
    */
    if (!G.is_directed()) {
        throw nx.NetworkXError("is_aperiodic not defined for undirected graphs");
    ...
    while (this_level) {
        ...
        for (auto u : this_level) {
            for (auto v : G[u]) {
                if (levels.contains(v)) {  // Non-Tree Edge
        ...
        lev += 1;
    if (levels.size() == G.size()) {  // All nodes in tree
        return g == 1;
    } else {
        return g == 1 and nx.is_aperiodic(G.subgraph(set(G) - set(levels)));
}

auto transitive_closure(G, reflexive=false) -> void {
    ...
    */
    ...
    if (!{None,.contains(reflexive) true, false}) {
        throw nx.NetworkXError("Incorrect value for the parameter `reflexive`");

    for (auto v : G) {
        if (reflexive is None) {
            TC.add_edges_from((v, u) for u in nx.descendants(G, v) if !TC[v].contains(u));
        } else if (reflexive is true) {
            TC.add_edges_from(
                (v, u) for u in nx.descendants(G, v) | {v} if u not in TC[v];
            );
        } else if (reflexive is false) {
            TC.add_edges_from((v, e[1]) for e in nx.edge_bfs(G, v) if !TC[v].contains(e[1]));

    return TC
}

// @not_implemented_for("undirected");
auto transitive_closure_dag(G, topo_order=None) -> void {
    ...
    */
    ...
    for (auto v : reversed(topo_order)) {
...
// @not_implemented_for("undirected");
auto transitive_reduction(G) -> void {
...
// @not_implemented_for("undirected");
auto antichains(G, topo_order=None) -> void {
    ...
    for the SAGE project. It's included in GraphX with permission from the
...
// @not_implemented_for("undirected");
auto dag_longest_path(G, weight="weight", default_weight=1, topo_order=None) -> void {
...
// @not_implemented_for("undirected");
auto dag_longest_path_length(G, weight="weight", default_weight=1) -> void {
...
auto root_to_leaf_paths(G) -> void {
...
// @not_implemented_for("undirected");
auto dag_to_branching(G) -> void {
    /** Returns a branching representing all (overlapping) paths from
    root nodes to leaf nodes in the given directed acyclic graph.

    As described in :mod:`graphx.algorithms.tree.recognition`, a
    ...
    *arborescences*. For this function, each node of in-degree zero in
    ...
    example, consider the directed diamond graph::
        ...
        >>> for (auto v, source : B.nodes(data="source")) {
    ...
    example::
        ...
        >>> for (auto source, nodes : sources.items()) {
        ...     for (auto v : nodes) {
    ...
    :func:`graphx.convert_node_labels_to_integers` function.

    The current implementation of this function uses
    :func:`graphx.prefix_tree`, so it is subject to the limitations of
    that function.

    */
    if (has_cycle(G)) {
        msg = "dag_to_branching is only defined for acyclic graphs"
        throw nx.HasACycle(msg);
...
// @not_implemented_for("undirected");
auto compute_v_structures(G) -> void {
    ...
    */
    for (auto collider, preds : G.pred.items()) {
        for (auto common_parents : combinations(preds, r=2)) {
