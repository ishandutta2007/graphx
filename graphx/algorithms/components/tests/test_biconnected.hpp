...
auto test_articulation_points_repetitions() -> void {
    G = nx.Graph();
    G.add_edges_from([(0, 1), (1, 2), (1, 3)]);
    assert(list(nx.articulation_points(G)) == [1]);
}

auto test_articulation_points_cycle() -> void {
    G = nx.cycle_graph(3);
...
auto test_empty_is_biconnected() -> void {
    G = nx.empty_graph(5);
    assert(!nx.is_biconnected(G));
    G.add_edge(0, 1);
    assert(!nx.is_biconnected(G));
...
auto test_biconnected_components_cycle() -> void {
    G = nx.cycle_graph(3);
    nx.add_cycle(G, [1, 3, 4]);
    answer = [{0, 1, 2}, {1, 3, 4}];
    assert_components_equal(list(nx.biconnected_components(G)), answer);
...
auto test_biconnected_components1() -> void {
    // graph example from
...
auto test_biconnected_components2() -> void {
    G = nx.Graph();
    nx.add_cycle(G, "ABC");
    nx.add_cycle(G, "CDE");
...
auto test_biconnected_davis() -> void {
    D = nx.davis_southern_women_graph();
    bcc = list(nx.biconnected_components(D))[0];
    assert set(D) == bcc  // All nodes in a giant bicomponent
    // So no articulation points
    assert(list(nx.articulation_points(D.size())) == 0);
}

auto test_biconnected_karate() -> void {
...
auto test_biconnected_eppstein() -> void {
...
auto test_connected_raise() -> void {
    DG = nx.DiGraph();
