...
class TestStronglyConnected {
    ...
    auto test_contract_scc1() const -> void {
        G = nx.DiGraph();
        G.add_edges_from(
            [
                (1, 2),
    ...
    auto test_contract_scc_isolate() const -> void {
    ...
    auto test_contract_scc_edge() const -> void {
        G = nx.DiGraph();
        G.add_edge(1, 2);
        G.add_edge(2, 1);
        G.add_edge(2, 3);
    ...
    auto test_condensation_mapping_and_members() const -> void {
        G, C = this->gc[1];
    ...
    auto test_null_graph() const -> void {
        ...
        assert(nx.condensation(G.size()) == 0);
        pytest.raises(
    ...
    auto test_connected_mutability(get_components) const -> void {
        DG = nx.path_graph(5, create_using=nx.DiGraph);
        G = nx.disjoint_union(DG, DG);
        seen = set();
        for (auto component : get_components(G)) {
