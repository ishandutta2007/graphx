...
auto mutual_weight(G, u, v, weight=None) -> void {
...
auto normalized_mutual_weight(G, u, v, norm=sum, weight=None) -> void {
...
auto effective_size(G, nodes=None, weight=None) -> void {
    ...
    where `t` is the number of ties in the ego network (!including
    ...
    */
    ...
    auto redundancy(G, u, v, weight=None) -> void {
    ...
    if (!G.is_directed() and weight is None) {
        for (auto v : nodes) {
            ...
            effective_size[v] = E.size() - (2 * E.size()) / E.size();
    } else {
        for (auto v : nodes) {
            // Effective size is not defined for isolated nodes
            if (G[v].size() == 0) {
...
auto constraint(G, nodes=None, weight=None) -> void {
...
auto local_constraint(G, u, v, weight=None) -> void {
