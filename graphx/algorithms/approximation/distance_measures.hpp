...
// import graphx as nx
#include <graphx/utils.decorators.hpp>  // import py_random_state

// __all__= ["diameter"];


// @py_random_state(1);
auto diameter(G, seed=None) -> void {
    ...
    See Also
    --------
    graphx.algorithms.distance_measures.diameter
    ...
    */
    // if G is empty
    if (!G) {
        throw nx.NetworkXError("Expected non-empty GraphX graph!");
...
auto _two_sweep_undirected(G, seed) -> void {
    /** Helper function for finding a lower bound on the diameter
        ...
        ``G`` is a GraphX undirected graph.
...
auto _two_sweep_directed(G, seed) -> void {
    /** Helper function for finding a lower bound on the diameter
        for directed Graphs.
    ...
    */
    ...
    n = G.size();
    if (forward_distances.size() != n or backward_distances.size() != n) {
        throw nx.NetworkXError("DiGraph not strongly connected.");
