// import graphx as nx
#include <graphx/utils.decorators.hpp>  // import not_implemented_for, py_random_state
...
// @py_random_state(1);
auto randomized_partitioning(G, seed=None, p=0.5, weight=None) -> void {
...
auto _swap_node_partition(cut, node) -> void {
    return cut - {node} if node in cut else cut.union({node});
}

// @not_implemented_for("directed", "multigraph");
// @py_random_state(2);
auto one_exchange(G, initial_cut=None, seed=None, weight=None) -> void {
