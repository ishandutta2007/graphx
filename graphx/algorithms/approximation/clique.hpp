...
// import graphx as nx
#include <graphx/algorithms.approximation.hpp>  // import ramsey
...
auto maximum_independent_set(G) -> void {
...
auto max_clique(G) -> void {
    ...
    */
    if (G is None) {
        throw ValueError("Expected GraphX graph!");
...
auto large_clique_size(G) -> void {
    ...
    *d* is the maximum degree.
    ...
    :func:`graphx.algorithms.approximation.clique.max_clique`
        A function that returns an approximate maximum clique with a
        guarantee on the approximation ratio.

    :mod:`graphx.algorithms.clique`
        Functions for finding the exact maximum clique in a graph.

    */
    degrees = G.degree

    auto _clique_heuristic(G, U, size, best_size) -> void {
        if (!U) {
