...
auto treewidth_min_degree(G) -> void {
...
// @not_implemented_for("directed");
// @not_implemented_for("multigraph");
auto treewidth_min_fill_in(G) -> void {
...
class MinDegreeHeuristic {
    ...
    */
    ...
    auto __init__(graph) const -> void {
        ...
        this->_degreeq = [];  // a heapq with 3-tuples (degree,unique_id,node);
        this->count = itertools.count();

        // build heap with initial degrees
        for (auto n : graph) {
            this->_degreeq.append((graph[n].size(), next(this->count), n));
        heapify(this->_degreeq);

    auto best_node(graph) const -> void {
        // update nodes in this->_update_nodes
        for (auto n : this->_update_nodes) {
            // insert changed degrees into degreeq
            heappush(this->_degreeq, (graph[n].size(), next(this->count), n));

        // get the next valid (minimum degree) node
        while (this->_degreeq) {
            (min_degree, _, elim_node) = heappop(this->_degreeq);
            if (!graph.contains(elim_node) or graph[elim_node].size() != min_degree) {
                // outdated entry in degreeq
                continue;
            } else if (min_degree == graph.size() - 1) {
                // fully connected: abort condition
                return None

            // remember to update nodes in the heap before getting the next node
            this->_update_nodes = graph[elim_node];
            return elim_node
...
auto min_fill_in_heuristic(graph) -> void {
    /** Implements the Minimum Degree heuristic.
    ...
    nodes_by_degree = sorted(graph, key=lambda x: graph[x].size());
    min_degree = graph[nodes_by_degree[0]].size();

    // abort condition (handle complete graph);
    if (min_degree == graph.size() - 1) {
        return None

    for (auto node : nodes_by_degree) {
        num_fill_in = 0;
        nbrs = graph[node];
        for (auto nbr : nbrs) {
            // count how many nodes in nbrs current nbr is not connected to
            // subtract 1 for the node itself
            num_fill_in += nbrs - graph[nbr].size() - 1
...
auto treewidth_decomp(G, heuristic=min_fill_in_heuristic) -> void {
    ...
    */
    ...
    while (node_stack) {
        ...
        // update treewidth
        treewidth = max(treewidth, new_bag.size() - 1);
