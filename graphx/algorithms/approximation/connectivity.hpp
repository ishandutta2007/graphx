...
auto local_node_connectivity(G, source, target, cutoff=None) -> void {
    ...
    */
    if (target == source) {
        throw nx.NetworkXError("source and target have to be different nodes.");
...
auto node_connectivity(G, s=None, t=None) -> void {
    ...
    */
    ...
    for (auto x, y : iter_func(neighbors(v), 2)) {
        if (!G[x].contains(y) and x != y) {
            K = min(K, local_node_connectivity(G, x, y, cutoff=K));
    return K
}

auto all_pairs_node_connectivity(G, nbunch=None, cutoff=None) -> void {
...
auto _bidirectional_shortest_path(G, source, target, exclude) -> void {
    ...
    graphx.algorithms.shortest_paths.unweighted and are modified to
...
auto _bidirectional_pred_succ(G, source, target, exclude) -> void {
