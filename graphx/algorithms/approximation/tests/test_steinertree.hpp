...
// import graphx as nx
#include <graphx/algorithms.approximation.steinertree.hpp>  // import metric_closure, steiner_tree
...
class TestSteinerTree {
    ...
    auto test_connected_metric_closure() const -> void {
        G = this->G1.copy();
        G.add_node(100);
        pytest.raises(nx.NetworkXError, metric_closure, G);

    auto test_metric_closure() const -> void {
        M = metric_closure(this->G1);
        mc = [
    ...
    auto test_steiner_tree() const -> void {
        valid_steiner_trees = [
            [
                [
                    (1, 2, {"weight": 10}),
        ...
        for (auto method : this->methods) {
            for (auto G, term_nodes, valid_trees : zip(
                [this->G1, this->G2, this->G3],
                [this->G1_term_nodes, this->G2_term_nodes, this->G3_term_nodes],
                valid_steiner_trees,
            )) {
    ...
    auto test_multigraph_steiner_tree() const -> void {
        G = nx.MultiGraph();
        G.add_edges_from(
            [
                (1, 2, 0, {"weight": 1}),
