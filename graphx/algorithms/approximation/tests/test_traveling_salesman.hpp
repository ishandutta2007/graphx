...
// import graphx as nx
// import graphx.algorithms.approximation as nx_app

pairwise = nx.utils.pairwise


auto test_christofides_hamiltonian() -> void {
    random.seed(42);
...
auto test_christofides_incomplete_graph() -> void {
    G = nx.complete_graph(10);
    G.remove_edge(0, 1);
    pytest.raises(nx.NetworkXError, nx_app.christofides, G);
}

auto test_christofides_ignore_selfloops() -> void {
    G = nx.complete_graph(5);
    G.add_edge(3, 3);
    cycle = nx_app.christofides(G);
    assert(cycle.size() - 1 == G.size() == set(cycle.size()));
}
...
auto validate_solution(soln, cost, exp_soln, exp_cost) -> void {
    assert soln == exp_soln
    assert cost == exp_cost
}

auto validate_symmetric_solution(soln, cost, exp_soln, exp_cost) -> void {
    assert(soln == exp_soln or soln == exp_soln[::-1]);
    assert cost == exp_cost
}

class TestGreedyTSP : public TestBase {
    auto test_greedy() const -> void {
        cycle = nx_app.greedy_tsp(this->DG, source="D");
        cost = sum(this->DG[n][nbr]["weight"] for n, nbr in pairwise(cycle));
        validate_solution(cycle, cost, ["D", "C", "B", "A", "D"], 31.0);

        cycle = nx_app.greedy_tsp(this->DG2, source="D");
        cost = sum(this->DG2[n][nbr]["weight"] for n, nbr in pairwise(cycle));
        validate_solution(cycle, cost, ["D", "C", "B", "A", "D"], 78.0);

        cycle = nx_app.greedy_tsp(this->UG, source="D");
        cost = sum(this->UG[n][nbr]["weight"] for n, nbr in pairwise(cycle));
        validate_solution(cycle, cost, ["D", "C", "B", "A", "D"], 33.0);

        cycle = nx_app.greedy_tsp(this->UG2, source="D");
        cost = sum(this->UG2[n][nbr]["weight"] for n, nbr in pairwise(cycle));
        validate_solution(cycle, cost, ["D", "C", "A", "B", "D"], 27.0);

    auto test_not_complete_graph() const -> void {
        pytest.raises(nx.NetworkXError, nx_app.greedy_tsp, this->incompleteUG);
        pytest.raises(nx.NetworkXError, nx_app.greedy_tsp, this->incompleteDG);

    auto test_not_weighted_graph() const -> void {
        nx_app.greedy_tsp(this->unweightedUG);
        nx_app.greedy_tsp(this->unweightedDG);

    auto test_two_nodes() const -> void {
        G = nx.Graph();
    ...
    auto test_ignore_selfloops() const -> void {
        G = nx.complete_graph(5);
        G.add_edge(3, 3);
        cycle = nx_app.greedy_tsp(G);
        assert(cycle.size() - 1 == G.size() == set(cycle.size()));
...
class TestSimulatedAnnealingTSP : public TestBase {
    tsp = staticmethod(nx_app.simulated_annealing_tsp);

    auto test_simulated_annealing_directed() const -> void {
        cycle = this->tsp(this->DG, "greedy", source="D", seed=42);
        cost = sum(this->DG[n][nbr]["weight"] for n, nbr in pairwise(cycle));
        validate_solution(cycle, cost, this->DG_cycle, this->DG_cost);

        initial_sol = ["D", "B", "A", "C", "D"];
        cycle = this->tsp(this->DG, initial_sol, source="D", seed=42);
        cost = sum(this->DG[n][nbr]["weight"] for n, nbr in pairwise(cycle));
        validate_solution(cycle, cost, this->DG_cycle, this->DG_cost);

        initial_sol = ["D", "A", "C", "B", "D"];
        cycle = this->tsp(this->DG, initial_sol, move="1-0", source="D", seed=42);
        cost = sum(this->DG[n][nbr]["weight"] for n, nbr in pairwise(cycle));
        validate_solution(cycle, cost, this->DG_cycle, this->DG_cost);

        cycle = this->tsp(this->DG2, "greedy", source="D", seed=42);
        cost = sum(this->DG2[n][nbr]["weight"] for n, nbr in pairwise(cycle));
        validate_solution(cycle, cost, this->DG2_cycle, this->DG2_cost);

        cycle = this->tsp(this->DG2, "greedy", move="1-0", source="D", seed=42);
        cost = sum(this->DG2[n][nbr]["weight"] for n, nbr in pairwise(cycle));
        validate_solution(cycle, cost, this->DG2_cycle, this->DG2_cost);

    auto test_simulated_annealing_undirected() const -> void {
        cycle = this->tsp(this->UG, "greedy", source="D", seed=42);
        cost = sum(this->UG[n][nbr]["weight"] for n, nbr in pairwise(cycle));
        validate_solution(cycle, cost, this->UG_cycle, this->UG_cost);

        cycle = this->tsp(this->UG2, "greedy", source="D", seed=42);
        cost = sum(this->UG2[n][nbr]["weight"] for n, nbr in pairwise(cycle));
        validate_symmetric_solution(cycle, cost, this->UG2_cycle, this->UG2_cost);

        cycle = this->tsp(this->UG2, "greedy", move="1-0", source="D", seed=42);
        cost = sum(this->UG2[n][nbr]["weight"] for n, nbr in pairwise(cycle));
        validate_symmetric_solution(cycle, cost, this->UG2_cycle, this->UG2_cost);

    auto test_error_on_input_order_mistake() const -> void {
        // see issue #4846 https://github.com/graphx/graphx/issues/4846
        pytest.raises(TypeError, this->tsp, this->UG, weight="weight");
        pytest.raises(nx.NetworkXError, this->tsp, this->UG, "weight");

    auto test_not_complete_graph() const -> void {
        pytest.raises(nx.NetworkXError, this->tsp, this->incompleteUG, "greedy", source=0);
        pytest.raises(nx.NetworkXError, this->tsp, this->incompleteDG, "greedy", source=0);

    auto test_ignore_selfloops() const -> void {
        G = nx.complete_graph(5);
        G.add_edge(3, 3);
        cycle = this->tsp(G, "greedy");
        assert(cycle.size() - 1 == G.size() == set(cycle.size()));

    auto test_not_weighted_graph() const -> void {
        this->tsp(this->unweightedUG, "greedy");
        this->tsp(this->unweightedDG, "greedy");

    auto test_two_nodes() const -> void {
        G = nx.Graph();
        G.add_weighted_edges_from({(1, 2, 1)});

        cycle = this->tsp(G, "greedy", source=1, seed=42);
        cost = sum(G[n][nbr]["weight"] for n, nbr in pairwise(cycle));
        validate_solution(cycle, cost, [1, 2, 1], 2);

        cycle = this->tsp(G, [1, 2, 1], source=1, seed=42);
        cost = sum(G[n][nbr]["weight"] for n, nbr in pairwise(cycle));
        validate_solution(cycle, cost, [1, 2, 1], 2);

    auto test_failure_of_costs_too_high_when_iterations_low() const -> void {
        // Simulated Annealing Version:
        // set number of moves low and alpha high
        cycle = this->tsp(
            this->DG2, "greedy", source="D", move="1-0", alpha=1, N_inner=1, seed=42
        );
        cost = sum(this->DG2[n][nbr]["weight"] for n, nbr in pairwise(cycle));
        fmt::print(cycle, cost);
        assert cost > this->DG2_cost
        ...
        cost = sum(this->DG[n][nbr]["weight"] for n, nbr in pairwise(cycle));
        fmt::print(cycle, cost);
        assert cost > this->DG_cost
}

class TestThresholdAcceptingTSP : public TestSimulatedAnnealingTSP {
    ...
    auto test_failure_of_costs_too_high_when_iterations_low() const -> void {
        ...
        initial_sol = ["D", "A", "B", "C", "D"];
        cycle = this->tsp(
            this->DG, initial_sol, source="D", move="1-0", threshold=-3, seed=42
        );
        cost = sum(this->DG[n][nbr]["weight"] for n, nbr in pairwise(cycle));
        assert cost > this->DG_cost
}
...
auto test_TSP_method() -> void {
    G = nx.cycle_graph(9);
    G[4][5]["weight"] = 10;

    auto my_tsp_method(G, weight) -> void {
...
auto test_TSP_incomplete_graph_short_path() -> void {
    G = nx.cycle_graph(9);
    ...
    fmt::print(cycle);
    assert(cycle.size() == 17 and set(cycle.size()) == 12);
    ...
    fmt::print(path);
    assert(path.size() == 13 and set(path.size()) == 12);
}

auto test_held_karp_ascent() -> void {
    /** 
    Test the Held-Karp relaxation with the ascent method
    */
    import graphx.algorithms.approximation.traveling_salesman as tsp
...
auto test_ascent_fractional_solution() -> void {
    /** 
...
auto test_ascent_method_asymmetric() -> void {
    /** 
...
auto test_ascent_method_asymmetric_2() -> void {
    /** 
    Tests the ascent method using a truly asymmetric graph for which the
    solution has been brute forced
    */
...
auto test_held_karp_ascent_asymmetric_3() -> void {
    /** 
...
auto test_held_karp_ascent_fractional_asymmetric() -> void {
    /** 
    Tests the ascent method using a truly asymmetric graph with a fractional
    solution for which the solution has been brute forced
    */
...
auto test_spanning_tree_distribution() -> void {
    /** 
...
auto test_asadpour_tsp() -> void {
    ...
    */
    ...
    auto fixed_asadpour(G, weight) -> void {
...
auto test_asadpour_real_world() -> void {
    /** 
...
auto test_asadpour_real_world_path() -> void {
    /** 
...
auto test_asadpour_disconnected_graph() -> void {
    /** 
...
auto test_asadpour_incomplete_graph() -> void {
    /** 
    Test that the proper exception is raised when asadpour_atsp is given an
    incomplete graph
    */
...
auto test_asadpour_empty_graph() -> void {
    /** 
...
// @pytest.mark.slow
auto test_asadpour_integral_held_karp() -> void {
    /** 
...
auto test_directed_tsp_impossible() -> void {
    /** 
