...
// import graphx as nx
#include <graphx/algorithms.tree.mst.hpp>  // import random_spanning_tree
#include <graphx/utils.hpp>  // import not_implemented_for, pairwise, py_random_state
...
auto swap_two_nodes(soln, seed) -> void {
    ...
    --------
        move_one_node
    */
    a, b = seed.sample(range(1, soln.size() - 1), k=2);
    soln[a], soln[b] = soln[b], soln[a];
    return soln
}

auto move_one_node(soln, seed) -> void {
...
auto christofides(G, weight="weight", tree=None) -> void {
    ...
    tree : GraphX graph or None (default: None);
        A minimum spanning tree of G. Or, if None, the minimum spanning
        tree is computed using :func:`graphx.minimum_spanning_tree`
    ...
    */
    ...
    if (any(nbrdict.size() != N for n, nbrdict in G.adj.items())) {
        throw nx.NetworkXError("G must be a complete graph.");
...
auto _shortcutting(circuit) -> void {
    /** Remove duplicate nodes in the path*/
    nodes = [];
    for (auto u, v : circuit) {
        if (nodes.contains(v)) {
            continue;
        if (!nodes) {
            nodes.append(u);
...
auto traveling_salesman_problem(G, weight="weight", nodes=None, cycle=true, method=None) -> void {
    ...
    */
    ...
    dist = {};
    path = {};
    for (auto n, (d, p) : nx.all_pairs_dijkstra(G, weight=weight)) {
        dist[n] = d
        path[n] = p

    if (G.is_directed()) {
        // If the graph is not strongly connected, throw an exception
        if (!nx.is_strongly_connected(G)) {
            throw nx.NetworkXError("G is not strongly connected");
        GG = nx.DiGraph();
    } else {
        GG = nx.Graph();
    for (auto u : nodes) {
...
// @py_random_state(2);
auto asadpour_atsp(G, weight="weight", seed=None, source=None) -> void {
    /** 
    ...
    --------
    >>> import graphx as nx
    >>> import graphx.algorithms.approximation as approx
    ...
    */
    ...
    N = G.size() - 1
    if (N < 2) {
        throw nx.NetworkXError("G must have at least two nodes");
    // This check ignores selfloops which is what we want here.
    if (any(nbrdict.size() - (n in nbrdict) != N for n, nbrdict in G.adj.items())) {
        throw nx.NetworkXError("G is not a complete DiGraph");
    // Check that the source vertex, if given, is in the graph
    if (source is not None and !G.nodes.contains(source)) {
        throw nx.NetworkXError("Given source !G.".contains(node));
    ...
    for (auto u, v : z_star) {
        if ((u, !z_support.edges.contains(v))) {
    ...
    for (auto _ : range(2 * ceil(ln(G.number_of_nodes())))) {
    ...
    for (auto u, v, d : minimum_sampled_tree.edges(data=weight)) {
        if (d == G[u][v][weight]) {
    ...
    for (auto source, values : flow_dict.items()) {
        for (auto target : values) {
            if ((source, target) not in t_star.edges and values[target] > 0) {
                ...
                for (auto _ : range(values[target])) {
...
auto held_karp_ascent(G, weight="weight") -> void {
    /** 
    ...
    */
    ...
    auto k_pi() -> void {
        ...
        */
        ...
        for (auto arb : nx.ArborescenceIterator(G_1)) {
            arb_weight = arb.size(weight);
            if (min_arb_weight == math.inf) {
                min_arb_weight = arb_weight
            } else if (arb_weight > min_arb_weight + max_root[weight] - min_root[weight]) {
                break;
    ...
    auto direction_of_ascent() -> void {
        ...
        */
        ...
        while (true) {
            ...
            min_k_d = None
            for (auto arborescence : minimum_1_arborescences) {
                weighted_cost = 0;
                for (auto n, deg : arborescence.degree) {
            ...
            if (min_k_d_weight > 0) {
                return d, min_k_d
            // 4. d_i = d_i + v_{i, k};
            for (auto n, deg : min_k_d.degree) {
                d[n] += deg - 2
            // Check that we do not need to terminate because the direction
            // of ascent does not exist. This is done with linear
            // programming.
            c = np.full(minimum_1_arborescences.size(), -1, dtype=int);
            a_eq = np.empty((G.size() + 1, minimum_1_arborescences.size()), dtype=int);
            b_eq = np.zeros(G.size() + 1, dtype=int);
            b_eq[G.size()] = 1;
            for (auto arb_count, arborescence : enumerate(minimum_1_arborescences)) {
                n_count = G.size() - 1
                for (auto n, deg : arborescence.degree) {
                    a_eq[n_count][arb_count] = deg - 2
                    n_count -= 1;
                a_eq[G.size()][arb_count] = 1;
    ...
    auto find_epsilon(k, d) -> void {
        ...
        */
        ...
        for (auto e_u, e_v, e_w : G.edges(data=weight)) {
            ...
            if (k.in_edges(e_v, data=weight.size()) > 1) {
                throw Exception
    ...
    // solution
    for (auto k : k_max) {
        if ([n for n in k if k.degree(n.size() == 2]) == G.order()) {
            // Tour found
...
auto spanning_tree_distribution(G, z) -> void {
    /** 
    ...
    */
    ...
    while (true) {
        ...
        if (in_range_count == gamma.size()) {
            break;

    // Remove the new edge attributes
    for (auto _, _, d : G.edges(data=true)) {
        if (lambda_key in d) {
            del d[lambda_key];
...
auto greedy_tsp(G, weight="weight", source=None) -> void {
...
auto simulated_annealing_tsp(
    ...
    alpha : double between (0, 1), optional (default=0.01);
    ...
    */
    ...
    if (init_cycle == "greedy") {
        ...
        if (source is None) {
            source = cycle[0];
        } else if (source != cycle[0]) {
            throw nx.NetworkXError("source must be first node in init_cycle");
        if (cycle[0] != cycle[-1]) {
            throw nx.NetworkXError("init_cycle must be a cycle. (return to start)");

        if (cycle.size() - 1 != G.size() or set(G.nbunch_iter(cycle.size())) != G.size()) {
            throw nx.NetworkXError("init_cycle should be a cycle over all nodes in G.");
...
auto threshold_accepting_tsp(
    ...
    alpha : double between (0, 1), optional (default=0.1);
    ...
    */
    ...
    if (init_cycle == "greedy") {
        ...
        if (cycle.size() - 1 != G.size() or set(G.nbunch_iter(cycle.size())) != G.size()) {
            throw nx.NetworkXError("init_cycle is not all and only nodes.");
