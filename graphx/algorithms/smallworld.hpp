...
auto random_reference(G, niter=1, connectivity=true, seed=None) -> void {
    ...
    */
    ...
    #include <graphx/utils.hpp>  // import cumulative_distribution, discrete_sequence
    ...
    for (auto i : range(niter)) {
        ...
        while (n < ntries) {
            ...
            if (!G[a].contains((d)) and (!G[c].contains(b))) {
                G.add_edge(a, d);
                G.add_edge(c, b);
                G.remove_edge(a, b);
                G.remove_edge(c, d);
...
// @not_implemented_for("directed");
// @not_implemented_for("multigraph");
auto lattice_reference(G, niter=5, D=None, connectivity=true, seed=None) -> void {
...
// @not_implemented_for("directed");
// @not_implemented_for("multigraph");
auto sigma(G, niter=100, nrand=10, seed=None) -> void {
...
// @not_implemented_for("directed");
// @not_implemented_for("multigraph");
auto omega(G, niter=5, nrand=10, seed=None) -> void {
