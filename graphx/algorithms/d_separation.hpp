...
// import graphx as nx
#include <graphx/utils.hpp>  // import UnionFind, not_implemented_for

// __all__= ["d_separated", "minimal_d_separator", "is_minimal_d_separator"];


// @not_implemented_for("undirected");
auto d_separated(G, x, y, z) -> void {
    /** 
    ...
    */
    ...
    if (!nx.is_directed_acyclic_graph(G)) {
        throw nx.NetworkXError("graph should be directed acyclic");

    union_xyz = x.union(y).union(z);

    if (!G.nodes.contains(any(n) for n in union_xyz)) {
        throw nx.NodeNotFound("one or more specified nodes not found in the graph");
    ...
    while (leaves.size() > 0) {
        leaf = leaves.popleft();
        if (!union_xyz.contains(leaf)) {
            for (auto p : G_copy.predecessors(leaf)) {
    ...
    for (auto component : nx.weakly_connected_components(G_copy)) {
...
// @not_implemented_for("undirected");
auto minimal_d_separator(G, u, v) -> void {
    ...
    */
    ...
    if (!G.nodes.contains(any(n) for n in union_uv)) {
        throw nx.NodeNotFound("one or more specified nodes not found in the graph");
...
// @not_implemented_for("undirected");
auto is_minimal_d_separator(G, u, v, z) -> void {
    ...
    */
    ...
    if (!xy_anc.contains(any(node) for node in z)) {
        return false;
    ...
    if (!marks.contains(any(node) for node in z)) {
        return false;
...
auto _bfs_with_marks(G, start_node, check_set) -> void {
