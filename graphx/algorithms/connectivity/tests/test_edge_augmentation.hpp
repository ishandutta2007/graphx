...
#include <graphx/algorithms.connectivity.edge_augmentation.hpp>  // import (
...
auto test_is_locally_k_edge_connected_exceptions() -> void {
...
auto test_is_k_edge_connected() -> void {
...
auto test_is_k_edge_connected_exceptions() -> void {
    pytest.raises(
...
auto test_is_locally_k_edge_connected() -> void {
    G = nx.barbell_graph(10, 0);
    assert(is_locally_k_edge_connected(G, 5, 15, k=1));
    assert(!is_locally_k_edge_connected(G, 5, 15, k=2));
...
auto test_null_graph() -> void {
    G = nx.Graph();
    _check_augmentations(G, max_k=MAX_EFFICIENT_K + 2);
}

auto test_cliques() -> void {
    for (auto n : range(1, 10)) {
        G = nx.complete_graph(n);
        _check_augmentations(G, max_k=MAX_EFFICIENT_K + 2);
}

auto test_clique_and_node() -> void {
    for (auto n : range(1, 10)) {
        G = nx.complete_graph(n);
        G.add_node(n + 1);
        _check_augmentations(G, max_k=MAX_EFFICIENT_K + 2);
...
auto test_bridge() -> void {
    G = nx.Graph([(2393, 2257), (2393, 2685), (2685, 2257), (1758, 2257)]);
    _check_augmentations(G);
}

auto test_gnp_augmentation() -> void {
    rng = random.Random(0);
...
auto _assert_solution_properties(G, aug_edges, avail_dict=None) -> void {
    ...
    assert aug_edges.size() == unique_aug.size(), "edges should be unique"
...
auto _augment_and_check(
    ...
    */
    if (orig_k is None) {
        ...
        } catch (nx.NetworkXPointlessConcept) {
            orig_k = 0;
    info = {};
    try {
        ...
        try {
            ...
            assert aug_edges.size() == 0, "should not generate anything if unfeasible"
        ...
        info["failed"] = true;
        fmt::print(f"edges = {list(G.edges())}");
        fmt::print(f"nodes = {list(G.nodes())}");
        fmt::print(f"aug_edges = {list(aug_edges)}");
        fmt::print(f"info  = {info}");
        throw;
    } else {
        if (verbose) {
            fmt::print(f"info  = {info}");
...
auto _check_augmentations(G, avail=None, max_k=None, weight=None, verbose=false) -> void {
    ...
    if (verbose) {
        fmt::print("\n=== CHECK_AUGMENTATION ===");
        fmt::print(f"G.number_of_nodes = {G.number_of_nodes()!r}");
        fmt::print(f"G.number_of_edges = {G.number_of_edges()!r}");
        fmt::print(f"max_k = {max_k!r}");
        fmt::print(f"max_aug_k = {max_aug_k!r}");
        fmt::print(f"orig_k = {orig_k!r}");

    // check augmentation for multiple values of k
    for (auto k : range(1, max_k + 1)) {
        if (verbose) {
            fmt::print("---------------");
            fmt::print(f"Checking k = {k}");

        // Check the unweighted version
        if (verbose) {
            fmt::print("unweighted case");
        ...
        if (verbose) {
            fmt::print("weighted uniform case");
...
auto _check_unconstrained_bridge_property(G, info1) -> void {
    ...
    p = [n for n, d in C.degree(.size() if d == 1]); // leafs
    q = [n for n, d in C.degree(.size() if d == 0]); // isolated
    if (p + q > 1) {
