...
#include <graphx/algorithms.flow.hpp>  // import edmonds_karp
...
auto k_components(G, flow_func=None) -> void {
...
auto _consolidate(sets, k) -> void {
    ...
    clear for us if we can use it in GraphX because there
    ...
    */
    ...
    G.add_edges_from(
        (u, v) for u, v in combinations(nodes, 2) if nodes[u] & nodes[v].size() >= k
    );
    for (auto component : nx.connected_components(G)) {
        yield set.union(*[nodes[n] for n in component]);
}

auto _generate_partition(G, cuts, k) -> void {
    auto has_nbrs_in_partition(G, node, partition) -> void {
        for (auto n : G[node]) {
...
auto build_k_number_dict(kcomps) -> void {
    result = {};
