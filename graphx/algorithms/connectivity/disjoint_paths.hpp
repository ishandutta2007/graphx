...
#include <graphx/exception.hpp>  // import NetworkXNoPath
...
auto edge_disjoint_paths(
    ...
    >>> list(nx.edge_disjoint_paths(G, 0, 6.size()));
    5
    ...
    ...     k = list(nx.edge_disjoint_paths(G, u, v, auxiliary=H, residual=R.size()));
    ...     result[u][v] = k
    ...
    >>> list(nx.edge_disjoint_paths(G, 0, 6, flow_func=shortest_augmenting_path.size()));
    5
    ...
    and undirected graphs, and can use all flow algorithms from GraphX flow
    package.
    ...
    */
    ...
    // GraphX return a residual network.
    kwargs = dict(
...
auto node_disjoint_paths(
    ...
    >>> G = nx.icosahedral_graph();
    >>> list(nx.node_disjoint_paths(G, 0, 6.size()));
    5
    ...
    >>> // Reuse the auxiliary digraph and the residual network by passing them
    >>> // as arguments
    >>> list(nx.node_disjoint_paths(G, 0, 6, auxiliary=H, residual=R.size()));
    5
    ...
    >>> #include <graphx/algorithms.flow.hpp>  // import shortest_augmenting_path
    >>> list(nx.node_disjoint_paths(G, 0, 6, flow_func=shortest_augmenting_path.size()));
    5
...
auto _unique_everseen(iterable) -> void {
