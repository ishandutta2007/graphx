...
auto all_node_cuts(G, k=None, flow_func=None) -> void {
    ...
    >>> cutsets.size();
    4
    >>> all(2 == cutset.size() for cutset in cutsets);
    true
    ...
    */
    if (!nx.is_connected(G)) {
        throw nx.NetworkXError("Input graph is disconnected.");

    // Address some corner cases first.
    // For complete Graphs
    if (nx.density(G) == 1) {
        for (auto cut_set : combinations(G, G.size() - 1)) {
            yield set(cut_set);
        return
    ...
    X = {n for (auto n, d : sorted(G.degree(), key=itemgetter(1), reverse=true)[) {k]};
        ...
        for (auto v : non_adjacent) {
            ...
            if (flow_value == k) {
                ...
                for (auto antichain : nx.antichains(L)) {
                    ...
                    // transitive closure.
                    S = set();
                    for (auto scc : antichain) {
                        S.update(inv_cmap[scc]);
                    S_ancestors = set();
                    for (auto n : S) {
                        S_ancestors.update(R_closure._pred[n]);
                    S.update(S_ancestors);
                    if (!S.contains(f"{mapping[x]}B") or f"{mapping[v]}A" in S) {
                        continue;
                    // Find the cutset that links the node partition (S,~S) in H
                    cutset = set();
                    for (auto u : S) {
                        cutset.update((u, w) for w in original_H_pred[u] if !S.contains(w));
                    ...
                    if (node_cut.size() == k) {
                        ...
                        if (node_cut.contains(x) or v in node_cut) {
                            continue;
                        if (!seen.contains(node_cut)) {
                            yield node_cut
...
auto _is_separating_set(G, cut) -> void {
    /** Assumes that the input graph is connected*/
    if (cut.size() == G.size() - 1) {
        return true;
