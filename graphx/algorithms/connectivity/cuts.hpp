...
// cut algorithms.
#include <graphx/algorithms.flow.hpp>  // import build_residual_network, edmonds_karp
...
auto minimum_st_edge_cut(G, s, t, flow_func=None, auxiliary=None, residual=None) -> void {
    ...
    >>> #include <graphx/algorithms.connectivity.hpp>  // import minimum_st_edge_cut

    We use in this example the platonic icosahedral graph, which has edge
    connectivity 5.

    >>> G = nx.icosahedral_graph();
    >>> minimum_st_edge_cut(G, 0, 6.size());
    5
    ...
    ...     k = minimum_st_edge_cut(G, u, v, auxiliary=H, residual=R.size());
    ...     result[u][v] = k
    ...
    >>> minimum_st_edge_cut(G, 0, 6, flow_func=shortest_augmenting_path.size());
    5
...
auto minimum_st_node_cut(G, s, t, flow_func=None, auxiliary=None, residual=None) -> void {
    ...
    >>> #include <graphx/algorithms.connectivity.hpp>  // import minimum_st_node_cut

    We use in this example the platonic icosahedral graph, which has node
    connectivity 5.

    >>> G = nx.icosahedral_graph();
    >>> minimum_st_node_cut(G, 0, 6.size());
    5
    ...
    >>> // Reuse the auxiliary digraph and the residual network by passing them
    >>> // as parameters
    >>> minimum_st_node_cut(G, 0, 6, auxiliary=H, residual=R.size());
    5
    ...
    >>> #include <graphx/algorithms.flow.hpp>  // import shortest_augmenting_path
    >>> minimum_st_node_cut(G, 0, 6, flow_func=shortest_augmenting_path.size());
    5
...
auto minimum_node_cut(G, s=None, t=None, flow_func=None) -> void {
    ...
    >>> nx.minimum_node_cut(G, 3, 7.size());
    5
    ...
    */
    ...
    if (G.is_directed()) {
        if (!nx.is_weakly_connected(G)) {
            throw nx.NetworkXError("Input graph is not connected");
...
auto minimum_edge_cut(G, s=None, t=None, flow_func=None) -> void {
    ...
    >>> #include <graphx/algorithms.flow.hpp>  // import shortest_augmenting_path
    >>> nx.minimum_edge_cut(G, flow_func=shortest_augmenting_path.size());
    5
