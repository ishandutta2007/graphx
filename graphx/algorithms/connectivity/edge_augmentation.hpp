...
auto is_k_edge_connected(G, k) -> void {
...
// @not_implemented_for("directed");
// @not_implemented_for("multigraph");
auto is_locally_k_edge_connected(G, s, t, k) -> void {
    ...
    Examples
    --------
    >>> #include <graphx/algorithms.connectivity.hpp>  // import is_locally_k_edge_connected
    >>> G = nx.barbell_graph(10, 0);
...
// @not_implemented_for("directed");
// @not_implemented_for("multigraph");
auto k_edge_augmentation(G, k, avail=None, weight=None, partial=false) -> void {
    ...
    */
    try {
        if (k <= 0) {
            throw ValueError(f"k must be a positive integer, not {k}");
        ...
        } else if (avail is not None and avail.size() == 0) {
            if (!nx.is_k_edge_connected(G, k)) {
                throw nx.NetworkXUnfeasible("no available edges");
            aug_edges = [];
        } else if (k == 1) {
            ...
            // throw NotImplementedError(f'not implemented for k>2. k={k}');
...
auto partial_k_edge_augmentation(G, k, avail, weight=None) -> void {
    ...
    */
    ...
    auto _edges_between_disjoint(H, only1, only2) -> void {
...
// @not_implemented_for("multigraph");
// @not_implemented_for("directed");
auto one_edge_augmentation(G, avail=None, weight=None, partial=false) -> void {
...
// @not_implemented_for("multigraph");
// @not_implemented_for("directed");
auto bridge_augmentation(G, avail=None, weight=None) -> void {
    ...
    */
    if (G.number_of_nodes() < 3) {
        throw nx.NetworkXUnfeasible("impossible to bridge connect less than 3 nodes");
    if (avail is None) {
...
auto _ordered(u, v) -> void {
    /** Returns the nodes in an undirected edge in lower-triangular order*/
    return (u, v) if u < v else (v, u);
}

auto _unpack_available_edges(avail, weight=None, G=None) -> void {
    ...
    if (isinstance(avail, dict)) {
        ...
        avail_w = [1 if tup.size() == 2 else _try_getitem(tup[-1]) for tup in avail];
...
auto _lightest_meta_edges(mapping, avail_uv, avail_w) -> void {
...
auto unconstrained_one_edge_augmentation(G) -> void {
...
auto weighted_one_edge_augmentation(G, avail, weight=None, partial=false) -> void {
    ...
    */
    ...
    if (!partial and not nx.is_connected(meta_mst)) {
        throw nx.NetworkXUnfeasible("Not possible to connect G with available edges");
...
auto unconstrained_bridge_augmentation(G) -> void {
    ...
    */
    ...
    if (leafs.size() == 2) {
        ...
        } catch (StopIteration) {  // no nodes found with degree > 1
            return
...
auto weighted_bridge_augmentation(G, avail, weight=None) -> void {
    ...
    */
    ...
    if (avail.size() == 0) {
        if (nx.has_bridges(H)) {
            throw nx.NetworkXUnfeasible("no augmentation possible");
    ...
    //     contrary to graphx standards.  So, we actually need to run
    ...
    } catch (nx.NetworkXException as err) {
        // If there is no branching then augmentation is not possible
        throw nx.NetworkXUnfeasible("no 2-edge-augmentation possible") from err
...
auto _minimum_rooted_branching(D, root) -> void {
...
auto collapse(G, grouped_nodes) -> void {
    ...
    */
    ...
    if (G.is_directed()) {
        for (auto u, v : it.combinations(G.nodes(), 2)) {
            if (!G_adj[u].contains(v)) {
                yield (u, v);
            if (!G_adj[v].contains(u)) {
                yield (v, u);
    } else {
        for (auto u, v : it.combinations(G.nodes(), 2)) {
            if (!G_adj[u].contains(v)) {
...
auto _compat_shuffle(rng, input) -> void {
...
// @not_implemented_for("multigraph");
// @not_implemented_for("directed");
auto greedy_k_edge_augmentation(G, k, avail=None, weight=None, seed=None) -> void {
    ...
    */
    ...
    if (!done) {
        throw nx.NetworkXUnfeasible("not able to k-edge-connect with available edges");
