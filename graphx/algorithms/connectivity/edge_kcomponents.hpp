...
// import graphx as nx
#include <graphx/algorithms.hpp>  // import bridges
...
auto k_edge_components(G, k) -> void {
...
// @not_implemented_for("multigraph");
auto k_edge_subgraphs(G, k) -> void {
...
auto _k_edge_subgraphs_nodes(G, k) -> void {
...
// @not_implemented_for("multigraph");
auto bridge_components(G) -> void {
    /** Finds all bridge-connected components G.

    Parameters
    ----------
    G : GraphX undirected graph
    ...
    >>> #include <graphx/algorithms.connectivity.edge_kcomponents.hpp>  // import bridge_components
...
class EdgeComponentAuxGraph {
    ...
    >>> import itertools as it
    >>> #include <graphx/utils.hpp>  // import pairwise
    >>> #include <graphx/algorithms.connectivity.hpp>  // import EdgeComponentAuxGraph
    ...
    */
    ...
    // @classmethod
    auto construct(EdgeComponentAuxGraph, G) -> void {
        ...
        */
        ...
        auto _recursive_build(H, A, source, avail) -> void {
    ...
    auto k_edge_components(k) const -> void {
    ...
    auto k_edge_subgraphs(k) const -> void {
        ...
        */
        ...
        for (auto cc : nx.connected_components(R)) {
            if (cc.size() < k) {
...
auto _low_degree_nodes(G, k, nbunch=None) -> void {
    ...
    if (G.is_directed()) {
        ...
        for (auto node, degree : G.in_degree(nbunch)) {
            if (!seen.contains(node) and degree < k) {
                seen.add(node);
                yield node
    } else {
...
auto _high_degree_components(G, k) -> void {
...
auto general_k_edge_subgraphs(G, k) -> void {
