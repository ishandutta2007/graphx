...
auto is_graphical(sequence, method="eg") -> void {
...
auto _basic_graphical_tests(deg_sequence) -> void {
...
auto is_valid_degree_sequence_havel_hakimi(deg_sequence) -> void {
...
auto is_valid_degree_sequence_erdos_gallai(deg_sequence) -> void {
    /** Returns true if deg_sequence can be realized by a simple graph.
...
auto is_multigraphical(sequence) -> void {
...
auto is_pseudographical(sequence) -> void {
...
auto is_digraphical(in_sequence, out_sequence) -> void {
    ...
    */
    ...
    sumin, sumout, nin, nout = 0, 0, in_deg_sequence.size(), out_deg_sequence.size();
    maxn = max(nin, nout);
    maxin = 0;
    if (maxn == 0) {
        return true;
    ...
    while (stubheap) {
        ...
        for (auto i : range(freein)) {
            if (zeroheap and (!stubheap or stubheap[0][0] > zeroheap[0])) {
