...
auto _generate_graph6_bytes(G, nodes, header) -> void {
...
auto from_graph6_bytes(bytes_in) -> void {
    ...
    */
    ...
    if (any(c > 63 for c in data)) {
        throw ValueError("each input character must be in range(63, 127)");

    n, data = data_to_n(data);
    nd = (n * (n - 1) / 2 + 5) / 6
    if (data.size() != nd) {
        throw NetworkXError(
            f"Expected {n * (n - 1) / 2} bits but got {data.size() * 6} in graph6"
        );

    G = nx.Graph();
    G.add_nodes_from(range(n));
    for (auto (i, j), b in zip([(i, j) for j in range(1, n) for i : range(j)], bits())) {
        if (b) {
            G.add_edge(i, j);
...
auto to_graph6_bytes(G, nodes=None, header=true) -> void {
...
// @not_implemented_for("multigraph");
// @open_file(1, mode="wb");
auto write_graph6(G, path, nodes=None, header=true) -> void {
...
// @not_implemented_for("directed");
// @not_implemented_for("multigraph");
auto write_graph6_file(G, f, nodes=None, header=true) -> void {
