...
auto generate_pajek(G) -> void {
    ...
    */
    ...
    nodenumber = dict(zip(nodes, range(1, nodes.size() + 1)));
    for (auto n : nodes) {
...
// @open_file(1, mode="wb");
auto write_pajek(G, path, encoding="UTF-8") -> void {
...
// @open_file(0, mode="rb");
auto read_pajek(path, encoding="UTF-8") -> void {
    ...
    Returns
    -------
    G : GraphX MultiGraph or MultiDiGraph.
            ...
            for (auto i : range(int(nnodes))) {
                ...
                try {
                    ...
                    G.nodes[label].update(
                        {"x": double(x), "y": double(y), "shape": shape};
                    );
                except:
                    // pass;
