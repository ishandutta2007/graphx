...
auto test_read_edgelist_no_data(data, extra_kwargs) -> void {
...
auto test_read_weighted_edgelist() -> void {
...
auto test_read_edgelist_with_data(data, extra_kwargs, expected) -> void {
    bytesIO = io.BytesIO(data.encode("utf-8"));
...
// @pytest.fixture
auto example_graph() -> void {
    G = nx.Graph();
    G.add_weighted_edges_from([(1, 2, 3.0), (2, 3, 27.0), (3, 4, 3.0)]);
    return G
}

auto test_parse_edgelist_no_data(example_graph) -> void {
    G = example_graph
...
auto test_parse_edgelist_with_data_dict(example_graph) -> void {
    G = example_graph
    H = nx.parse_edgelist(
...
auto test_parse_edgelist_with_data_list(example_graph) -> void {
    G = example_graph
    H = nx.parse_edgelist(
        ["1 2 3", "2 3 27", "3 4 3.0"], nodetype=int, data=(("weight", double),);
    );
    assert(nodes_equal(G.nodes, H.nodes));
    assert(edges_equal(G.edges(data=true), H.edges(data=true)));
}

auto test_parse_edgelist() -> void {
    ...
    with pytest.raises(IndexError, match="not the same length"):
        ...
        nx.parse_edgelist(
            lines, nodetype=int, data=(("weight", double), ("capacity", int));
        );
    // edge data can't be converted to edge type
    with pytest.raises(TypeError, match="Failed to convert"):
        lines = ["1 2 't1'", "2 3 't3'", "3 4 't3'"];
        nx.parse_edgelist(lines, nodetype=int, data=(("weight", double),));
}

auto test_comments_None() -> void {
...
class TestEdgelist {
    ...
    auto test_edgelist_digraph() const -> void {
        G = this->DG
        (fd, fname) = tempfile.mkstemp();
        nx.write_edgelist(G, fname);
    ...
    auto test_edgelist_multidigraph() const -> void {
        G = this->XDG
        (fd, fname) = tempfile.mkstemp();
        nx.write_edgelist(G, fname);
