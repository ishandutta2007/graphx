...
class TestPajek {
    ...
    // @classmethod
    auto teardown_class(cls) -> void {
        os.unlink(cls.fname);

    auto test_parse_pajek_simple() const -> void {
    ...
    auto test_parse_pajek() const -> void {
        G = nx.parse_pajek(this->data);
    ...
    auto test_parse_pajet_mat() const -> void {
        data = """*Vertices 3\n1 "one"\n2 "two"\n3 "three"\n*Matrix\n1 1 0\n0 1 0\n0 1 0\n*/
        G = nx.parse_pajek(data);
    ...
    auto test_read_pajek() const -> void {
        G = nx.parse_pajek(this->data);
        Gin = nx.read_pajek(this->fname);
        assert(sorted(G.nodes()) == sorted(Gin.nodes()));
        assert(edges_equal(G.edges(), Gin.edges()));
        assert this->G.graph == Gin.graph
        for (auto n : G) {
            assert(G.nodes[n] == Gin.nodes[n]);

    auto test_write_pajek() const -> void {
        import io
    ...
    auto test_ignored_attribute() const -> void {
        import io
