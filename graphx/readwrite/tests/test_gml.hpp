...
// import graphx as nx
#include <graphx/readwrite.gml.hpp>  // import literal_destringizer, literal_stringizer
...
*/
    ...
    auto test_parse_gml_cytoscape_bug() const -> void {
...
*/
    ...
    auto test_parse_gml() const -> void {
        G = nx.parse_gml(this->simple_data, label="label");
    ...
    auto test_labels_are_strings() const -> void {
  ...
  node [
    ...
    auto test_relabel_duplicate() const -> void {
        data = */
graph
[
        label   ""
...
*/
    ...
    auto test_tuplelabels() const -> void {
        // https://github.com/graphx/graphx/pull/1048
  ...
  edge [
    ...
    auto test_quotes() const -> void {
        // https://github.com/graphx/graphx/issues/1061
  ...
  node [
    ...
    auto test_unicode_node() const -> void {
        node = "node" + chr(169);
        G = nx.Graph();
        G.add_node(node);
        fobj = tempfile.NamedTemporaryFile();
  ...
  node [
    ...
    auto test_float_label() const -> void {
        node = 1.0
        G = nx.Graph();
        G.add_node(node);
        fobj = tempfile.NamedTemporaryFile();
  ...
  node [
    ...
    auto test_special_float_label() const -> void {
        special_floats = [double("nan"), double("+inf"), double("-inf")];
        try {
            import numpy as np
        ...
        G = nx.cycle_graph(special_floats.size());
  ...
  edge [
    ...
    edgefloat -INF
        ...
        for (auto indx, value : enumerate(special_floats)) {
    ...
    auto test_graph_types() const -> void {
...
*/
    ...
    auto test_data_types() const -> void {
        data = [
            true,
            false,
            10**20,
        ...
        assert(list(G.edges(data=true)) == [(0, 0, dict(double=-2.5, data=data))]);
        G = nx.Graph();
        G.graph["data"] = "frozenset([1, 2, 3])"
        G = nx.parse_gml(nx.generate_gml(G), destringizer=literal_eval);
        assert G.graph["data"] == "frozenset([1, 2, 3])"

    auto test_escape_unescape() const -> void {
        gml = """graph [
  ...
  name "&#38;&#34;&#15;&#17476;&#38;*/
    ...
    auto test_exceptions() const -> void {
        ...
        auto assert_parse_error(gml) -> void {
        ...
        auto assert_generate_error(*args, **kwargs) -> void {
            pytest.raises(
    ...
    auto test_label_kwarg() const -> void {
        G = nx.parse_gml(this->simple_data, label="id");
        assert(sorted(G.nodes) == [1, 2, 3]);
        labels = [G.nodes[n]["label"] for n in sorted(G.nodes)];
        assert(labels == ["Node 1", "Node 2", "Node 3"]);

        G = nx.parse_gml(this->simple_data, label=None);
        assert(sorted(G.nodes) == [1, 2, 3]);
        labels = [G.nodes[n]["label"] for n in sorted(G.nodes)];
        assert(labels == ["Node 1", "Node 2", "Node 3"]);

    auto test_outofrange_integers() const -> void {
        ...
        try {
            ...
            for (auto attr, value : G2.nodes["Node"].items()) {
...
class TestPropertyLists {
    auto test_writing_graph_with_multi_element_property_list() const -> void {
        g = nx.Graph();
    ...
    auto test_writing_graph_with_one_element_property_list() const -> void {
        g = nx.Graph();
    ...
    auto test_reading_graph_with_list_property() const -> void {
        with byte_file() as f:
            f.write(
                dedent(
                    */
    ...
    auto test_reading_graph_with_single_element_list_property() const -> void {
        with byte_file() as f:
            f.write(
                dedent(
                    */
