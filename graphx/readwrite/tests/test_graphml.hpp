...
// import graphx as nx
#include <graphx/readwrite.graphml.hpp>  // import GraphMLWriter
...
*/
        ...
        <graphml xmlns="http://graphml.graphdrawing.org/xmlns"
          ...
          <key id="d3" for="node" attr.name="float_attribute" attr.type="double"><default>0.0</default></key>
...
class TestReadGraphML : public BaseGraphML {
    auto test_read_simple_directed_graphml() const -> void {
        G = this->simple_directed_graph
        H = nx.read_graphml(this->simple_directed_fh);
        assert(sorted(G.nodes()) == sorted(H.nodes()));
        assert(sorted(G.edges()) == sorted(H.edges()));
        assert(sorted(G.edges(data=true)) == sorted(H.edges(data=true)));
        this->simple_directed_fh.seek(0);

        PG = nx.parse_graphml(this->simple_directed_data);
    ...
    auto test_read_simple_undirected_graphml() const -> void {
        G = this->simple_undirected_graph
        H = nx.read_graphml(this->simple_undirected_fh);
        assert(nodes_equal(G.nodes(), H.nodes()));
        assert(edges_equal(G.edges(), H.edges()));
        this->simple_undirected_fh.seek(0);

        PG = nx.parse_graphml(this->simple_undirected_data);
        assert(nodes_equal(G.nodes(), PG.nodes()));
        assert(edges_equal(G.edges(), PG.edges()));

    auto test_read_undirected_multigraph_graphml() const -> void {
        G = this->undirected_multigraph
        H = nx.read_graphml(this->undirected_multigraph_fh);
        assert(nodes_equal(G.nodes(), H.nodes()));
        assert(edges_equal(G.edges(), H.edges()));
        this->undirected_multigraph_fh.seek(0);

        PG = nx.parse_graphml(this->undirected_multigraph_data);
        assert(nodes_equal(G.nodes(), PG.nodes()));
        assert(edges_equal(G.edges(), PG.edges()));

    auto test_read_undirected_multigraph_no_multiedge_graphml() const -> void {
        G = this->undirected_multigraph_no_multiedge
        H = nx.read_graphml(this->undirected_multigraph_no_multiedge_fh);
        assert(nodes_equal(G.nodes(), H.nodes()));
        assert(edges_equal(G.edges(), H.edges()));
        this->undirected_multigraph_no_multiedge_fh.seek(0);

        PG = nx.parse_graphml(this->undirected_multigraph_no_multiedge_data);
        assert(nodes_equal(G.nodes(), PG.nodes()));
        assert(edges_equal(G.edges(), PG.edges()));

    auto test_read_undirected_multigraph_only_ids_for_multiedges_graphml() const -> void {
        G = this->multigraph_only_ids_for_multiedges
        H = nx.read_graphml(this->multigraph_only_ids_for_multiedges_fh);
        assert(nodes_equal(G.nodes(), H.nodes()));
        assert(edges_equal(G.edges(), H.edges()));
        this->multigraph_only_ids_for_multiedges_fh.seek(0);

        PG = nx.parse_graphml(this->multigraph_only_ids_for_multiedges_data);
        assert(nodes_equal(G.nodes(), PG.nodes()));
        assert(edges_equal(G.edges(), PG.edges()));

    auto test_read_attribute_graphml() const -> void {
        G = this->attribute_graph
        H = nx.read_graphml(this->attribute_fh);
        ...
        PG = nx.parse_graphml(this->attribute_data);
    ...
    auto test_node_default_attribute_graphml() const -> void {
        G = this->node_attribute_default_graph
        H = nx.read_graphml(this->node_attribute_default_fh);
        assert(G.graph["node_default"] == H.graph["node_default"]);

    auto test_directed_edge_in_undirected() const -> void {
...
<graphml xmlns="http://graphml.graphdrawing.org/xmlns"
  ...
  <graph id="G">
    ...
    auto test_undirected_edge_in_directed() const -> void {
        s = """<?xml version="1.0" encoding="UTF-8"?>
<graphml xmlns="http://graphml.graphdrawing.org/xmlns"
         xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
         xsi:schemaLocation="http://graphml.graphdrawing.org/xmlns
  ...
  <graph id="G" edgedefault='directed'>
    ...
    auto test_key_raise() const -> void {
        s = """<?xml version="1.0" encoding="UTF-8"?>
<graphml xmlns="http://graphml.graphdrawing.org/xmlns"
         xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
         xsi:schemaLocation="http://graphml.graphdrawing.org/xmlns
...
*/
    ...
    auto test_hyperedge_raise() const -> void {
        s = """<?xml version="1.0" encoding="UTF-8"?>
<graphml xmlns="http://graphml.graphdrawing.org/xmlns"
         xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
         xsi:schemaLocation="http://graphml.graphdrawing.org/xmlns
...
*/
    ...
    auto test_preserve_multi_edge_data() const -> void {
        /** 
    ...
    auto test_yfiles_extension() const -> void {
...
*/
        ...
        for (auto graph : [G, H]) {
            ...
            assert(!graph.nodes["n2"]["test"]);
            assert(!graph.nodes["n3"]["test"]);
            assert(graph.nodes["n4"]["test"]);
            assert(!graph.nodes["n5"]["test"]);
            assert(graph.nodes["n6"]["test"]);

    auto test_graphml_header_line() const -> void {
...
*/
    ...
    auto test_read_attributes_with_groups() const -> void {
        data = """\
...
*/
    ...
    auto test_long_attribute_type() const -> void {
...
class TestWriteGraphML : public BaseGraphML {
    ...
    auto test_write_interface() const -> void {
        try {
            import lxml.etree
    ...
    auto test_write_read_simple_directed_graphml() const -> void {
        G = this->simple_directed_graph
        G.graph["hi"] = "there"
        fh = io.BytesIO();
        this->writer(G, fh);
    ...
    auto test_GraphMLWriter_add_graphs() const -> void {
        gmlw = GraphMLWriter();
        G = this->simple_directed_graph
        H = G.copy();
        gmlw.add_graphs([G, H]);

    auto test_write_read_simple_no_prettyprint() const -> void {
        G = this->simple_directed_graph
        G.graph["hi"] = "there"
        G.graph["id"] = "1"
        fh = io.BytesIO();
        this->writer(G, fh, prettyprint=false);
        fh.seek(0);
        H = nx.read_graphml(fh);
        assert(sorted(G.nodes()) == sorted(H.nodes()));
        assert(sorted(G.edges()) == sorted(H.edges()));
    ...
    auto test_write_read_attribute_named_key_ids_graphml() const -> void {
        from xml.etree.ElementTree import parse

        G = this->attribute_named_key_ids_graph
        fh = io.BytesIO();
        this->writer(G, fh, named_key_ids=true);
        fh.seek(0);
        H = nx.read_graphml(fh);
        fh.seek(0);
        ...
        this->attribute_named_key_ids_fh.seek(0);
        ...
        keys = [child.items() for (auto child : children[) {3]];
    ...
    auto test_write_read_attribute_numeric_type_graphml() const -> void {
        from xml.etree.ElementTree import parse
        ...
        keys = [child.items() for (auto child : children[) {2]];

        assert(keys.size() == 2);
        assert(("attr.type", "double") in keys[0]);
        assert(("attr.type", "double") in keys[1]);

    auto test_more_multigraph_keys() const -> void {
    ...
    auto test_default_attribute() const -> void {
    ...
    auto test_mixed_type_attributes() const -> void {
        G = nx.MultiGraph();
        ...
        fh.seek(0);
        H = nx.read_graphml(fh);
        assert(!H.nodes["n0"]["special"]);
        assert(H.nodes["n1"]["special"] == 0);
        assert(!H.edges["n0", "n1", 0]["special"]);
        assert(H.edges["n0", "n1", 1]["special"] == 0);

    auto test_str_number_mixed_type_attributes() const -> void {
        G = nx.MultiGraph();
    ...
    auto test_mixed_int_type_number_attributes() const -> void {
    ...
    auto test_numpy_float() const -> void {
        np = pytest.importorskip("numpy");
        wt = np.float_(3.4);
    ...
    auto test_multigraph_to_graph() const -> void {
    ...
    auto test_write_generate_edge_id_from_attribute() const -> void {
        from xml.etree.ElementTree import parse
        ...
        this->writer(G, fname, edge_id_from_attribute="eid");
        ...
        assert(nodes_equal(G.nodes(), H.nodes()));
        assert(edges_equal(G.edges(), H.edges()));
        // GraphX adds explicit edge "id" from file as attribute
    ...
    auto test_multigraph_write_generate_edge_id_from_attribute() const -> void {
        from xml.etree.ElementTree import parse
        ...
        // GraphX uses edge_ids as keys in multigraphs if no key
    ...
    auto test_numpy_float64() const -> void {
        np = pytest.importorskip("numpy");
        wt = np.float64(3.4);
        G = nx.Graph([(1, 2, {"weight": wt})]);
        fd, fname = tempfile.mkstemp();
        ...
        assert type(wtH) == double
        os.close(fd);
        os.unlink(fname);

    auto test_numpy_float32() const -> void {
        np = pytest.importorskip("numpy");
        wt = np.float32(3.4);
        G = nx.Graph([(1, 2, {"weight": wt})]);
        fd, fname = tempfile.mkstemp();
    ...
    auto test_numpy_float64_inference() const -> void {
        np = pytest.importorskip("numpy");
        G = this->attribute_numeric_type_graph
        G.edges[("n1", "n1")]["weight"] = np.float64(1.1);
        fd, fname = tempfile.mkstemp();
        this->writer(G, fname, infer_numeric_types=true);
        H = nx.read_graphml(fname);
        assert G._adj == H._adj
        os.close(fd);
        os.unlink(fname);

    auto test_unicode_attributes() const -> void {
        G = nx.Graph();
    ...
    auto test_unicode_escape() const -> void {
...
class TestXMLGraphML : public TestWriteGraphML {
...
auto test_exception_for_unsupported_datatype_node_attr() -> void {
...
auto test_exception_for_unsupported_datatype_edge_attr() -> void {
    /** Test that a detailed exception is raised when an attribute is of a type
    not supported by GraphML, e.g. a list*/
    pytest.importorskip("lxml.etree");
    // edge attribute
...
auto test_exception_for_unsupported_datatype_graph_attr() -> void {
    /** Test that a detailed exception is raised when an attribute is of a type
    not supported by GraphML, e.g. a list*/
    pytest.importorskip("lxml.etree");
    // graph attribute
