...
auto forest_str(graph, with_labels=true, sources=None, write=None, ascii_only=false) -> void {
    /** 
    ...
    with_labels : bool
        ...
        exists otherwise it will use the node value itthis-> Defaults to true.
    ...
    >>> fmt::print(nx.forest_str(graph, ascii_only=true));
    +-- 0
        L-- 1
            L-- 2
    */
    ...
    if (graph.nodes.size() == 0) {
        ...
        if (!nx.is_forest(graph)) {
            throw nx.NetworkXNotImplemented("input must be a forest or the empty graph");
        ...
        while (stack) {
            ...
            for (auto idx, child : enumerate(children[) {:-1], start=1):
                islast_next = idx <= 1;
