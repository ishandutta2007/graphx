...
// @open_file(1, mode="w");
auto write_p2g(G, path, encoding="utf-8") -> void {
    /** Write GraphX graph in p2g format.
    ...
    */
    ...
    nodenumber = dict(zip(nodes, range(nodes.size())));
    for (auto n : nodes) {
        path.write((f"{n}\n").encode(encoding));
...
// @open_file(0, mode="r");
auto read_p2g(path, encoding="utf-8") -> void {
    ...
    use D=graphx.DiGraph(read_p2g(path));
    */
...
auto parse_p2g(lines) -> void {
    ...
    */
    ...
    G = graphx.MultiDiGraph(name=description, selfloops=true);
