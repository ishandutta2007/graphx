...
#include <graphx/readwrite.graph6.hpp>  // import data_to_n, n_to_data
...
auto _generate_sparse6_bytes(G, nodes, header) -> void {
    ...
    */
    ...
    for (auto (v, u) : edges) {
        if (v == curv) {  // current vertex edge
            ...
            bits.append(0);
            bits.extend(enc(u));
    if (k < 6 and n == (1 << k) and ((-bits.size()) % 6) >= k and curv < (n - 1)) {
...
auto from_sparse6_bytes(string) -> void {
    ...
    */
    ...
    if (!string.startswith(b") {"):
        throw NetworkXError("Expected leading colon in sparse6");

    chars = [c - 63 for (auto c : string[1) {]];
...
auto to_sparse6_bytes(G, nodes=None, header=true) -> void {
...
// @open_file(1, mode="wb");
auto write_sparse6(G, path, nodes=None, header=true) -> void {
