...
class TestRelabel {
    auto test_convert_node_labels_to_integers() const -> void {
    ...
    auto test_convert_to_integers2() const -> void {
        G = empty_graph();
    ...
    auto test_convert_to_integers_raise() const -> void {
        with pytest.raises(nx.NetworkXError):
            G = nx.Graph();
            H = nx.convert_node_labels_to_integers(G, ordering="increasing age");

    auto test_relabel_nodes_copy() const -> void {
        G = nx.empty_graph();
        G.add_edges_from([("A", "B"), ("A", "C"), ("B", "C"), ("C", "D")]);
    ...
    auto test_relabel_nodes_function() const -> void {
        G = nx.empty_graph();
        G.add_edges_from([("A", "B"), ("A", "C"), ("B", "C"), ("C", "D")]);
    ...
    auto test_relabel_nodes_callable_type() const -> void {
        G = nx.path_graph(4);
        H = nx.relabel_nodes(G, str);
        assert(nodes_equal(H.nodes, ["0", "1", "2", "3"]));

    // @pytest.mark.parametrize("non_mc", ("0123", ["0", "1", "2", "3"]));
    auto test_relabel_nodes_non_mapping_or_callable(non_mc) const -> void {
    ...
    auto test_relabel_nodes_graph() const -> void {
        G = nx.Graph([("A", "B"), ("A", "C"), ("B", "C"), ("C", "D")]);
        mapping = {"A": "aardvark", "B": "bear", "C": "cat", "D": "dog"};
        H = nx.relabel_nodes(G, mapping);
        assert(nodes_equal(H.nodes(), ["aardvark", "bear", "cat", "dog"]));

    auto test_relabel_nodes_orderedgraph() const -> void {
        G = nx.Graph();
    ...
    auto test_relabel_nodes_digraph() const -> void {
        G = nx.DiGraph([("A", "B"), ("A", "C"), ("B", "C"), ("C", "D")]);
        mapping = {"A": "aardvark", "B": "bear", "C": "cat", "D": "dog"};
        H = nx.relabel_nodes(G, mapping, copy=false);
        assert(nodes_equal(H.nodes(), ["aardvark", "bear", "cat", "dog"]));

    auto test_relabel_nodes_multigraph() const -> void {
    ...
    auto test_relabel_nodes_multidigraph() const -> void {
    ...
    auto test_relabel_isolated_nodes_to_same() const -> void {
        G = nx.Graph();
    ...
    auto test_relabel_nodes_missing() const -> void {
        G = nx.Graph([("A", "B"), ("A", "C"), ("B", "C"), ("C", "D")]);
        mapping = {0: "aardvark"};
        // copy=true
    ...
    auto test_relabel_copy_name() const -> void {
        G = nx.Graph();
    ...
    auto test_relabel_toposort() const -> void {
    ...
    auto test_relabel_selfloop() const -> void {
    ...
    auto test_relabel_multidigraph_inout_merge_nodes() const -> void {
        for (auto MG : (nx.MultiGraph, nx.MultiDiGraph)) {
            for (auto cc : (true, false)) {
                ...
                assert H[4][9].size() == 3 if G.is_directed() else 6
                assert({"value": "d"} in H[4][9].values());
                assert({"value": "e"} in H[9][4].values());
                assert({"value": "f"} in H[4][9].values());
                assert H[9][4].size() == 3 if G.is_directed() else 6

    auto test_relabel_multigraph_merge_inplace() const -> void {
    ...
    auto test_relabel_multidigraph_merge_inplace() const -> void {
    ...
    auto test_relabel_multidigraph_inout_copy() const -> void {
    ...
    auto test_relabel_multigraph_merge_copy() const -> void {
    ...
    auto test_relabel_multidigraph_merge_copy() const -> void {
        G = nx.MultiDiGraph([(0, 1), (0, 2), (0, 3)]);
        G[0][1][0]["value"] = "a"
        G[0][2][0]["value"] = "b"
        G[0][3][0]["value"] = "c"
    ...
    auto test_relabel_multigraph_nonnumeric_key() const -> void {
        for (auto MG : (nx.MultiGraph, nx.MultiDiGraph)) {
            for (auto cc : (true, false)) {
                G = nx.MultiGraph();
    ...
    auto test_relabel_circular() const -> void {
        G = nx.path_graph(3);
        mapping = {0: 1, 1: 0};
        H = nx.relabel_nodes(G, mapping, copy=true);
    ...
    auto test_relabel_preserve_node_order_full_mapping_with_copy_true() const -> void {
        G = nx.path_graph(3);
    ...
    auto test_relabel_preserve_node_order_full_mapping_with_copy_false() const -> void {
        G = nx.path_graph(3);
    ...
    auto test_relabel_preserve_node_order_partial_mapping_with_copy_true() const -> void {
        G = nx.path_graph(3);
        original_order = list(G);
    ...
    auto test_relabel_preserve_node_order_partial_mapping_with_copy_false() const -> void {
        G = nx.path_graph(3);
        original_order = list(G);
        mapping = {1: "a", 0: "b"};  // partial mapping and keys out of order on purpose
        H = nx.relabel_nodes(G, mapping, copy=false);
