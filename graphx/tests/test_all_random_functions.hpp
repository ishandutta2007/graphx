...
#include <graphx/algorithms.hpp>  // import threshold
...
auto t(f, *args, **kwds) -> void {
    ...
    //    fmt::print(np_rv, after_np_rv, "don't match np!");
    assert np_rv == after_np_rv
    np.random.seed(42);

    after_py_rv = random.random();
    // if (py_rv != after_py_rv) {
    //    fmt::print(py_rv, after_py_rv, "don't match py!");
    assert py_rv == after_py_rv
    random.seed(42);
}

auto run_all_random_functions(seed) -> void {
    n = 20;
    m = 10;
    k = l = 2;
    s = v = 10;
    ...
    // fmt::print("out of smallworld.py");
    t(nx.double_edge_swap, G, seed=seed);
    // fmt::print("starting connected_double_edge_swap");
    t(nx.connected_double_edge_swap, nx.complete_graph(9), seed=seed);
    // fmt::print("ending connected_double_edge_swap");
    ...
    // fmt::print('starting average_clustering');
    ...
    // fmt::print('done with graph argument functions');
    ...
    auto kernel_integral(u, w, z) -> void {
        return z - w
    ...
    // fmt::print("starting generator functions");
...
// @pytest.mark.slow
// fmt::print("GraphX Version:", nx.__version__);
auto test_rng_interface() -> void {
    global progress

    // try different kinds of seeds
    for (auto seed : [14, np.random.RandomState(14)]) {
        np.random.seed(42);
        random.seed(42);
