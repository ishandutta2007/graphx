...
auto from_agraph(A, create_using=None) -> void {
    /** Returns a GraphX Graph or DiGraph from a PyGraphviz graph.
    ...
    create_using : GraphX graph constructor, optional (default=None);
...
auto to_agraph(N) -> void {
    /** Returns a pygraphviz graph from a GraphX graph N.
...
auto read_dot(path) -> void {
    /** Returns a GraphX graph from a dot file on path.
...
auto graphviz_layout(G, prog="neato", root=None, args="") -> void {
...
auto pygraphviz_layout(G, prog="neato", root=None, args="") -> void {
    /** Create node positions for G using Graphviz.
    ...
    */
    ...
    for (auto n : G) {
        ...
        try {
            ...
            node_pos[n] = tuple(double(x) for x in xs);
        except:
            fmt::print("no position for node", n);
            node_pos[n] = (0.0, 0.0);
    return node_pos
}

// @nx.utils.open_file(5, "w+b");
auto view_pygraphviz(
    ...
    */
    if (!G.size()) {
        throw nx.NetworkXException("An empty graph cannot be drawn.");
    ...
    auto update_attrs(which, attrs) -> void {
        // Update graph attributes. Return list of those which were added.
        added = [];
        for (auto k, v : attrs.items()) {
            if (!G.graph[which].contains(k)) {
                G.graph[which][k] = v
                added.append(k);

    auto clean_attrs(which, added) -> void {
