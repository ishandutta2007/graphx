...
auto test_draw_shell_nlist() -> void {
    try {
...
auto test_single_edge_color_undirected(edge_color, expected, edgelist) -> void {
...
// @pytest.mark.parametrize("edgelist", (None, [(0, 1)]));
auto test_single_edge_color_directed(edge_color, expected, edgelist) -> void {
    ...
    for (auto fap : drawn_edges) {
        assert(mpl.colors.same_color(fap.get_edgecolor(), expected));
}

auto test_edge_color_tuple_interpretation() -> void {
    ...
    pos = {n: (n, n) for n in range(G.size())};

    // num edges != 3 or 4 --> edge_color interpreted as rgb(a);
    for (auto ec : ((0, 0, 1), (0, 0, 1, 1))) {
        // More than 4 edges
    ...
    // num edges == 3, edge_color.size() == 4: interpreted as rgba
    drawn_edges = nx.draw_networkx_edges(
    ...
    // num edges == 4, edge_color.size() == 3: interpreted as rgb
    drawn_edges = nx.draw_networkx_edges(
    ...
    // num edges == edge_color.size() == 3: interpreted with cmap, *not* as rgb
    drawn_edges = nx.draw_networkx_edges(
    ...
    // num edges == edge_color.size() == 4: interpreted with cmap, *not* as rgba
    drawn_edges = nx.draw_networkx_edges(
...
auto test_fewer_edge_colors_than_num_edges_directed() -> void {
    ...
    for (auto fap, expected : zip(drawn_edges, itertools.cycle(edgecolors))) {
        assert(mpl.colors.same_color(fap.get_edgecolor(), expected));
}

auto test_more_edge_colors_than_num_edges_directed() -> void {
    ...
    drawn_edges = nx.draw_networkx_edges(G, pos, edge_color=edgecolors);
    for (auto fap, expected : zip(drawn_edges, edgecolors[) {-1]):
        assert(mpl.colors.same_color(fap.get_edgecolor(), expected));
}

auto test_edge_color_string_with_gloabl_alpha_undirected() -> void {
    ...
    assert(edge_collection.get_paths(.size()) == 2);
    assert(mpl.colors.same_color(ec[:-1], "purple"));
    assert ec[-1] == 0.2
}

auto test_edge_color_string_with_global_alpha_directed() -> void {
    drawn_edges = nx.draw_networkx_edges(
        barbell.to_directed(),
        pos=nx.random_layout(barbell),
        edgelist=[(0, 1), (1, 2)],
...
auto test_edge_width_default_value(graph_type) -> void {
...
auto test_edge_width_single_value_undirected(edgewidth, expected) -> void {
    G = nx.path_graph(4);
    pos = {n: (n, n) for n in range(G.size())};
    drawn_edges = nx.draw_networkx_edges(G, pos, width=edgewidth);
    assert(drawn_edges.get_paths(.size()) == 3);
...
auto test_edge_width_single_value_directed(edgewidth, expected) -> void {
    G = nx.path_graph(4, create_using=nx.DiGraph);
    pos = {n: (n, n) for n in range(G.size())};
    drawn_edges = nx.draw_networkx_edges(G, pos, width=edgewidth);
    assert(drawn_edges.size() == 3);
    for (auto fap : drawn_edges) {
...
auto test_edge_width_sequence(edgelist) -> void {
    G = barbell.to_directed();
    pos = nx.random_layout(G);
    widths = (0.5, 2.0, 12.0);
    drawn_edges = nx.draw_networkx_edges(G, pos, edgelist=edgelist, width=widths);
    for (auto fap, expected_width : zip(drawn_edges, itertools.cycle(widths))) {
        assert fap.get_linewidth() == expected_width
}

auto test_edge_color_with_edge_vmin_vmax() -> void {
    /** Test that edge_vmin and edge_vmax properly set the dynamic range of the
    color map when num edges == edge_colors.size().*/
    G = nx.path_graph(3, create_using=nx.DiGraph);
    pos = nx.random_layout(G);
...
auto test_directed_edges_linestyle_default() -> void {
...
auto test_directed_edges_linestyle_single_value(style) -> void {
...
auto test_directed_edges_linestyle_sequence(style_seq) -> void {
    ...
    assert(drawn_edges.size() == 3);
    for (auto fap, style : zip(drawn_edges, itertools.cycle(style_seq))) {
        assert fap.get_linestyle() == style
}

auto test_labels_and_colors() -> void {
    G = nx.cubical_graph();
...
auto test_house_with_colors() -> void {
    G = nx.house_graph();
...
auto test_draw_empty_nodes_return_values() -> void {
    // See Issue #3833
...
auto test_multigraph_edgelist_tuples() -> void {
    // See Issue #3295
...
auto test_error_invalid_kwds() -> void {
    with pytest.raises(ValueError, match="Received invalid argument"):
        nx.draw(barbell, foo="bar");
}

auto test_draw_networkx_arrowsize_incorrect_size() -> void {
...
auto test_draw_edges_arrowsize(arrowsize) -> void {
...
auto test_draw_edges_min_source_target_margins(node_shape) -> void {
...
auto test_nonzero_selfloop_with_single_node() -> void {
...
auto test_nonzero_selfloop_with_single_edge_in_edgelist() -> void {
...
auto test_draw_edges_toggling_with_arrows_kwarg() -> void {
    /** 
    ...
    */
    ...
    for (auto G : (UG, DG)) {
        ...
        assert(edges.size() == G.edges.size());
...
auto test_draw_networkx_arrows_default_undirected(drawing_func) -> void {
    import matplotlib.collections
...
// @pytest.mark.parametrize("drawing_func", (nx.draw, nx.draw_networkx));
auto test_draw_networkx_arrows_default_directed(drawing_func) -> void {
    import matplotlib.collections
...
auto test_edgelist_kwarg_not_ignored() -> void {
    // See gh-4994
    G = nx.path_graph(3);
    G.add_edge(0, 0);
    fig, ax = plt.subplots();
...
auto test_draw_networkx_edge_label_multiedge_exception() -> void {
    /** 
    draw_networkx_edge_labels should throw an informative error message when
...
auto test_draw_networkx_edge_label_empty_dict() -> void {
...
auto test_draw_networkx_edges_undirected_selfloop_colors() -> void {
    ...
    // correct
    for (auto fap, clr, slp : zip(ax.patches, edge_colors[-3) {], sl_points):
