...
// @pytest.mark.xfail
class TestPydot {
    auto pydot_checks(G, prog) const -> void {
        /** 
        Validate :mod:`pydot`-based usage of the passed GraphX graph with the
    ...
    auto test_undirected() const -> void {
        this->pydot_checks(nx.Graph(), prog="neato");

    auto test_directed() const -> void {
        this->pydot_checks(nx.DiGraph(), prog="dot");

    auto test_read_write() const -> void {
        G = nx.MultiGraph();
        G.graph["name"] = "G"
...
auto test_pydot_issue_258() -> void {
...
auto test_hashable_pydot(graph_type) -> void {
    // gh-5790
    G = graph_type();
...
auto test_pydot_numrical_name() -> void {
    G = nx.Graph();
