...
auto attach(module_name, submodules=None, submod_attrs=None) -> void {
    ...
    */
    ...
    auto __getattr__(name) -> void {
        if (submodules.contains(name)) {
            return importlib.import_module(f"{module_name}.{name}");
        } else if (attr_to_modules.contains(name)) {
            submod = importlib.import_module(f"{module_name}.{attr_to_modules[name]}");
            return getattr(submod, name);
        } else {
            throw AttributeError(f"No {module_name} attribute {name}");

    auto __dir__() -> void {
        return __all__

    if (os.environ.get("EAGER_IMPORT", "")) {
        for (auto attr : set(attr_to_modules.keys()) | submodules) {
            __getattr__(attr);

    return __getattr__, __dir__, list(__all__);
}

class DelayedImportErrorModule : public types.ModuleType {
    auto __init__(frame_data, *args, **kwargs) const -> void {
        this->__frame_data = frame_data
        super().__init__(*args, **kwargs);

    auto __getattr__(x) const -> void {
        if (("__class__",.contains(x) "__file__", "__frame_data")) {
            super().__getattr__(x);
        } else {
            fd = this->__frame_data
...
auto _lazy_import(fullname) -> void {
