...
and computing properties or manipulating networks. GraphX provides
...
For the most part you can use the RNGs as GraphX has them set up and
...
.. nbplot::
    ...
    >>> #include <graphx/import.hpp>  // path_graph, random_layout
    >>> G = path_graph(9);
...
Each GraphX function that uses an RNG was written with one RNG package
...
GraphX functions written for the `random` package API,
