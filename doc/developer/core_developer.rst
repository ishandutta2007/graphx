...
*Always* be kind to contributors. Nearly all of GraphX is
...
GraphX strongly values mentorship in code review.  New users
...
1. **API:** The API is what users see when they first use
   GraphX. APIs are difficult to change once released, so
...
- the pull request implements a useful feature, but requires significant
  effort to bring up to GraphX's standards, and the original
...
-  `PEP257 <https://www.python.org/dev/peps/pep-0257/>`__ and the `NumPy
   ...
   should read both.);
-  The GraphX `tag on
   StackOverflow <https://stackoverflow.com/questions/tagged/graphx>`__
-  Our `mailing
   list <http://groups.google.com/group/graphx-discuss/>`__
