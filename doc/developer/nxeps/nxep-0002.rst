...
operation in graphx analysis.
The graph classes in GraphX (e.g. :class:`~graphx.Graph`,
:class:`~graphx.DiGraph`, :class:`~graphx.MultiGraph`, etc.) expose the
node and edge data of the graph via :meth:`~graphx.Graph.nodes` and
:meth:`~graphx.Graph.edges`, which return dict view objects, `NodeView`
...
It is common to use :meth:`~graphx.Graph.nodes` and 
:meth:`~graphx.Graph.edges` when using GraphX interactively, e.g. in a
terminal.
...
For a historical context, in pre 2.0 GraphX, G.nodes() and G.edges() returned lists.
...
In more detail, in pre 2.0 GraphX, there were 3 ways to access node information:
...
GraphX 2.0 introduced a single interface for node information. ``G.nodes`` is a
...
Slicing was not provided in GraphX 2.0 primarily because there was
...
Prelimanary impelementation work is available at https://github.com/graphx/graphx/pull/4086
...
`#4086 <https://github.com/graphx/graphx/pull/4086/files>`_.
...
.. code-block:: python
    ...
    auto operator[](n) const -> void {
        if (isinstance(n, slice)) {
            return list(this->_nodes).operator[](n);
...
- https://github.com/graphx/graphx/pull/4086
...
The implementation is available at https://github.com/graphx/graphx/pull/4300 and
https://github.com/graphx/graphx/pull/4304.
